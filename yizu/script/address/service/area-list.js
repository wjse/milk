define(function(){
    var areaList = [{
        "area_id": "1",
        "area_name": "\u5317\u4eac",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u5317"
    }, {
        "area_id": "2",
        "area_name": "\u5929\u6d25",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u5317"
    }, {
        "area_id": "3",
        "area_name": "\u6cb3\u5317",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u5317"
    }, {
        "area_id": "4",
        "area_name": "\u5c71\u897f",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u5317"
    }, {
        "area_id": "5",
        "area_name": "\u5185\u8499\u53e4",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u5317"
    }, {
        "area_id": "6",
        "area_name": "\u8fbd\u5b81",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u4e1c\u5317"
    }, {
        "area_id": "7",
        "area_name": "\u5409\u6797",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u4e1c\u5317"
    }, {
        "area_id": "8",
        "area_name": "\u9ed1\u9f99\u6c5f",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u4e1c\u5317"
    }, {
        "area_id": "9",
        "area_name": "\u4e0a\u6d77",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u4e1c"
    }, {
        "area_id": "10",
        "area_name": "\u6c5f\u82cf",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u4e1c"
    }, {
        "area_id": "11",
        "area_name": "\u6d59\u6c5f",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u4e1c"
    }, {
        "area_id": "12",
        "area_name": "\u5b89\u5fbd",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u4e1c"
    }, {
        "area_id": "13",
        "area_name": "\u798f\u5efa",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u5357"
    }, {
        "area_id": "14",
        "area_name": "\u6c5f\u897f",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u4e1c"
    }, {
        "area_id": "15",
        "area_name": "\u5c71\u4e1c",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u4e1c"
    }, {
        "area_id": "16",
        "area_name": "\u6cb3\u5357",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u4e2d"
    }, {
        "area_id": "17",
        "area_name": "\u6e56\u5317",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u4e2d"
    }, {
        "area_id": "18",
        "area_name": "\u6e56\u5357",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u4e2d"
    }, {
        "area_id": "19",
        "area_name": "\u5e7f\u4e1c",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u5357"
    }, {
        "area_id": "20",
        "area_name": "\u5e7f\u897f",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u5357"
    }, {
        "area_id": "21",
        "area_name": "\u6d77\u5357",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u534e\u5357"
    }, {
        "area_id": "22",
        "area_name": "\u91cd\u5e86",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u897f\u5357"
    }, {
        "area_id": "23",
        "area_name": "\u56db\u5ddd",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u897f\u5357"
    }, {
        "area_id": "24",
        "area_name": "\u8d35\u5dde",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u897f\u5357"
    }, {
        "area_id": "25",
        "area_name": "\u4e91\u5357",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u897f\u5357"
    }, {
        "area_id": "26",
        "area_name": "\u897f\u85cf",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u897f\u5357"
    }, {
        "area_id": "27",
        "area_name": "\u9655\u897f",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u897f\u5317"
    }, {
        "area_id": "28",
        "area_name": "\u7518\u8083",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u897f\u5317"
    }, {
        "area_id": "29",
        "area_name": "\u9752\u6d77",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u897f\u5317"
    }, {
        "area_id": "30",
        "area_name": "\u5b81\u590f",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u897f\u5317"
    }, {
        "area_id": "31",
        "area_name": "\u65b0\u7586",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u897f\u5317"
    }, {
        "area_id": "32",
        "area_name": "\u53f0\u6e7e",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u6e2f\u6fb3\u53f0"
    }, {
        "area_id": "33",
        "area_name": "\u9999\u6e2f",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u6e2f\u6fb3\u53f0"
    }, {
        "area_id": "34",
        "area_name": "\u6fb3\u95e8",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u6e2f\u6fb3\u53f0"
    }, {
        "area_id": "35",
        "area_name": "\u6d77\u5916",
        "area_parent_id": "0",
        "area_deep": "1",
        "area_region": "\u6d77\u5916"
    }, {
        "area_id": "36",
        "area_name": "\u5317\u4eac\u5e02",
        "area_parent_id": "1",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "37",
        "area_name": "\u4e1c\u57ce\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "38",
        "area_name": "\u897f\u57ce\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "39",
        "area_name": "\u4e0a\u6d77\u5e02",
        "area_parent_id": "9",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "40",
        "area_name": "\u5929\u6d25\u5e02",
        "area_parent_id": "2",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "41",
        "area_name": "\u671d\u9633\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "42",
        "area_name": "\u4e30\u53f0\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "43",
        "area_name": "\u77f3\u666f\u5c71\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "44",
        "area_name": "\u6d77\u6dc0\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "45",
        "area_name": "\u95e8\u5934\u6c9f\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "46",
        "area_name": "\u623f\u5c71\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "47",
        "area_name": "\u901a\u5dde\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "48",
        "area_name": "\u987a\u4e49\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "49",
        "area_name": "\u660c\u5e73\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "50",
        "area_name": "\u5927\u5174\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "51",
        "area_name": "\u6000\u67d4\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "52",
        "area_name": "\u5e73\u8c37\u533a",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "53",
        "area_name": "\u5bc6\u4e91\u53bf",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "54",
        "area_name": "\u5ef6\u5e86\u53bf",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "55",
        "area_name": "\u548c\u5e73\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "56",
        "area_name": "\u6cb3\u4e1c\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "57",
        "area_name": "\u6cb3\u897f\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "58",
        "area_name": "\u5357\u5f00\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "59",
        "area_name": "\u6cb3\u5317\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "60",
        "area_name": "\u7ea2\u6865\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "61",
        "area_name": "\u5858\u6cbd\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "62",
        "area_name": "\u91cd\u5e86\u5e02",
        "area_parent_id": "22",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "64",
        "area_name": "\u4e1c\u4e3d\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "65",
        "area_name": "\u897f\u9752\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "66",
        "area_name": "\u6d25\u5357\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "67",
        "area_name": "\u5317\u8fb0\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "68",
        "area_name": "\u6b66\u6e05\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "69",
        "area_name": "\u5b9d\u577b\u533a",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "70",
        "area_name": "\u5b81\u6cb3\u53bf",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "71",
        "area_name": "\u9759\u6d77\u53bf",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "72",
        "area_name": "\u84df\u53bf",
        "area_parent_id": "40",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "73",
        "area_name": "\u77f3\u5bb6\u5e84\u5e02",
        "area_parent_id": "3",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "74",
        "area_name": "\u5510\u5c71\u5e02",
        "area_parent_id": "3",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "75",
        "area_name": "\u79e6\u7687\u5c9b\u5e02",
        "area_parent_id": "3",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "76",
        "area_name": "\u90af\u90f8\u5e02",
        "area_parent_id": "3",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "77",
        "area_name": "\u90a2\u53f0\u5e02",
        "area_parent_id": "3",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "78",
        "area_name": "\u4fdd\u5b9a\u5e02",
        "area_parent_id": "3",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "79",
        "area_name": "\u5f20\u5bb6\u53e3\u5e02",
        "area_parent_id": "3",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "80",
        "area_name": "\u627f\u5fb7\u5e02",
        "area_parent_id": "3",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "81",
        "area_name": "\u8861\u6c34\u5e02",
        "area_parent_id": "3",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "82",
        "area_name": "\u5eca\u574a\u5e02",
        "area_parent_id": "3",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "83",
        "area_name": "\u6ca7\u5dde\u5e02",
        "area_parent_id": "3",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "84",
        "area_name": "\u592a\u539f\u5e02",
        "area_parent_id": "4",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "85",
        "area_name": "\u5927\u540c\u5e02",
        "area_parent_id": "4",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "86",
        "area_name": "\u9633\u6cc9\u5e02",
        "area_parent_id": "4",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "87",
        "area_name": "\u957f\u6cbb\u5e02",
        "area_parent_id": "4",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "88",
        "area_name": "\u664b\u57ce\u5e02",
        "area_parent_id": "4",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "89",
        "area_name": "\u6714\u5dde\u5e02",
        "area_parent_id": "4",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "90",
        "area_name": "\u664b\u4e2d\u5e02",
        "area_parent_id": "4",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "91",
        "area_name": "\u8fd0\u57ce\u5e02",
        "area_parent_id": "4",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "92",
        "area_name": "\u5ffb\u5dde\u5e02",
        "area_parent_id": "4",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "93",
        "area_name": "\u4e34\u6c7e\u5e02",
        "area_parent_id": "4",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "94",
        "area_name": "\u5415\u6881\u5e02",
        "area_parent_id": "4",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "95",
        "area_name": "\u547c\u548c\u6d69\u7279\u5e02",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "96",
        "area_name": "\u5305\u5934\u5e02",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "97",
        "area_name": "\u4e4c\u6d77\u5e02",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "98",
        "area_name": "\u8d64\u5cf0\u5e02",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "99",
        "area_name": "\u901a\u8fbd\u5e02",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "100",
        "area_name": "\u9102\u5c14\u591a\u65af\u5e02",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "101",
        "area_name": "\u547c\u4f26\u8d1d\u5c14\u5e02",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "102",
        "area_name": "\u5df4\u5f66\u6dd6\u5c14\u5e02",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "103",
        "area_name": "\u4e4c\u5170\u5bdf\u5e03\u5e02",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "104",
        "area_name": "\u5174\u5b89\u76df",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "105",
        "area_name": "\u9521\u6797\u90ed\u52d2\u76df",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "106",
        "area_name": "\u963f\u62c9\u5584\u76df",
        "area_parent_id": "5",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "107",
        "area_name": "\u6c88\u9633\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "108",
        "area_name": "\u5927\u8fde\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "109",
        "area_name": "\u978d\u5c71\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "110",
        "area_name": "\u629a\u987a\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "111",
        "area_name": "\u672c\u6eaa\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "112",
        "area_name": "\u4e39\u4e1c\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "113",
        "area_name": "\u9526\u5dde\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "114",
        "area_name": "\u8425\u53e3\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "115",
        "area_name": "\u961c\u65b0\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "116",
        "area_name": "\u8fbd\u9633\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "117",
        "area_name": "\u76d8\u9526\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "118",
        "area_name": "\u94c1\u5cad\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "119",
        "area_name": "\u671d\u9633\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "120",
        "area_name": "\u846b\u82a6\u5c9b\u5e02",
        "area_parent_id": "6",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "121",
        "area_name": "\u957f\u6625\u5e02",
        "area_parent_id": "7",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "122",
        "area_name": "\u5409\u6797\u5e02",
        "area_parent_id": "7",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "123",
        "area_name": "\u56db\u5e73\u5e02",
        "area_parent_id": "7",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "124",
        "area_name": "\u8fbd\u6e90\u5e02",
        "area_parent_id": "7",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "125",
        "area_name": "\u901a\u5316\u5e02",
        "area_parent_id": "7",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "126",
        "area_name": "\u767d\u5c71\u5e02",
        "area_parent_id": "7",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "127",
        "area_name": "\u677e\u539f\u5e02",
        "area_parent_id": "7",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "128",
        "area_name": "\u767d\u57ce\u5e02",
        "area_parent_id": "7",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "129",
        "area_name": "\u5ef6\u8fb9\u671d\u9c9c\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "7",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "130",
        "area_name": "\u54c8\u5c14\u6ee8\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "131",
        "area_name": "\u9f50\u9f50\u54c8\u5c14\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "132",
        "area_name": "\u9e21\u897f\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "133",
        "area_name": "\u9e64\u5c97\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "134",
        "area_name": "\u53cc\u9e2d\u5c71\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "135",
        "area_name": "\u5927\u5e86\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "136",
        "area_name": "\u4f0a\u6625\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "137",
        "area_name": "\u4f73\u6728\u65af\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "138",
        "area_name": "\u4e03\u53f0\u6cb3\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "139",
        "area_name": "\u7261\u4e39\u6c5f\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "140",
        "area_name": "\u9ed1\u6cb3\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "141",
        "area_name": "\u7ee5\u5316\u5e02",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "142",
        "area_name": "\u5927\u5174\u5b89\u5cad\u5730\u533a",
        "area_parent_id": "8",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "143",
        "area_name": "\u9ec4\u6d66\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "144",
        "area_name": "\u5362\u6e7e\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "145",
        "area_name": "\u5f90\u6c47\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "146",
        "area_name": "\u957f\u5b81\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "147",
        "area_name": "\u9759\u5b89\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "148",
        "area_name": "\u666e\u9640\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "149",
        "area_name": "\u95f8\u5317\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "150",
        "area_name": "\u8679\u53e3\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "151",
        "area_name": "\u6768\u6d66\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "152",
        "area_name": "\u95f5\u884c\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "153",
        "area_name": "\u5b9d\u5c71\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "154",
        "area_name": "\u5609\u5b9a\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "155",
        "area_name": "\u6d66\u4e1c\u65b0\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "156",
        "area_name": "\u91d1\u5c71\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "157",
        "area_name": "\u677e\u6c5f\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "158",
        "area_name": "\u9752\u6d66\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "159",
        "area_name": "\u5357\u6c47\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "160",
        "area_name": "\u5949\u8d24\u533a",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "161",
        "area_name": "\u5d07\u660e\u53bf",
        "area_parent_id": "39",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "162",
        "area_name": "\u5357\u4eac\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "163",
        "area_name": "\u65e0\u9521\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "164",
        "area_name": "\u5f90\u5dde\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "165",
        "area_name": "\u5e38\u5dde\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "166",
        "area_name": "\u82cf\u5dde\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "167",
        "area_name": "\u5357\u901a\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "168",
        "area_name": "\u8fde\u4e91\u6e2f\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "169",
        "area_name": "\u6dee\u5b89\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "170",
        "area_name": "\u76d0\u57ce\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "171",
        "area_name": "\u626c\u5dde\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "172",
        "area_name": "\u9547\u6c5f\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "173",
        "area_name": "\u6cf0\u5dde\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "174",
        "area_name": "\u5bbf\u8fc1\u5e02",
        "area_parent_id": "10",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "175",
        "area_name": "\u676d\u5dde\u5e02",
        "area_parent_id": "11",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "176",
        "area_name": "\u5b81\u6ce2\u5e02",
        "area_parent_id": "11",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "177",
        "area_name": "\u6e29\u5dde\u5e02",
        "area_parent_id": "11",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "178",
        "area_name": "\u5609\u5174\u5e02",
        "area_parent_id": "11",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "179",
        "area_name": "\u6e56\u5dde\u5e02",
        "area_parent_id": "11",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "180",
        "area_name": "\u7ecd\u5174\u5e02",
        "area_parent_id": "11",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "181",
        "area_name": "\u821f\u5c71\u5e02",
        "area_parent_id": "11",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "182",
        "area_name": "\u8862\u5dde\u5e02",
        "area_parent_id": "11",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "183",
        "area_name": "\u91d1\u534e\u5e02",
        "area_parent_id": "11",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "184",
        "area_name": "\u53f0\u5dde\u5e02",
        "area_parent_id": "11",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "185",
        "area_name": "\u4e3d\u6c34\u5e02",
        "area_parent_id": "11",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "186",
        "area_name": "\u5408\u80a5\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "187",
        "area_name": "\u829c\u6e56\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "188",
        "area_name": "\u868c\u57e0\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "189",
        "area_name": "\u6dee\u5357\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "190",
        "area_name": "\u9a6c\u978d\u5c71\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "191",
        "area_name": "\u6dee\u5317\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "192",
        "area_name": "\u94dc\u9675\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "193",
        "area_name": "\u5b89\u5e86\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "194",
        "area_name": "\u9ec4\u5c71\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "195",
        "area_name": "\u6ec1\u5dde\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "196",
        "area_name": "\u961c\u9633\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "197",
        "area_name": "\u5bbf\u5dde\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "198",
        "area_name": "\u5de2\u6e56\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "199",
        "area_name": "\u516d\u5b89\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "200",
        "area_name": "\u4eb3\u5dde\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "201",
        "area_name": "\u6c60\u5dde\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "202",
        "area_name": "\u5ba3\u57ce\u5e02",
        "area_parent_id": "12",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "203",
        "area_name": "\u798f\u5dde\u5e02",
        "area_parent_id": "13",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "204",
        "area_name": "\u53a6\u95e8\u5e02",
        "area_parent_id": "13",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "205",
        "area_name": "\u8386\u7530\u5e02",
        "area_parent_id": "13",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "206",
        "area_name": "\u4e09\u660e\u5e02",
        "area_parent_id": "13",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "207",
        "area_name": "\u6cc9\u5dde\u5e02",
        "area_parent_id": "13",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "208",
        "area_name": "\u6f33\u5dde\u5e02",
        "area_parent_id": "13",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "209",
        "area_name": "\u5357\u5e73\u5e02",
        "area_parent_id": "13",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "210",
        "area_name": "\u9f99\u5ca9\u5e02",
        "area_parent_id": "13",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "211",
        "area_name": "\u5b81\u5fb7\u5e02",
        "area_parent_id": "13",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "212",
        "area_name": "\u5357\u660c\u5e02",
        "area_parent_id": "14",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "213",
        "area_name": "\u666f\u5fb7\u9547\u5e02",
        "area_parent_id": "14",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "214",
        "area_name": "\u840d\u4e61\u5e02",
        "area_parent_id": "14",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "215",
        "area_name": "\u4e5d\u6c5f\u5e02",
        "area_parent_id": "14",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "216",
        "area_name": "\u65b0\u4f59\u5e02",
        "area_parent_id": "14",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "217",
        "area_name": "\u9e70\u6f6d\u5e02",
        "area_parent_id": "14",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "218",
        "area_name": "\u8d63\u5dde\u5e02",
        "area_parent_id": "14",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "219",
        "area_name": "\u5409\u5b89\u5e02",
        "area_parent_id": "14",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "220",
        "area_name": "\u5b9c\u6625\u5e02",
        "area_parent_id": "14",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "221",
        "area_name": "\u629a\u5dde\u5e02",
        "area_parent_id": "14",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "222",
        "area_name": "\u4e0a\u9976\u5e02",
        "area_parent_id": "14",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "223",
        "area_name": "\u6d4e\u5357\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "224",
        "area_name": "\u9752\u5c9b\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "225",
        "area_name": "\u6dc4\u535a\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "226",
        "area_name": "\u67a3\u5e84\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "227",
        "area_name": "\u4e1c\u8425\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "228",
        "area_name": "\u70df\u53f0\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "229",
        "area_name": "\u6f4d\u574a\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "230",
        "area_name": "\u6d4e\u5b81\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "231",
        "area_name": "\u6cf0\u5b89\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "232",
        "area_name": "\u5a01\u6d77\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "233",
        "area_name": "\u65e5\u7167\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "234",
        "area_name": "\u83b1\u829c\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "235",
        "area_name": "\u4e34\u6c82\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "236",
        "area_name": "\u5fb7\u5dde\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "237",
        "area_name": "\u804a\u57ce\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "238",
        "area_name": "\u6ee8\u5dde\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "239",
        "area_name": "\u83cf\u6cfd\u5e02",
        "area_parent_id": "15",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "240",
        "area_name": "\u90d1\u5dde\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "241",
        "area_name": "\u5f00\u5c01\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "242",
        "area_name": "\u6d1b\u9633\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "243",
        "area_name": "\u5e73\u9876\u5c71\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "244",
        "area_name": "\u5b89\u9633\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "245",
        "area_name": "\u9e64\u58c1\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "246",
        "area_name": "\u65b0\u4e61\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "247",
        "area_name": "\u7126\u4f5c\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "248",
        "area_name": "\u6fee\u9633\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "249",
        "area_name": "\u8bb8\u660c\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "250",
        "area_name": "\u6f2f\u6cb3\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "251",
        "area_name": "\u4e09\u95e8\u5ce1\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "252",
        "area_name": "\u5357\u9633\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "253",
        "area_name": "\u5546\u4e18\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "254",
        "area_name": "\u4fe1\u9633\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "255",
        "area_name": "\u5468\u53e3\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "256",
        "area_name": "\u9a7b\u9a6c\u5e97\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "257",
        "area_name": "\u6d4e\u6e90\u5e02",
        "area_parent_id": "16",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "258",
        "area_name": "\u6b66\u6c49\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "259",
        "area_name": "\u9ec4\u77f3\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "260",
        "area_name": "\u5341\u5830\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "261",
        "area_name": "\u5b9c\u660c\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "262",
        "area_name": "\u8944\u6a0a\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "263",
        "area_name": "\u9102\u5dde\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "264",
        "area_name": "\u8346\u95e8\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "265",
        "area_name": "\u5b5d\u611f\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "266",
        "area_name": "\u8346\u5dde\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "267",
        "area_name": "\u9ec4\u5188\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "268",
        "area_name": "\u54b8\u5b81\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "269",
        "area_name": "\u968f\u5dde\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "270",
        "area_name": "\u6069\u65bd\u571f\u5bb6\u65cf\u82d7\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "271",
        "area_name": "\u4ed9\u6843\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "272",
        "area_name": "\u6f5c\u6c5f\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "273",
        "area_name": "\u5929\u95e8\u5e02",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "274",
        "area_name": "\u795e\u519c\u67b6\u6797\u533a",
        "area_parent_id": "17",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "275",
        "area_name": "\u957f\u6c99\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "276",
        "area_name": "\u682a\u6d32\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "277",
        "area_name": "\u6e58\u6f6d\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "278",
        "area_name": "\u8861\u9633\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "279",
        "area_name": "\u90b5\u9633\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "280",
        "area_name": "\u5cb3\u9633\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "281",
        "area_name": "\u5e38\u5fb7\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "282",
        "area_name": "\u5f20\u5bb6\u754c\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "283",
        "area_name": "\u76ca\u9633\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "284",
        "area_name": "\u90f4\u5dde\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "285",
        "area_name": "\u6c38\u5dde\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "286",
        "area_name": "\u6000\u5316\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "287",
        "area_name": "\u5a04\u5e95\u5e02",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "288",
        "area_name": "\u6e58\u897f\u571f\u5bb6\u65cf\u82d7\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "18",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "289",
        "area_name": "\u5e7f\u5dde\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "290",
        "area_name": "\u97f6\u5173\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "291",
        "area_name": "\u6df1\u5733\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "292",
        "area_name": "\u73e0\u6d77\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "293",
        "area_name": "\u6c55\u5934\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "294",
        "area_name": "\u4f5b\u5c71\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "295",
        "area_name": "\u6c5f\u95e8\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "296",
        "area_name": "\u6e5b\u6c5f\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "297",
        "area_name": "\u8302\u540d\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "298",
        "area_name": "\u8087\u5e86\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "299",
        "area_name": "\u60e0\u5dde\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "300",
        "area_name": "\u6885\u5dde\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "301",
        "area_name": "\u6c55\u5c3e\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "302",
        "area_name": "\u6cb3\u6e90\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "303",
        "area_name": "\u9633\u6c5f\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "304",
        "area_name": "\u6e05\u8fdc\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "305",
        "area_name": "\u4e1c\u839e\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "306",
        "area_name": "\u4e2d\u5c71\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "307",
        "area_name": "\u6f6e\u5dde\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "308",
        "area_name": "\u63ed\u9633\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "309",
        "area_name": "\u4e91\u6d6e\u5e02",
        "area_parent_id": "19",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "310",
        "area_name": "\u5357\u5b81\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "311",
        "area_name": "\u67f3\u5dde\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "312",
        "area_name": "\u6842\u6797\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "313",
        "area_name": "\u68a7\u5dde\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "314",
        "area_name": "\u5317\u6d77\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "315",
        "area_name": "\u9632\u57ce\u6e2f\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "316",
        "area_name": "\u94a6\u5dde\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "317",
        "area_name": "\u8d35\u6e2f\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "318",
        "area_name": "\u7389\u6797\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "319",
        "area_name": "\u767e\u8272\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "320",
        "area_name": "\u8d3a\u5dde\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "321",
        "area_name": "\u6cb3\u6c60\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "322",
        "area_name": "\u6765\u5bbe\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "323",
        "area_name": "\u5d07\u5de6\u5e02",
        "area_parent_id": "20",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "324",
        "area_name": "\u6d77\u53e3\u5e02",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "325",
        "area_name": "\u4e09\u4e9a\u5e02",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "326",
        "area_name": "\u4e94\u6307\u5c71\u5e02",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "327",
        "area_name": "\u743c\u6d77\u5e02",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "328",
        "area_name": "\u510b\u5dde\u5e02",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "329",
        "area_name": "\u6587\u660c\u5e02",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "330",
        "area_name": "\u4e07\u5b81\u5e02",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "331",
        "area_name": "\u4e1c\u65b9\u5e02",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "332",
        "area_name": "\u5b9a\u5b89\u53bf",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "333",
        "area_name": "\u5c6f\u660c\u53bf",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "334",
        "area_name": "\u6f84\u8fc8\u53bf",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "335",
        "area_name": "\u4e34\u9ad8\u53bf",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "336",
        "area_name": "\u767d\u6c99\u9ece\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "337",
        "area_name": "\u660c\u6c5f\u9ece\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "338",
        "area_name": "\u4e50\u4e1c\u9ece\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "339",
        "area_name": "\u9675\u6c34\u9ece\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "340",
        "area_name": "\u4fdd\u4ead\u9ece\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "341",
        "area_name": "\u743c\u4e2d\u9ece\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "342",
        "area_name": "\u897f\u6c99\u7fa4\u5c9b",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "343",
        "area_name": "\u5357\u6c99\u7fa4\u5c9b",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "344",
        "area_name": "\u4e2d\u6c99\u7fa4\u5c9b\u7684\u5c9b\u7901\u53ca\u5176\u6d77\u57df",
        "area_parent_id": "21",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "345",
        "area_name": "\u4e07\u5dde\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "346",
        "area_name": "\u6daa\u9675\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "347",
        "area_name": "\u6e1d\u4e2d\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "348",
        "area_name": "\u5927\u6e21\u53e3\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "349",
        "area_name": "\u6c5f\u5317\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "350",
        "area_name": "\u6c99\u576a\u575d\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "351",
        "area_name": "\u4e5d\u9f99\u5761\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "352",
        "area_name": "\u5357\u5cb8\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "353",
        "area_name": "\u5317\u789a\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "354",
        "area_name": "\u53cc\u6865\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "355",
        "area_name": "\u4e07\u76db\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "356",
        "area_name": "\u6e1d\u5317\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "357",
        "area_name": "\u5df4\u5357\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "358",
        "area_name": "\u9ed4\u6c5f\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "359",
        "area_name": "\u957f\u5bff\u533a",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "360",
        "area_name": "\u7da6\u6c5f\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "361",
        "area_name": "\u6f7c\u5357\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "362",
        "area_name": "\u94dc\u6881\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "363",
        "area_name": "\u5927\u8db3\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "364",
        "area_name": "\u8363\u660c\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "365",
        "area_name": "\u74a7\u5c71\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "366",
        "area_name": "\u6881\u5e73\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "367",
        "area_name": "\u57ce\u53e3\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "368",
        "area_name": "\u4e30\u90fd\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "369",
        "area_name": "\u57ab\u6c5f\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "370",
        "area_name": "\u6b66\u9686\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "371",
        "area_name": "\u5fe0\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "372",
        "area_name": "\u5f00\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "373",
        "area_name": "\u4e91\u9633\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "374",
        "area_name": "\u5949\u8282\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "375",
        "area_name": "\u5deb\u5c71\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "376",
        "area_name": "\u5deb\u6eaa\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "377",
        "area_name": "\u77f3\u67f1\u571f\u5bb6\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "378",
        "area_name": "\u79c0\u5c71\u571f\u5bb6\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "379",
        "area_name": "\u9149\u9633\u571f\u5bb6\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "380",
        "area_name": "\u5f6d\u6c34\u82d7\u65cf\u571f\u5bb6\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "381",
        "area_name": "\u6c5f\u6d25\u5e02",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "382",
        "area_name": "\u5408\u5ddd\u5e02",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "383",
        "area_name": "\u6c38\u5ddd\u5e02",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "384",
        "area_name": "\u5357\u5ddd\u5e02",
        "area_parent_id": "62",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "385",
        "area_name": "\u6210\u90fd\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "386",
        "area_name": "\u81ea\u8d21\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "387",
        "area_name": "\u6500\u679d\u82b1\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "388",
        "area_name": "\u6cf8\u5dde\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "389",
        "area_name": "\u5fb7\u9633\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "390",
        "area_name": "\u7ef5\u9633\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "391",
        "area_name": "\u5e7f\u5143\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "392",
        "area_name": "\u9042\u5b81\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "393",
        "area_name": "\u5185\u6c5f\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "394",
        "area_name": "\u4e50\u5c71\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "395",
        "area_name": "\u5357\u5145\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "396",
        "area_name": "\u7709\u5c71\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "397",
        "area_name": "\u5b9c\u5bbe\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "398",
        "area_name": "\u5e7f\u5b89\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "399",
        "area_name": "\u8fbe\u5dde\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "400",
        "area_name": "\u96c5\u5b89\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "401",
        "area_name": "\u5df4\u4e2d\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "402",
        "area_name": "\u8d44\u9633\u5e02",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "403",
        "area_name": "\u963f\u575d\u85cf\u65cf\u7f8c\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "404",
        "area_name": "\u7518\u5b5c\u85cf\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "405",
        "area_name": "\u51c9\u5c71\u5f5d\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "23",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "406",
        "area_name": "\u8d35\u9633\u5e02",
        "area_parent_id": "24",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "407",
        "area_name": "\u516d\u76d8\u6c34\u5e02",
        "area_parent_id": "24",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "408",
        "area_name": "\u9075\u4e49\u5e02",
        "area_parent_id": "24",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "409",
        "area_name": "\u5b89\u987a\u5e02",
        "area_parent_id": "24",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "410",
        "area_name": "\u94dc\u4ec1\u5730\u533a",
        "area_parent_id": "24",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "411",
        "area_name": "\u9ed4\u897f\u5357\u5e03\u4f9d\u65cf\u82d7\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "24",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "412",
        "area_name": "\u6bd5\u8282\u5730\u533a",
        "area_parent_id": "24",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "413",
        "area_name": "\u9ed4\u4e1c\u5357\u82d7\u65cf\u4f97\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "24",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "414",
        "area_name": "\u9ed4\u5357\u5e03\u4f9d\u65cf\u82d7\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "24",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "415",
        "area_name": "\u6606\u660e\u5e02",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "416",
        "area_name": "\u66f2\u9756\u5e02",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "417",
        "area_name": "\u7389\u6eaa\u5e02",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "418",
        "area_name": "\u4fdd\u5c71\u5e02",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "419",
        "area_name": "\u662d\u901a\u5e02",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "420",
        "area_name": "\u4e3d\u6c5f\u5e02",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "421",
        "area_name": "\u601d\u8305\u5e02",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "422",
        "area_name": "\u4e34\u6ca7\u5e02",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "423",
        "area_name": "\u695a\u96c4\u5f5d\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "424",
        "area_name": "\u7ea2\u6cb3\u54c8\u5c3c\u65cf\u5f5d\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "425",
        "area_name": "\u6587\u5c71\u58ee\u65cf\u82d7\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "426",
        "area_name": "\u897f\u53cc\u7248\u7eb3\u50a3\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "427",
        "area_name": "\u5927\u7406\u767d\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "428",
        "area_name": "\u5fb7\u5b8f\u50a3\u65cf\u666f\u9887\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "429",
        "area_name": "\u6012\u6c5f\u5088\u50f3\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "430",
        "area_name": "\u8fea\u5e86\u85cf\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "25",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "431",
        "area_name": "\u62c9\u8428\u5e02",
        "area_parent_id": "26",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "432",
        "area_name": "\u660c\u90fd\u5730\u533a",
        "area_parent_id": "26",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "433",
        "area_name": "\u5c71\u5357\u5730\u533a",
        "area_parent_id": "26",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "434",
        "area_name": "\u65e5\u5580\u5219\u5730\u533a",
        "area_parent_id": "26",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "435",
        "area_name": "\u90a3\u66f2\u5730\u533a",
        "area_parent_id": "26",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "436",
        "area_name": "\u963f\u91cc\u5730\u533a",
        "area_parent_id": "26",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "437",
        "area_name": "\u6797\u829d\u5730\u533a",
        "area_parent_id": "26",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "438",
        "area_name": "\u897f\u5b89\u5e02",
        "area_parent_id": "27",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "439",
        "area_name": "\u94dc\u5ddd\u5e02",
        "area_parent_id": "27",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "440",
        "area_name": "\u5b9d\u9e21\u5e02",
        "area_parent_id": "27",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "441",
        "area_name": "\u54b8\u9633\u5e02",
        "area_parent_id": "27",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "442",
        "area_name": "\u6e2d\u5357\u5e02",
        "area_parent_id": "27",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "443",
        "area_name": "\u5ef6\u5b89\u5e02",
        "area_parent_id": "27",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "444",
        "area_name": "\u6c49\u4e2d\u5e02",
        "area_parent_id": "27",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "445",
        "area_name": "\u6986\u6797\u5e02",
        "area_parent_id": "27",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "446",
        "area_name": "\u5b89\u5eb7\u5e02",
        "area_parent_id": "27",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "447",
        "area_name": "\u5546\u6d1b\u5e02",
        "area_parent_id": "27",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "448",
        "area_name": "\u5170\u5dde\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "449",
        "area_name": "\u5609\u5cea\u5173\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "450",
        "area_name": "\u91d1\u660c\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "451",
        "area_name": "\u767d\u94f6\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "452",
        "area_name": "\u5929\u6c34\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "453",
        "area_name": "\u6b66\u5a01\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "454",
        "area_name": "\u5f20\u6396\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "455",
        "area_name": "\u5e73\u51c9\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "456",
        "area_name": "\u9152\u6cc9\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "457",
        "area_name": "\u5e86\u9633\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "458",
        "area_name": "\u5b9a\u897f\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "459",
        "area_name": "\u9647\u5357\u5e02",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "460",
        "area_name": "\u4e34\u590f\u56de\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "461",
        "area_name": "\u7518\u5357\u85cf\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "28",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "462",
        "area_name": "\u897f\u5b81\u5e02",
        "area_parent_id": "29",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "463",
        "area_name": "\u6d77\u4e1c\u5730\u533a",
        "area_parent_id": "29",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "464",
        "area_name": "\u6d77\u5317\u85cf\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "29",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "465",
        "area_name": "\u9ec4\u5357\u85cf\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "29",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "466",
        "area_name": "\u6d77\u5357\u85cf\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "29",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "467",
        "area_name": "\u679c\u6d1b\u85cf\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "29",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "468",
        "area_name": "\u7389\u6811\u85cf\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "29",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "469",
        "area_name": "\u6d77\u897f\u8499\u53e4\u65cf\u85cf\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "29",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "470",
        "area_name": "\u94f6\u5ddd\u5e02",
        "area_parent_id": "30",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "471",
        "area_name": "\u77f3\u5634\u5c71\u5e02",
        "area_parent_id": "30",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "472",
        "area_name": "\u5434\u5fe0\u5e02",
        "area_parent_id": "30",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "473",
        "area_name": "\u56fa\u539f\u5e02",
        "area_parent_id": "30",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "474",
        "area_name": "\u4e2d\u536b\u5e02",
        "area_parent_id": "30",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "475",
        "area_name": "\u4e4c\u9c81\u6728\u9f50\u5e02",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "476",
        "area_name": "\u514b\u62c9\u739b\u4f9d\u5e02",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "477",
        "area_name": "\u5410\u9c81\u756a\u5730\u533a",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "478",
        "area_name": "\u54c8\u5bc6\u5730\u533a",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "479",
        "area_name": "\u660c\u5409\u56de\u65cf\u81ea\u6cbb\u5dde",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "480",
        "area_name": "\u535a\u5c14\u5854\u62c9\u8499\u53e4\u81ea\u6cbb\u5dde",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "481",
        "area_name": "\u5df4\u97f3\u90ed\u695e\u8499\u53e4\u81ea\u6cbb\u5dde",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "482",
        "area_name": "\u963f\u514b\u82cf\u5730\u533a",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "483",
        "area_name": "\u514b\u5b5c\u52d2\u82cf\u67ef\u5c14\u514b\u5b5c\u81ea\u6cbb\u5dde",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "484",
        "area_name": "\u5580\u4ec0\u5730\u533a",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "485",
        "area_name": "\u548c\u7530\u5730\u533a",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "486",
        "area_name": "\u4f0a\u7281\u54c8\u8428\u514b\u81ea\u6cbb\u5dde",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "487",
        "area_name": "\u5854\u57ce\u5730\u533a",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "488",
        "area_name": "\u963f\u52d2\u6cf0\u5730\u533a",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "489",
        "area_name": "\u77f3\u6cb3\u5b50\u5e02",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "490",
        "area_name": "\u963f\u62c9\u5c14\u5e02",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "491",
        "area_name": "\u56fe\u6728\u8212\u514b\u5e02",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "492",
        "area_name": "\u4e94\u5bb6\u6e20\u5e02",
        "area_parent_id": "31",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "493",
        "area_name": "\u53f0\u5317\u5e02",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "494",
        "area_name": "\u9ad8\u96c4\u5e02",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "495",
        "area_name": "\u57fa\u9686\u5e02",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "496",
        "area_name": "\u53f0\u4e2d\u5e02",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "497",
        "area_name": "\u53f0\u5357\u5e02",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "498",
        "area_name": "\u65b0\u7af9\u5e02",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "499",
        "area_name": "\u5609\u4e49\u5e02",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "500",
        "area_name": "\u53f0\u5317\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "501",
        "area_name": "\u5b9c\u5170\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "502",
        "area_name": "\u6843\u56ed\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "503",
        "area_name": "\u65b0\u7af9\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "504",
        "area_name": "\u82d7\u6817\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "505",
        "area_name": "\u53f0\u4e2d\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "506",
        "area_name": "\u5f70\u5316\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "507",
        "area_name": "\u5357\u6295\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "508",
        "area_name": "\u4e91\u6797\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "509",
        "area_name": "\u5609\u4e49\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "510",
        "area_name": "\u53f0\u5357\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "511",
        "area_name": "\u9ad8\u96c4\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "512",
        "area_name": "\u5c4f\u4e1c\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "513",
        "area_name": "\u6f8e\u6e56\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "514",
        "area_name": "\u53f0\u4e1c\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "515",
        "area_name": "\u82b1\u83b2\u53bf",
        "area_parent_id": "32",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "516",
        "area_name": "\u4e2d\u897f\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "517",
        "area_name": "\u4e1c\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "518",
        "area_name": "\u4e5d\u9f99\u57ce\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "519",
        "area_name": "\u89c2\u5858\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "520",
        "area_name": "\u5357\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "521",
        "area_name": "\u6df1\u6c34\u57d7\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "522",
        "area_name": "\u9ec4\u5927\u4ed9\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "523",
        "area_name": "\u6e7e\u4ed4\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "524",
        "area_name": "\u6cb9\u5c16\u65fa\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "525",
        "area_name": "\u79bb\u5c9b\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "526",
        "area_name": "\u8475\u9752\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "527",
        "area_name": "\u5317\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "528",
        "area_name": "\u897f\u8d21\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "529",
        "area_name": "\u6c99\u7530\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "530",
        "area_name": "\u5c6f\u95e8\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "531",
        "area_name": "\u5927\u57d4\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "532",
        "area_name": "\u8343\u6e7e\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "533",
        "area_name": "\u5143\u6717\u533a",
        "area_parent_id": "33",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "534",
        "area_name": "\u6fb3\u95e8\u7279\u522b\u884c\u653f\u533a",
        "area_parent_id": "34",
        "area_deep": "2",
        "area_region": null
    }, {
        "area_id": "535",
        "area_name": "\u7f8e\u56fd",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "536",
        "area_name": "\u52a0\u62ff\u5927",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "537",
        "area_name": "\u6fb3\u5927\u5229\u4e9a",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "538",
        "area_name": "\u65b0\u897f\u5170",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "539",
        "area_name": "\u82f1\u56fd",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "540",
        "area_name": "\u6cd5\u56fd",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "541",
        "area_name": "\u5fb7\u56fd",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "542",
        "area_name": "\u6377\u514b",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "543",
        "area_name": "\u8377\u5170",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "544",
        "area_name": "\u745e\u58eb",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "545",
        "area_name": "\u5e0c\u814a",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "546",
        "area_name": "\u632a\u5a01",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "547",
        "area_name": "\u745e\u5178",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "548",
        "area_name": "\u4e39\u9ea6",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "549",
        "area_name": "\u82ac\u5170",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "550",
        "area_name": "\u7231\u5c14\u5170",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "551",
        "area_name": "\u5965\u5730\u5229",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "552",
        "area_name": "\u610f\u5927\u5229",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "553",
        "area_name": "\u4e4c\u514b\u5170",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "554",
        "area_name": "\u4fc4\u7f57\u65af",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "555",
        "area_name": "\u897f\u73ed\u7259",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "556",
        "area_name": "\u97e9\u56fd",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "557",
        "area_name": "\u65b0\u52a0\u5761",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "558",
        "area_name": "\u9a6c\u6765\u897f\u4e9a",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "559",
        "area_name": "\u5370\u5ea6",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "560",
        "area_name": "\u6cf0\u56fd",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "561",
        "area_name": "\u65e5\u672c",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "562",
        "area_name": "\u5df4\u897f",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "563",
        "area_name": "\u963f\u6839\u5ef7",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "564",
        "area_name": "\u5357\u975e",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "565",
        "area_name": "\u57c3\u53ca",
        "area_parent_id": "45055",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "566",
        "area_name": "\u5176\u4ed6",
        "area_parent_id": "36",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1126",
        "area_name": "\u4e95\u9649\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1127",
        "area_name": "\u4e95\u9649\u77ff\u533a",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1128",
        "area_name": "\u5143\u6c0f\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1129",
        "area_name": "\u5e73\u5c71\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1130",
        "area_name": "\u65b0\u4e50\u5e02",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1131",
        "area_name": "\u65b0\u534e\u533a",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1132",
        "area_name": "\u65e0\u6781\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1133",
        "area_name": "\u664b\u5dde\u5e02",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1134",
        "area_name": "\u683e\u57ce\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1135",
        "area_name": "\u6865\u4e1c\u533a",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1136",
        "area_name": "\u6865\u897f\u533a",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1137",
        "area_name": "\u6b63\u5b9a\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1138",
        "area_name": "\u6df1\u6cfd\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1139",
        "area_name": "\u7075\u5bff\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1140",
        "area_name": "\u85c1\u57ce\u5e02",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1141",
        "area_name": "\u884c\u5510\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1142",
        "area_name": "\u88d5\u534e\u533a",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1143",
        "area_name": "\u8d5e\u7687\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1144",
        "area_name": "\u8d75\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1145",
        "area_name": "\u8f9b\u96c6\u5e02",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1146",
        "area_name": "\u957f\u5b89\u533a",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1147",
        "area_name": "\u9ad8\u9091\u53bf",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1148",
        "area_name": "\u9e7f\u6cc9\u5e02",
        "area_parent_id": "73",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1149",
        "area_name": "\u4e30\u5357\u533a",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1150",
        "area_name": "\u4e30\u6da6\u533a",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1151",
        "area_name": "\u4e50\u4ead\u53bf",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1152",
        "area_name": "\u53e4\u51b6\u533a",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1153",
        "area_name": "\u5510\u6d77\u53bf",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1154",
        "area_name": "\u5f00\u5e73\u533a",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1155",
        "area_name": "\u6ee6\u5357\u53bf",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1156",
        "area_name": "\u6ee6\u53bf",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1157",
        "area_name": "\u7389\u7530\u53bf",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1158",
        "area_name": "\u8def\u5317\u533a",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1159",
        "area_name": "\u8def\u5357\u533a",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1160",
        "area_name": "\u8fc1\u5b89\u5e02",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1161",
        "area_name": "\u8fc1\u897f\u53bf",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1162",
        "area_name": "\u9075\u5316\u5e02",
        "area_parent_id": "74",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1163",
        "area_name": "\u5317\u6234\u6cb3\u533a",
        "area_parent_id": "75",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1164",
        "area_name": "\u5362\u9f99\u53bf",
        "area_parent_id": "75",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1165",
        "area_name": "\u5c71\u6d77\u5173\u533a",
        "area_parent_id": "75",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1166",
        "area_name": "\u629a\u5b81\u53bf",
        "area_parent_id": "75",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1167",
        "area_name": "\u660c\u9ece\u53bf",
        "area_parent_id": "75",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1168",
        "area_name": "\u6d77\u6e2f\u533a",
        "area_parent_id": "75",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1169",
        "area_name": "\u9752\u9f99\u6ee1\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "75",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1170",
        "area_name": "\u4e1b\u53f0\u533a",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1171",
        "area_name": "\u4e34\u6f33\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1172",
        "area_name": "\u590d\u5174\u533a",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1173",
        "area_name": "\u5927\u540d\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1174",
        "area_name": "\u5cf0\u5cf0\u77ff\u533a",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1175",
        "area_name": "\u5e7f\u5e73\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1176",
        "area_name": "\u6210\u5b89\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1177",
        "area_name": "\u66f2\u5468\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1178",
        "area_name": "\u6b66\u5b89\u5e02",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1179",
        "area_name": "\u6c38\u5e74\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1180",
        "area_name": "\u6d89\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1181",
        "area_name": "\u78c1\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1182",
        "area_name": "\u80a5\u4e61\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1183",
        "area_name": "\u90af\u5c71\u533a",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1184",
        "area_name": "\u90af\u90f8\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1185",
        "area_name": "\u90b1\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1186",
        "area_name": "\u9986\u9676\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1187",
        "area_name": "\u9b4f\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1188",
        "area_name": "\u9e21\u6cfd\u53bf",
        "area_parent_id": "76",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1189",
        "area_name": "\u4e34\u57ce\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1190",
        "area_name": "\u4e34\u897f\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1191",
        "area_name": "\u4efb\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1192",
        "area_name": "\u5185\u4e18\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1193",
        "area_name": "\u5357\u548c\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1194",
        "area_name": "\u5357\u5bab\u5e02",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1195",
        "area_name": "\u5a01\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1196",
        "area_name": "\u5b81\u664b\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1197",
        "area_name": "\u5de8\u9e7f\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1198",
        "area_name": "\u5e73\u4e61\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1199",
        "area_name": "\u5e7f\u5b97\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1200",
        "area_name": "\u65b0\u6cb3\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1201",
        "area_name": "\u67cf\u4e61\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1202",
        "area_name": "\u6865\u4e1c\u533a",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1203",
        "area_name": "\u6865\u897f\u533a",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1204",
        "area_name": "\u6c99\u6cb3\u5e02",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1205",
        "area_name": "\u6e05\u6cb3\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1206",
        "area_name": "\u90a2\u53f0\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1207",
        "area_name": "\u9686\u5c27\u53bf",
        "area_parent_id": "77",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1208",
        "area_name": "\u5317\u5e02\u533a",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1209",
        "area_name": "\u5357\u5e02\u533a",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1210",
        "area_name": "\u535a\u91ce\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1211",
        "area_name": "\u5510\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1212",
        "area_name": "\u5b89\u56fd\u5e02",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1213",
        "area_name": "\u5b89\u65b0\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1214",
        "area_name": "\u5b9a\u5174\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1215",
        "area_name": "\u5b9a\u5dde\u5e02",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1216",
        "area_name": "\u5bb9\u57ce\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1217",
        "area_name": "\u5f90\u6c34\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1218",
        "area_name": "\u65b0\u5e02\u533a",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1219",
        "area_name": "\u6613\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1220",
        "area_name": "\u66f2\u9633\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1221",
        "area_name": "\u671b\u90fd\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1222",
        "area_name": "\u6d9e\u6c34\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1223",
        "area_name": "\u6d9e\u6e90\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1224",
        "area_name": "\u6dbf\u5dde\u5e02",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1225",
        "area_name": "\u6e05\u82d1\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1226",
        "area_name": "\u6ee1\u57ce\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1227",
        "area_name": "\u8821\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1228",
        "area_name": "\u961c\u5e73\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1229",
        "area_name": "\u96c4\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1230",
        "area_name": "\u987a\u5e73\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1231",
        "area_name": "\u9ad8\u7891\u5e97\u5e02",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1232",
        "area_name": "\u9ad8\u9633\u53bf",
        "area_parent_id": "78",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1233",
        "area_name": "\u4e07\u5168\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1234",
        "area_name": "\u4e0b\u82b1\u56ed\u533a",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1235",
        "area_name": "\u5ba3\u5316\u533a",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1236",
        "area_name": "\u5ba3\u5316\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1237",
        "area_name": "\u5c1a\u4e49\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1238",
        "area_name": "\u5d07\u793c\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1239",
        "area_name": "\u5eb7\u4fdd\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1240",
        "area_name": "\u5f20\u5317\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1241",
        "area_name": "\u6000\u5b89\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1242",
        "area_name": "\u6000\u6765\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1243",
        "area_name": "\u6865\u4e1c\u533a",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1244",
        "area_name": "\u6865\u897f\u533a",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1245",
        "area_name": "\u6cbd\u6e90\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1246",
        "area_name": "\u6dbf\u9e7f\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1247",
        "area_name": "\u851a\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1248",
        "area_name": "\u8d64\u57ce\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1249",
        "area_name": "\u9633\u539f\u53bf",
        "area_parent_id": "79",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1250",
        "area_name": "\u4e30\u5b81\u6ee1\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "80",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1251",
        "area_name": "\u5174\u9686\u53bf",
        "area_parent_id": "80",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1252",
        "area_name": "\u53cc\u6865\u533a",
        "area_parent_id": "80",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1253",
        "area_name": "\u53cc\u6ee6\u533a",
        "area_parent_id": "80",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1254",
        "area_name": "\u56f4\u573a\u6ee1\u65cf\u8499\u53e4\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "80",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1255",
        "area_name": "\u5bbd\u57ce\u6ee1\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "80",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1256",
        "area_name": "\u5e73\u6cc9\u53bf",
        "area_parent_id": "80",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1257",
        "area_name": "\u627f\u5fb7\u53bf",
        "area_parent_id": "80",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1258",
        "area_name": "\u6ee6\u5e73\u53bf",
        "area_parent_id": "80",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1259",
        "area_name": "\u9686\u5316\u53bf",
        "area_parent_id": "80",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1260",
        "area_name": "\u9e70\u624b\u8425\u5b50\u77ff\u533a",
        "area_parent_id": "80",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1261",
        "area_name": "\u5180\u5dde\u5e02",
        "area_parent_id": "81",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1262",
        "area_name": "\u5b89\u5e73\u53bf",
        "area_parent_id": "81",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1263",
        "area_name": "\u6545\u57ce\u53bf",
        "area_parent_id": "81",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1264",
        "area_name": "\u666f\u53bf",
        "area_parent_id": "81",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1265",
        "area_name": "\u67a3\u5f3a\u53bf",
        "area_parent_id": "81",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1266",
        "area_name": "\u6843\u57ce\u533a",
        "area_parent_id": "81",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1267",
        "area_name": "\u6b66\u5f3a\u53bf",
        "area_parent_id": "81",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1268",
        "area_name": "\u6b66\u9091\u53bf",
        "area_parent_id": "81",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1269",
        "area_name": "\u6df1\u5dde\u5e02",
        "area_parent_id": "81",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1270",
        "area_name": "\u961c\u57ce\u53bf",
        "area_parent_id": "81",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1271",
        "area_name": "\u9976\u9633\u53bf",
        "area_parent_id": "81",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1272",
        "area_name": "\u4e09\u6cb3\u5e02",
        "area_parent_id": "82",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1273",
        "area_name": "\u56fa\u5b89\u53bf",
        "area_parent_id": "82",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1274",
        "area_name": "\u5927\u5382\u56de\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "82",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1275",
        "area_name": "\u5927\u57ce\u53bf",
        "area_parent_id": "82",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1276",
        "area_name": "\u5b89\u6b21\u533a",
        "area_parent_id": "82",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1277",
        "area_name": "\u5e7f\u9633\u533a",
        "area_parent_id": "82",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1278",
        "area_name": "\u6587\u5b89\u53bf",
        "area_parent_id": "82",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1279",
        "area_name": "\u6c38\u6e05\u53bf",
        "area_parent_id": "82",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1280",
        "area_name": "\u9738\u5dde\u5e02",
        "area_parent_id": "82",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1281",
        "area_name": "\u9999\u6cb3\u53bf",
        "area_parent_id": "82",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1282",
        "area_name": "\u4e1c\u5149\u53bf",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1283",
        "area_name": "\u4efb\u4e18\u5e02",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1284",
        "area_name": "\u5357\u76ae\u53bf",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1285",
        "area_name": "\u5434\u6865\u53bf",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1286",
        "area_name": "\u5b5f\u6751\u56de\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1287",
        "area_name": "\u65b0\u534e\u533a",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1288",
        "area_name": "\u6ca7\u53bf",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1289",
        "area_name": "\u6cb3\u95f4\u5e02",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1290",
        "area_name": "\u6cca\u5934\u5e02",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1291",
        "area_name": "\u6d77\u5174\u53bf",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1292",
        "area_name": "\u732e\u53bf",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1293",
        "area_name": "\u76d0\u5c71\u53bf",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1294",
        "area_name": "\u8083\u5b81\u53bf",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1295",
        "area_name": "\u8fd0\u6cb3\u533a",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1296",
        "area_name": "\u9752\u53bf",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1297",
        "area_name": "\u9ec4\u9a85\u5e02",
        "area_parent_id": "83",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1298",
        "area_name": "\u4e07\u67cf\u6797\u533a",
        "area_parent_id": "84",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1299",
        "area_name": "\u53e4\u4ea4\u5e02",
        "area_parent_id": "84",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1300",
        "area_name": "\u5a04\u70e6\u53bf",
        "area_parent_id": "84",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1301",
        "area_name": "\u5c0f\u5e97\u533a",
        "area_parent_id": "84",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1302",
        "area_name": "\u5c16\u8349\u576a\u533a",
        "area_parent_id": "84",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1303",
        "area_name": "\u664b\u6e90\u533a",
        "area_parent_id": "84",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1304",
        "area_name": "\u674f\u82b1\u5cad\u533a",
        "area_parent_id": "84",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1305",
        "area_name": "\u6e05\u5f90\u53bf",
        "area_parent_id": "84",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1306",
        "area_name": "\u8fce\u6cfd\u533a",
        "area_parent_id": "84",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1307",
        "area_name": "\u9633\u66f2\u53bf",
        "area_parent_id": "84",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1308",
        "area_name": "\u5357\u90ca\u533a",
        "area_parent_id": "85",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1309",
        "area_name": "\u57ce\u533a",
        "area_parent_id": "85",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1310",
        "area_name": "\u5927\u540c\u53bf",
        "area_parent_id": "85",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1311",
        "area_name": "\u5929\u9547\u53bf",
        "area_parent_id": "85",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1312",
        "area_name": "\u5de6\u4e91\u53bf",
        "area_parent_id": "85",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1313",
        "area_name": "\u5e7f\u7075\u53bf",
        "area_parent_id": "85",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1314",
        "area_name": "\u65b0\u8363\u533a",
        "area_parent_id": "85",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1315",
        "area_name": "\u6d51\u6e90\u53bf",
        "area_parent_id": "85",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1316",
        "area_name": "\u7075\u4e18\u53bf",
        "area_parent_id": "85",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1317",
        "area_name": "\u77ff\u533a",
        "area_parent_id": "85",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1318",
        "area_name": "\u9633\u9ad8\u53bf",
        "area_parent_id": "85",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1319",
        "area_name": "\u57ce\u533a",
        "area_parent_id": "86",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1320",
        "area_name": "\u5e73\u5b9a\u53bf",
        "area_parent_id": "86",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1321",
        "area_name": "\u76c2\u53bf",
        "area_parent_id": "86",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1322",
        "area_name": "\u77ff\u533a",
        "area_parent_id": "86",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1323",
        "area_name": "\u90ca\u533a",
        "area_parent_id": "86",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1324",
        "area_name": "\u57ce\u533a",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1325",
        "area_name": "\u58f6\u5173\u53bf",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1326",
        "area_name": "\u5c6f\u7559\u53bf",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1327",
        "area_name": "\u5e73\u987a\u53bf",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1328",
        "area_name": "\u6b66\u4e61\u53bf",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1329",
        "area_name": "\u6c81\u53bf",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1330",
        "area_name": "\u6c81\u6e90\u53bf",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1331",
        "area_name": "\u6f5e\u57ce\u5e02",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1332",
        "area_name": "\u8944\u57a3\u53bf",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1333",
        "area_name": "\u90ca\u533a",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1334",
        "area_name": "\u957f\u5b50\u53bf",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1335",
        "area_name": "\u957f\u6cbb\u53bf",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1336",
        "area_name": "\u9ece\u57ce\u53bf",
        "area_parent_id": "87",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1337",
        "area_name": "\u57ce\u533a",
        "area_parent_id": "88",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1338",
        "area_name": "\u6c81\u6c34\u53bf",
        "area_parent_id": "88",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1339",
        "area_name": "\u6cfd\u5dde\u53bf",
        "area_parent_id": "88",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1340",
        "area_name": "\u9633\u57ce\u53bf",
        "area_parent_id": "88",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1341",
        "area_name": "\u9675\u5ddd\u53bf",
        "area_parent_id": "88",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1342",
        "area_name": "\u9ad8\u5e73\u5e02",
        "area_parent_id": "88",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1343",
        "area_name": "\u53f3\u7389\u53bf",
        "area_parent_id": "89",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1344",
        "area_name": "\u5c71\u9634\u53bf",
        "area_parent_id": "89",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1345",
        "area_name": "\u5e73\u9c81\u533a",
        "area_parent_id": "89",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1346",
        "area_name": "\u5e94\u53bf",
        "area_parent_id": "89",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1347",
        "area_name": "\u6000\u4ec1\u53bf",
        "area_parent_id": "89",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1348",
        "area_name": "\u6714\u57ce\u533a",
        "area_parent_id": "89",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1349",
        "area_name": "\u4ecb\u4f11\u5e02",
        "area_parent_id": "90",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1350",
        "area_name": "\u548c\u987a\u53bf",
        "area_parent_id": "90",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1351",
        "area_name": "\u592a\u8c37\u53bf",
        "area_parent_id": "90",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1352",
        "area_name": "\u5bff\u9633\u53bf",
        "area_parent_id": "90",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1353",
        "area_name": "\u5de6\u6743\u53bf",
        "area_parent_id": "90",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1354",
        "area_name": "\u5e73\u9065\u53bf",
        "area_parent_id": "90",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1355",
        "area_name": "\u6614\u9633\u53bf",
        "area_parent_id": "90",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1356",
        "area_name": "\u6986\u6b21\u533a",
        "area_parent_id": "90",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1357",
        "area_name": "\u6986\u793e\u53bf",
        "area_parent_id": "90",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1358",
        "area_name": "\u7075\u77f3\u53bf",
        "area_parent_id": "90",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1359",
        "area_name": "\u7941\u53bf",
        "area_parent_id": "90",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1360",
        "area_name": "\u4e07\u8363\u53bf",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1361",
        "area_name": "\u4e34\u7317\u53bf",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1362",
        "area_name": "\u57a3\u66f2\u53bf",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1363",
        "area_name": "\u590f\u53bf",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1364",
        "area_name": "\u5e73\u9646\u53bf",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1365",
        "area_name": "\u65b0\u7edb\u53bf",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1366",
        "area_name": "\u6c38\u6d4e\u5e02",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1367",
        "area_name": "\u6cb3\u6d25\u5e02",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1368",
        "area_name": "\u76d0\u6e56\u533a",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1369",
        "area_name": "\u7a37\u5c71\u53bf",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1370",
        "area_name": "\u7edb\u53bf",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1371",
        "area_name": "\u82ae\u57ce\u53bf",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1372",
        "area_name": "\u95fb\u559c\u53bf",
        "area_parent_id": "91",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1373",
        "area_name": "\u4e94\u53f0\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1374",
        "area_name": "\u4e94\u5be8\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1375",
        "area_name": "\u4ee3\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1376",
        "area_name": "\u4fdd\u5fb7\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1377",
        "area_name": "\u504f\u5173\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1378",
        "area_name": "\u539f\u5e73\u5e02",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1379",
        "area_name": "\u5b81\u6b66\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1380",
        "area_name": "\u5b9a\u8944\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1381",
        "area_name": "\u5ca2\u5c9a\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1382",
        "area_name": "\u5ffb\u5e9c\u533a",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1383",
        "area_name": "\u6cb3\u66f2\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1384",
        "area_name": "\u795e\u6c60\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1385",
        "area_name": "\u7e41\u5cd9\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1386",
        "area_name": "\u9759\u4e50\u53bf",
        "area_parent_id": "92",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1387",
        "area_name": "\u4e61\u5b81\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1388",
        "area_name": "\u4faf\u9a6c\u5e02",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1389",
        "area_name": "\u53e4\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1390",
        "area_name": "\u5409\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1391",
        "area_name": "\u5927\u5b81\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1392",
        "area_name": "\u5b89\u6cfd\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1393",
        "area_name": "\u5c27\u90fd\u533a",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1394",
        "area_name": "\u66f2\u6c83\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1395",
        "area_name": "\u6c38\u548c\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1396",
        "area_name": "\u6c7e\u897f\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1397",
        "area_name": "\u6d2a\u6d1e\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1398",
        "area_name": "\u6d6e\u5c71\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1399",
        "area_name": "\u7ffc\u57ce\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1400",
        "area_name": "\u84b2\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1401",
        "area_name": "\u8944\u6c7e\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1402",
        "area_name": "\u96b0\u53bf",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1403",
        "area_name": "\u970d\u5dde\u5e02",
        "area_parent_id": "93",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1404",
        "area_name": "\u4e2d\u9633\u53bf",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1405",
        "area_name": "\u4e34\u53bf",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1406",
        "area_name": "\u4ea4\u53e3\u53bf",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1407",
        "area_name": "\u4ea4\u57ce\u53bf",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1408",
        "area_name": "\u5174\u53bf",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1409",
        "area_name": "\u5b5d\u4e49\u5e02",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1410",
        "area_name": "\u5c9a\u53bf",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1411",
        "area_name": "\u6587\u6c34\u53bf",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1412",
        "area_name": "\u65b9\u5c71\u53bf",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1413",
        "area_name": "\u67f3\u6797\u53bf",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1414",
        "area_name": "\u6c7e\u9633\u5e02",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1415",
        "area_name": "\u77f3\u697c\u53bf",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1416",
        "area_name": "\u79bb\u77f3\u533a",
        "area_parent_id": "94",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1417",
        "area_name": "\u548c\u6797\u683c\u5c14\u53bf",
        "area_parent_id": "95",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1418",
        "area_name": "\u56de\u6c11\u533a",
        "area_parent_id": "95",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1419",
        "area_name": "\u571f\u9ed8\u7279\u5de6\u65d7",
        "area_parent_id": "95",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1420",
        "area_name": "\u6258\u514b\u6258\u53bf",
        "area_parent_id": "95",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1421",
        "area_name": "\u65b0\u57ce\u533a",
        "area_parent_id": "95",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1422",
        "area_name": "\u6b66\u5ddd\u53bf",
        "area_parent_id": "95",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1423",
        "area_name": "\u6e05\u6c34\u6cb3\u53bf",
        "area_parent_id": "95",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1424",
        "area_name": "\u7389\u6cc9\u533a",
        "area_parent_id": "95",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1425",
        "area_name": "\u8d5b\u7f55\u533a",
        "area_parent_id": "95",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1426",
        "area_name": "\u4e1c\u6cb3\u533a",
        "area_parent_id": "96",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1427",
        "area_name": "\u4e5d\u539f\u533a",
        "area_parent_id": "96",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1428",
        "area_name": "\u56fa\u9633\u53bf",
        "area_parent_id": "96",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1429",
        "area_name": "\u571f\u9ed8\u7279\u53f3\u65d7",
        "area_parent_id": "96",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1430",
        "area_name": "\u6606\u90fd\u4ed1\u533a",
        "area_parent_id": "96",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1431",
        "area_name": "\u767d\u4e91\u77ff\u533a",
        "area_parent_id": "96",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1432",
        "area_name": "\u77f3\u62d0\u533a",
        "area_parent_id": "96",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1433",
        "area_name": "\u8fbe\u5c14\u7f55\u8302\u660e\u5b89\u8054\u5408\u65d7",
        "area_parent_id": "96",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1434",
        "area_name": "\u9752\u5c71\u533a",
        "area_parent_id": "96",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1435",
        "area_name": "\u4e4c\u8fbe\u533a",
        "area_parent_id": "97",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1436",
        "area_name": "\u6d77\u52c3\u6e7e\u533a",
        "area_parent_id": "97",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1437",
        "area_name": "\u6d77\u5357\u533a",
        "area_parent_id": "97",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1438",
        "area_name": "\u5143\u5b9d\u5c71\u533a",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1439",
        "area_name": "\u514b\u4ec0\u514b\u817e\u65d7",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1440",
        "area_name": "\u5580\u5587\u6c81\u65d7",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1441",
        "area_name": "\u5b81\u57ce\u53bf",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1442",
        "area_name": "\u5df4\u6797\u53f3\u65d7",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1443",
        "area_name": "\u5df4\u6797\u5de6\u65d7",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1444",
        "area_name": "\u6556\u6c49\u65d7",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1445",
        "area_name": "\u677e\u5c71\u533a",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1446",
        "area_name": "\u6797\u897f\u53bf",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1447",
        "area_name": "\u7ea2\u5c71\u533a",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1448",
        "area_name": "\u7fc1\u725b\u7279\u65d7",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1449",
        "area_name": "\u963f\u9c81\u79d1\u5c14\u6c81\u65d7",
        "area_parent_id": "98",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1450",
        "area_name": "\u5948\u66fc\u65d7",
        "area_parent_id": "99",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1451",
        "area_name": "\u5e93\u4f26\u65d7",
        "area_parent_id": "99",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1452",
        "area_name": "\u5f00\u9c81\u53bf",
        "area_parent_id": "99",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1453",
        "area_name": "\u624e\u9c81\u7279\u65d7",
        "area_parent_id": "99",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1454",
        "area_name": "\u79d1\u5c14\u6c81\u533a",
        "area_parent_id": "99",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1455",
        "area_name": "\u79d1\u5c14\u6c81\u5de6\u7ffc\u4e2d\u65d7",
        "area_parent_id": "99",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1456",
        "area_name": "\u79d1\u5c14\u6c81\u5de6\u7ffc\u540e\u65d7",
        "area_parent_id": "99",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1457",
        "area_name": "\u970d\u6797\u90ed\u52d2\u5e02",
        "area_parent_id": "99",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1458",
        "area_name": "\u4e1c\u80dc\u533a",
        "area_parent_id": "100",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1459",
        "area_name": "\u4e4c\u5ba1\u65d7",
        "area_parent_id": "100",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1460",
        "area_name": "\u4f0a\u91d1\u970d\u6d1b\u65d7",
        "area_parent_id": "100",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1461",
        "area_name": "\u51c6\u683c\u5c14\u65d7",
        "area_parent_id": "100",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1462",
        "area_name": "\u676d\u9526\u65d7",
        "area_parent_id": "100",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1463",
        "area_name": "\u8fbe\u62c9\u7279\u65d7",
        "area_parent_id": "100",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1464",
        "area_name": "\u9102\u4e1c\u80dc\u533a",
        "area_parent_id": "100",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1465",
        "area_name": "\u9102\u6258\u514b\u524d\u65d7",
        "area_parent_id": "100",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1466",
        "area_name": "\u9102\u6258\u514b\u65d7",
        "area_parent_id": "100",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1467",
        "area_name": "\u624e\u5170\u5c6f\u5e02",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1468",
        "area_name": "\u65b0\u5df4\u5c14\u864e\u53f3\u65d7",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1469",
        "area_name": "\u65b0\u5df4\u5c14\u864e\u5de6\u65d7",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1470",
        "area_name": "\u6839\u6cb3\u5e02",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1471",
        "area_name": "\u6d77\u62c9\u5c14\u533a",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1472",
        "area_name": "\u6ee1\u6d32\u91cc\u5e02",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1473",
        "area_name": "\u7259\u514b\u77f3\u5e02",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1474",
        "area_name": "\u83ab\u529b\u8fbe\u74e6\u8fbe\u65a1\u5c14\u65cf\u81ea\u6cbb\u65d7",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1475",
        "area_name": "\u9102\u4f26\u6625\u81ea\u6cbb\u65d7",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1476",
        "area_name": "\u9102\u6e29\u514b\u65cf\u81ea\u6cbb\u65d7",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1477",
        "area_name": "\u963f\u8363\u65d7",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1478",
        "area_name": "\u9648\u5df4\u5c14\u864e\u65d7",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1479",
        "area_name": "\u989d\u5c14\u53e4\u7eb3\u5e02",
        "area_parent_id": "101",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1480",
        "area_name": "\u4e34\u6cb3\u533a",
        "area_parent_id": "102",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1481",
        "area_name": "\u4e4c\u62c9\u7279\u4e2d\u65d7",
        "area_parent_id": "102",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1482",
        "area_name": "\u4e4c\u62c9\u7279\u524d\u65d7",
        "area_parent_id": "102",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1483",
        "area_name": "\u4e4c\u62c9\u7279\u540e\u65d7",
        "area_parent_id": "102",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1484",
        "area_name": "\u4e94\u539f\u53bf",
        "area_parent_id": "102",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1485",
        "area_name": "\u676d\u9526\u540e\u65d7",
        "area_parent_id": "102",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1486",
        "area_name": "\u78f4\u53e3\u53bf",
        "area_parent_id": "102",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1487",
        "area_name": "\u4e30\u9547\u5e02",
        "area_parent_id": "103",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1488",
        "area_name": "\u5174\u548c\u53bf",
        "area_parent_id": "103",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1489",
        "area_name": "\u51c9\u57ce\u53bf",
        "area_parent_id": "103",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1490",
        "area_name": "\u5316\u5fb7\u53bf",
        "area_parent_id": "103",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1491",
        "area_name": "\u5353\u8d44\u53bf",
        "area_parent_id": "103",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1492",
        "area_name": "\u5546\u90fd\u53bf",
        "area_parent_id": "103",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1493",
        "area_name": "\u56db\u5b50\u738b\u65d7",
        "area_parent_id": "103",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1494",
        "area_name": "\u5bdf\u54c8\u5c14\u53f3\u7ffc\u4e2d\u65d7",
        "area_parent_id": "103",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1495",
        "area_name": "\u5bdf\u54c8\u5c14\u53f3\u7ffc\u524d\u65d7",
        "area_parent_id": "103",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1496",
        "area_name": "\u5bdf\u54c8\u5c14\u53f3\u7ffc\u540e\u65d7",
        "area_parent_id": "103",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1497",
        "area_name": "\u96c6\u5b81\u533a",
        "area_parent_id": "103",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1498",
        "area_name": "\u4e4c\u5170\u6d69\u7279\u5e02",
        "area_parent_id": "104",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1499",
        "area_name": "\u624e\u8d49\u7279\u65d7",
        "area_parent_id": "104",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1500",
        "area_name": "\u79d1\u5c14\u6c81\u53f3\u7ffc\u4e2d\u65d7",
        "area_parent_id": "104",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1501",
        "area_name": "\u79d1\u5c14\u6c81\u53f3\u7ffc\u524d\u65d7",
        "area_parent_id": "104",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1502",
        "area_name": "\u7a81\u6cc9\u53bf",
        "area_parent_id": "104",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1503",
        "area_name": "\u963f\u5c14\u5c71\u5e02",
        "area_parent_id": "104",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1504",
        "area_name": "\u4e1c\u4e4c\u73e0\u7a46\u6c81\u65d7",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1505",
        "area_name": "\u4e8c\u8fde\u6d69\u7279\u5e02",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1506",
        "area_name": "\u591a\u4f26\u53bf",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1507",
        "area_name": "\u592a\u4ec6\u5bfa\u65d7",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1508",
        "area_name": "\u6b63\u84dd\u65d7",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1509",
        "area_name": "\u6b63\u9576\u767d\u65d7",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1510",
        "area_name": "\u82cf\u5c3c\u7279\u53f3\u65d7",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1511",
        "area_name": "\u82cf\u5c3c\u7279\u5de6\u65d7",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1512",
        "area_name": "\u897f\u4e4c\u73e0\u7a46\u6c81\u65d7",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1513",
        "area_name": "\u9521\u6797\u6d69\u7279\u5e02",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1514",
        "area_name": "\u9576\u9ec4\u65d7",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1515",
        "area_name": "\u963f\u5df4\u560e\u65d7",
        "area_parent_id": "105",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1516",
        "area_name": "\u963f\u62c9\u5584\u53f3\u65d7",
        "area_parent_id": "106",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1517",
        "area_name": "\u963f\u62c9\u5584\u5de6\u65d7",
        "area_parent_id": "106",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1518",
        "area_name": "\u989d\u6d4e\u7eb3\u65d7",
        "area_parent_id": "106",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1519",
        "area_name": "\u4e1c\u9675\u533a",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1520",
        "area_name": "\u4e8e\u6d2a\u533a",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1521",
        "area_name": "\u548c\u5e73\u533a",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1522",
        "area_name": "\u5927\u4e1c\u533a",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1523",
        "area_name": "\u5eb7\u5e73\u53bf",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1524",
        "area_name": "\u65b0\u6c11\u5e02",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1525",
        "area_name": "\u6c88\u5317\u65b0\u533a",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1526",
        "area_name": "\u6c88\u6cb3\u533a",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1527",
        "area_name": "\u6cd5\u5e93\u53bf",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1528",
        "area_name": "\u7687\u59d1\u533a",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1529",
        "area_name": "\u82cf\u5bb6\u5c6f\u533a",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1530",
        "area_name": "\u8fbd\u4e2d\u53bf",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1531",
        "area_name": "\u94c1\u897f\u533a",
        "area_parent_id": "107",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1532",
        "area_name": "\u4e2d\u5c71\u533a",
        "area_parent_id": "108",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1533",
        "area_name": "\u5e84\u6cb3\u5e02",
        "area_parent_id": "108",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1534",
        "area_name": "\u65c5\u987a\u53e3\u533a",
        "area_parent_id": "108",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1535",
        "area_name": "\u666e\u5170\u5e97\u5e02",
        "area_parent_id": "108",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1536",
        "area_name": "\u6c99\u6cb3\u53e3\u533a",
        "area_parent_id": "108",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1537",
        "area_name": "\u74e6\u623f\u5e97\u5e02",
        "area_parent_id": "108",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1538",
        "area_name": "\u7518\u4e95\u5b50\u533a",
        "area_parent_id": "108",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1539",
        "area_name": "\u897f\u5c97\u533a",
        "area_parent_id": "108",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1540",
        "area_name": "\u91d1\u5dde\u533a",
        "area_parent_id": "108",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1541",
        "area_name": "\u957f\u6d77\u53bf",
        "area_parent_id": "108",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1542",
        "area_name": "\u5343\u5c71\u533a",
        "area_parent_id": "109",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1543",
        "area_name": "\u53f0\u5b89\u53bf",
        "area_parent_id": "109",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1544",
        "area_name": "\u5cab\u5ca9\u6ee1\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "109",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1545",
        "area_name": "\u6d77\u57ce\u5e02",
        "area_parent_id": "109",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1546",
        "area_name": "\u7acb\u5c71\u533a",
        "area_parent_id": "109",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1547",
        "area_name": "\u94c1\u4e1c\u533a",
        "area_parent_id": "109",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1548",
        "area_name": "\u94c1\u897f\u533a",
        "area_parent_id": "109",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1549",
        "area_name": "\u4e1c\u6d32\u533a",
        "area_parent_id": "110",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1550",
        "area_name": "\u629a\u987a\u53bf",
        "area_parent_id": "110",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1551",
        "area_name": "\u65b0\u5bbe\u6ee1\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "110",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1552",
        "area_name": "\u65b0\u629a\u533a",
        "area_parent_id": "110",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1553",
        "area_name": "\u671b\u82b1\u533a",
        "area_parent_id": "110",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1554",
        "area_name": "\u6e05\u539f\u6ee1\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "110",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1555",
        "area_name": "\u987a\u57ce\u533a",
        "area_parent_id": "110",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1556",
        "area_name": "\u5357\u82ac\u533a",
        "area_parent_id": "111",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1557",
        "area_name": "\u5e73\u5c71\u533a",
        "area_parent_id": "111",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1558",
        "area_name": "\u660e\u5c71\u533a",
        "area_parent_id": "111",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1559",
        "area_name": "\u672c\u6eaa\u6ee1\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "111",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1560",
        "area_name": "\u6853\u4ec1\u6ee1\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "111",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1561",
        "area_name": "\u6eaa\u6e56\u533a",
        "area_parent_id": "111",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1562",
        "area_name": "\u4e1c\u6e2f\u5e02",
        "area_parent_id": "112",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1563",
        "area_name": "\u5143\u5b9d\u533a",
        "area_parent_id": "112",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1564",
        "area_name": "\u51e4\u57ce\u5e02",
        "area_parent_id": "112",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1565",
        "area_name": "\u5bbd\u7538\u6ee1\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "112",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1566",
        "area_name": "\u632f\u5174\u533a",
        "area_parent_id": "112",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1567",
        "area_name": "\u632f\u5b89\u533a",
        "area_parent_id": "112",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1568",
        "area_name": "\u4e49\u53bf",
        "area_parent_id": "113",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1569",
        "area_name": "\u51cc\u6cb3\u533a",
        "area_parent_id": "113",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1570",
        "area_name": "\u51cc\u6d77\u5e02",
        "area_parent_id": "113",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1571",
        "area_name": "\u5317\u9547\u5e02",
        "area_parent_id": "113",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1572",
        "area_name": "\u53e4\u5854\u533a",
        "area_parent_id": "113",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1573",
        "area_name": "\u592a\u548c\u533a",
        "area_parent_id": "113",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1574",
        "area_name": "\u9ed1\u5c71\u53bf",
        "area_parent_id": "113",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1575",
        "area_name": "\u5927\u77f3\u6865\u5e02",
        "area_parent_id": "114",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1576",
        "area_name": "\u76d6\u5dde\u5e02",
        "area_parent_id": "114",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1577",
        "area_name": "\u7ad9\u524d\u533a",
        "area_parent_id": "114",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1578",
        "area_name": "\u8001\u8fb9\u533a",
        "area_parent_id": "114",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1579",
        "area_name": "\u897f\u5e02\u533a",
        "area_parent_id": "114",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1580",
        "area_name": "\u9c85\u9c7c\u5708\u533a",
        "area_parent_id": "114",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1581",
        "area_name": "\u592a\u5e73\u533a",
        "area_parent_id": "115",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1582",
        "area_name": "\u5f70\u6b66\u53bf",
        "area_parent_id": "115",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1583",
        "area_name": "\u65b0\u90b1\u533a",
        "area_parent_id": "115",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1584",
        "area_name": "\u6d77\u5dde\u533a",
        "area_parent_id": "115",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1585",
        "area_name": "\u6e05\u6cb3\u95e8\u533a",
        "area_parent_id": "115",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1586",
        "area_name": "\u7ec6\u6cb3\u533a",
        "area_parent_id": "115",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1587",
        "area_name": "\u8499\u53e4\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "115",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1588",
        "area_name": "\u592a\u5b50\u6cb3\u533a",
        "area_parent_id": "116",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1589",
        "area_name": "\u5b8f\u4f1f\u533a",
        "area_parent_id": "116",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1590",
        "area_name": "\u5f13\u957f\u5cad\u533a",
        "area_parent_id": "116",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1591",
        "area_name": "\u6587\u5723\u533a",
        "area_parent_id": "116",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1592",
        "area_name": "\u706f\u5854\u5e02",
        "area_parent_id": "116",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1593",
        "area_name": "\u767d\u5854\u533a",
        "area_parent_id": "116",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1594",
        "area_name": "\u8fbd\u9633\u53bf",
        "area_parent_id": "116",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1595",
        "area_name": "\u5174\u9686\u53f0\u533a",
        "area_parent_id": "117",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1596",
        "area_name": "\u53cc\u53f0\u5b50\u533a",
        "area_parent_id": "117",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1597",
        "area_name": "\u5927\u6d3c\u53bf",
        "area_parent_id": "117",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1598",
        "area_name": "\u76d8\u5c71\u53bf",
        "area_parent_id": "117",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1599",
        "area_name": "\u5f00\u539f\u5e02",
        "area_parent_id": "118",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1600",
        "area_name": "\u660c\u56fe\u53bf",
        "area_parent_id": "118",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1601",
        "area_name": "\u6e05\u6cb3\u533a",
        "area_parent_id": "118",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1602",
        "area_name": "\u897f\u4e30\u53bf",
        "area_parent_id": "118",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1603",
        "area_name": "\u8c03\u5175\u5c71\u5e02",
        "area_parent_id": "118",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1604",
        "area_name": "\u94c1\u5cad\u53bf",
        "area_parent_id": "118",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1605",
        "area_name": "\u94f6\u5dde\u533a",
        "area_parent_id": "118",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1606",
        "area_name": "\u51cc\u6e90\u5e02",
        "area_parent_id": "119",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1607",
        "area_name": "\u5317\u7968\u5e02",
        "area_parent_id": "119",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1608",
        "area_name": "\u53cc\u5854\u533a",
        "area_parent_id": "119",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1609",
        "area_name": "\u5580\u5587\u6c81\u5de6\u7ffc\u8499\u53e4\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "119",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1610",
        "area_name": "\u5efa\u5e73\u53bf",
        "area_parent_id": "119",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1611",
        "area_name": "\u671d\u9633\u53bf",
        "area_parent_id": "119",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1612",
        "area_name": "\u9f99\u57ce\u533a",
        "area_parent_id": "119",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1613",
        "area_name": "\u5174\u57ce\u5e02",
        "area_parent_id": "120",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1614",
        "area_name": "\u5357\u7968\u533a",
        "area_parent_id": "120",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1615",
        "area_name": "\u5efa\u660c\u53bf",
        "area_parent_id": "120",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1616",
        "area_name": "\u7ee5\u4e2d\u53bf",
        "area_parent_id": "120",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1617",
        "area_name": "\u8fde\u5c71\u533a",
        "area_parent_id": "120",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1618",
        "area_name": "\u9f99\u6e2f\u533a",
        "area_parent_id": "120",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1619",
        "area_name": "\u4e5d\u53f0\u5e02",
        "area_parent_id": "121",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1620",
        "area_name": "\u4e8c\u9053\u533a",
        "area_parent_id": "121",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1621",
        "area_name": "\u519c\u5b89\u53bf",
        "area_parent_id": "121",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1622",
        "area_name": "\u5357\u5173\u533a",
        "area_parent_id": "121",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1623",
        "area_name": "\u53cc\u9633\u533a",
        "area_parent_id": "121",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1624",
        "area_name": "\u5bbd\u57ce\u533a",
        "area_parent_id": "121",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1625",
        "area_name": "\u5fb7\u60e0\u5e02",
        "area_parent_id": "121",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1626",
        "area_name": "\u671d\u9633\u533a",
        "area_parent_id": "121",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1627",
        "area_name": "\u6986\u6811\u5e02",
        "area_parent_id": "121",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1628",
        "area_name": "\u7eff\u56ed\u533a",
        "area_parent_id": "121",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1629",
        "area_name": "\u4e30\u6ee1\u533a",
        "area_parent_id": "122",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1630",
        "area_name": "\u660c\u9091\u533a",
        "area_parent_id": "122",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1631",
        "area_name": "\u6866\u7538\u5e02",
        "area_parent_id": "122",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1632",
        "area_name": "\u6c38\u5409\u53bf",
        "area_parent_id": "122",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1633",
        "area_name": "\u78d0\u77f3\u5e02",
        "area_parent_id": "122",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1634",
        "area_name": "\u8212\u5170\u5e02",
        "area_parent_id": "122",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1635",
        "area_name": "\u8239\u8425\u533a",
        "area_parent_id": "122",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1636",
        "area_name": "\u86df\u6cb3\u5e02",
        "area_parent_id": "122",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1637",
        "area_name": "\u9f99\u6f6d\u533a",
        "area_parent_id": "122",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1638",
        "area_name": "\u4f0a\u901a\u6ee1\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "123",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1639",
        "area_name": "\u516c\u4e3b\u5cad\u5e02",
        "area_parent_id": "123",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1640",
        "area_name": "\u53cc\u8fbd\u5e02",
        "area_parent_id": "123",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1641",
        "area_name": "\u68a8\u6811\u53bf",
        "area_parent_id": "123",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1642",
        "area_name": "\u94c1\u4e1c\u533a",
        "area_parent_id": "123",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1643",
        "area_name": "\u94c1\u897f\u533a",
        "area_parent_id": "123",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1644",
        "area_name": "\u4e1c\u4e30\u53bf",
        "area_parent_id": "124",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1645",
        "area_name": "\u4e1c\u8fbd\u53bf",
        "area_parent_id": "124",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1646",
        "area_name": "\u897f\u5b89\u533a",
        "area_parent_id": "124",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1647",
        "area_name": "\u9f99\u5c71\u533a",
        "area_parent_id": "124",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1648",
        "area_name": "\u4e1c\u660c\u533a",
        "area_parent_id": "125",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1649",
        "area_name": "\u4e8c\u9053\u6c5f\u533a",
        "area_parent_id": "125",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1650",
        "area_name": "\u67f3\u6cb3\u53bf",
        "area_parent_id": "125",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1651",
        "area_name": "\u6885\u6cb3\u53e3\u5e02",
        "area_parent_id": "125",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1652",
        "area_name": "\u8f89\u5357\u53bf",
        "area_parent_id": "125",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1653",
        "area_name": "\u901a\u5316\u53bf",
        "area_parent_id": "125",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1654",
        "area_name": "\u96c6\u5b89\u5e02",
        "area_parent_id": "125",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1655",
        "area_name": "\u4e34\u6c5f\u5e02",
        "area_parent_id": "126",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1656",
        "area_name": "\u516b\u9053\u6c5f\u533a",
        "area_parent_id": "126",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1657",
        "area_name": "\u629a\u677e\u53bf",
        "area_parent_id": "126",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1658",
        "area_name": "\u6c5f\u6e90\u533a",
        "area_parent_id": "126",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1659",
        "area_name": "\u957f\u767d\u671d\u9c9c\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "126",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1660",
        "area_name": "\u9756\u5b87\u53bf",
        "area_parent_id": "126",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1661",
        "area_name": "\u5e72\u5b89\u53bf",
        "area_parent_id": "127",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1662",
        "area_name": "\u524d\u90ed\u5c14\u7f57\u65af\u8499\u53e4\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "127",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1663",
        "area_name": "\u5b81\u6c5f\u533a",
        "area_parent_id": "127",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1664",
        "area_name": "\u6276\u4f59\u53bf",
        "area_parent_id": "127",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1665",
        "area_name": "\u957f\u5cad\u53bf",
        "area_parent_id": "127",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1666",
        "area_name": "\u5927\u5b89\u5e02",
        "area_parent_id": "128",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1667",
        "area_name": "\u6d2e\u5317\u533a",
        "area_parent_id": "128",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1668",
        "area_name": "\u6d2e\u5357\u5e02",
        "area_parent_id": "128",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1669",
        "area_name": "\u901a\u6986\u53bf",
        "area_parent_id": "128",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1670",
        "area_name": "\u9547\u8d49\u53bf",
        "area_parent_id": "128",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1671",
        "area_name": "\u548c\u9f99\u5e02",
        "area_parent_id": "129",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1672",
        "area_name": "\u56fe\u4eec\u5e02",
        "area_parent_id": "129",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1673",
        "area_name": "\u5b89\u56fe\u53bf",
        "area_parent_id": "129",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1674",
        "area_name": "\u5ef6\u5409\u5e02",
        "area_parent_id": "129",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1675",
        "area_name": "\u6566\u5316\u5e02",
        "area_parent_id": "129",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1676",
        "area_name": "\u6c6a\u6e05\u53bf",
        "area_parent_id": "129",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1677",
        "area_name": "\u73f2\u6625\u5e02",
        "area_parent_id": "129",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1678",
        "area_name": "\u9f99\u4e95\u5e02",
        "area_parent_id": "129",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1679",
        "area_name": "\u4e94\u5e38\u5e02",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1680",
        "area_name": "\u4f9d\u5170\u53bf",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1681",
        "area_name": "\u5357\u5c97\u533a",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1682",
        "area_name": "\u53cc\u57ce\u5e02",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1683",
        "area_name": "\u547c\u5170\u533a",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1684",
        "area_name": "\u54c8\u5c14\u6ee8\u5e02\u9053\u91cc\u533a",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1685",
        "area_name": "\u5bbe\u53bf",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1686",
        "area_name": "\u5c1a\u5fd7\u5e02",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1687",
        "area_name": "\u5df4\u5f66\u53bf",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1688",
        "area_name": "\u5e73\u623f\u533a",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1689",
        "area_name": "\u5ef6\u5bff\u53bf",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1690",
        "area_name": "\u65b9\u6b63\u53bf",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1691",
        "area_name": "\u6728\u5170\u53bf",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1692",
        "area_name": "\u677e\u5317\u533a",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1693",
        "area_name": "\u901a\u6cb3\u53bf",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1694",
        "area_name": "\u9053\u5916\u533a",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1695",
        "area_name": "\u963f\u57ce\u533a",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1696",
        "area_name": "\u9999\u574a\u533a",
        "area_parent_id": "130",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1697",
        "area_name": "\u4f9d\u5b89\u53bf",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1698",
        "area_name": "\u514b\u4e1c\u53bf",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1699",
        "area_name": "\u514b\u5c71\u53bf",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1700",
        "area_name": "\u5bcc\u62c9\u5c14\u57fa\u533a",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1701",
        "area_name": "\u5bcc\u88d5\u53bf",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1702",
        "area_name": "\u5efa\u534e\u533a",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1703",
        "area_name": "\u62dc\u6cc9\u53bf",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1704",
        "area_name": "\u6602\u6602\u6eaa\u533a",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1705",
        "area_name": "\u6885\u91cc\u65af\u8fbe\u65a1\u5c14\u65cf\u533a",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1706",
        "area_name": "\u6cf0\u6765\u53bf",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1707",
        "area_name": "\u7518\u5357\u53bf",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1708",
        "area_name": "\u78be\u5b50\u5c71\u533a",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1709",
        "area_name": "\u8bb7\u6cb3\u5e02",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1710",
        "area_name": "\u94c1\u950b\u533a",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1711",
        "area_name": "\u9f99\u6c5f\u53bf",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1712",
        "area_name": "\u9f99\u6c99\u533a",
        "area_parent_id": "131",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1713",
        "area_name": "\u57ce\u5b50\u6cb3\u533a",
        "area_parent_id": "132",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1714",
        "area_name": "\u5bc6\u5c71\u5e02",
        "area_parent_id": "132",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1715",
        "area_name": "\u6052\u5c71\u533a",
        "area_parent_id": "132",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1716",
        "area_name": "\u68a8\u6811\u533a",
        "area_parent_id": "132",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1717",
        "area_name": "\u6ef4\u9053\u533a",
        "area_parent_id": "132",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1718",
        "area_name": "\u864e\u6797\u5e02",
        "area_parent_id": "132",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1719",
        "area_name": "\u9e21\u4e1c\u53bf",
        "area_parent_id": "132",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1720",
        "area_name": "\u9e21\u51a0\u533a",
        "area_parent_id": "132",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1721",
        "area_name": "\u9ebb\u5c71\u533a",
        "area_parent_id": "132",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1722",
        "area_name": "\u4e1c\u5c71\u533a",
        "area_parent_id": "133",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1723",
        "area_name": "\u5174\u5b89\u533a",
        "area_parent_id": "133",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1724",
        "area_name": "\u5174\u5c71\u533a",
        "area_parent_id": "133",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1725",
        "area_name": "\u5357\u5c71\u533a",
        "area_parent_id": "133",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1726",
        "area_name": "\u5411\u9633\u533a",
        "area_parent_id": "133",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1727",
        "area_name": "\u5de5\u519c\u533a",
        "area_parent_id": "133",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1728",
        "area_name": "\u7ee5\u6ee8\u53bf",
        "area_parent_id": "133",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1729",
        "area_name": "\u841d\u5317\u53bf",
        "area_parent_id": "133",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1730",
        "area_name": "\u53cb\u8c0a\u53bf",
        "area_parent_id": "134",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1731",
        "area_name": "\u56db\u65b9\u53f0\u533a",
        "area_parent_id": "134",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1732",
        "area_name": "\u5b9d\u5c71\u533a",
        "area_parent_id": "134",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1733",
        "area_name": "\u5b9d\u6e05\u53bf",
        "area_parent_id": "134",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1734",
        "area_name": "\u5c16\u5c71\u533a",
        "area_parent_id": "134",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1735",
        "area_name": "\u5cad\u4e1c\u533a",
        "area_parent_id": "134",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1736",
        "area_name": "\u96c6\u8d24\u53bf",
        "area_parent_id": "134",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1737",
        "area_name": "\u9976\u6cb3\u53bf",
        "area_parent_id": "134",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1738",
        "area_name": "\u5927\u540c\u533a",
        "area_parent_id": "135",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1739",
        "area_name": "\u675c\u5c14\u4f2f\u7279\u8499\u53e4\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "135",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1740",
        "area_name": "\u6797\u7538\u53bf",
        "area_parent_id": "135",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1741",
        "area_name": "\u7ea2\u5c97\u533a",
        "area_parent_id": "135",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1742",
        "area_name": "\u8087\u5dde\u53bf",
        "area_parent_id": "135",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1743",
        "area_name": "\u8087\u6e90\u53bf",
        "area_parent_id": "135",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1744",
        "area_name": "\u80e1\u8def\u533a",
        "area_parent_id": "135",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1745",
        "area_name": "\u8428\u5c14\u56fe\u533a",
        "area_parent_id": "135",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1746",
        "area_name": "\u9f99\u51e4\u533a",
        "area_parent_id": "135",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1747",
        "area_name": "\u4e0a\u7518\u5cad\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1748",
        "area_name": "\u4e4c\u4f0a\u5cad\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1749",
        "area_name": "\u4e4c\u9a6c\u6cb3\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1750",
        "area_name": "\u4e94\u8425\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1751",
        "area_name": "\u4f0a\u6625\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1752",
        "area_name": "\u5357\u5c94\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1753",
        "area_name": "\u53cb\u597d\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1754",
        "area_name": "\u5609\u836b\u53bf",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1755",
        "area_name": "\u5e26\u5cad\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1756",
        "area_name": "\u65b0\u9752\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1757",
        "area_name": "\u6c64\u65fa\u6cb3\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1758",
        "area_name": "\u7ea2\u661f\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1759",
        "area_name": "\u7f8e\u6eaa\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1760",
        "area_name": "\u7fe0\u5ce6\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1761",
        "area_name": "\u897f\u6797\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1762",
        "area_name": "\u91d1\u5c71\u5c6f\u533a",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1763",
        "area_name": "\u94c1\u529b\u5e02",
        "area_parent_id": "136",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1764",
        "area_name": "\u4e1c\u98ce\u533a",
        "area_parent_id": "137",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1765",
        "area_name": "\u524d\u8fdb\u533a",
        "area_parent_id": "137",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1766",
        "area_name": "\u540c\u6c5f\u5e02",
        "area_parent_id": "137",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1767",
        "area_name": "\u5411\u9633\u533a",
        "area_parent_id": "137",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1768",
        "area_name": "\u5bcc\u9526\u5e02",
        "area_parent_id": "137",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1769",
        "area_name": "\u629a\u8fdc\u53bf",
        "area_parent_id": "137",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1770",
        "area_name": "\u6866\u5357\u53bf",
        "area_parent_id": "137",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1771",
        "area_name": "\u6866\u5ddd\u53bf",
        "area_parent_id": "137",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1772",
        "area_name": "\u6c64\u539f\u53bf",
        "area_parent_id": "137",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1773",
        "area_name": "\u90ca\u533a",
        "area_parent_id": "137",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1774",
        "area_name": "\u52c3\u5229\u53bf",
        "area_parent_id": "138",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1775",
        "area_name": "\u65b0\u5174\u533a",
        "area_parent_id": "138",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1776",
        "area_name": "\u6843\u5c71\u533a",
        "area_parent_id": "138",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1777",
        "area_name": "\u8304\u5b50\u6cb3\u533a",
        "area_parent_id": "138",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1778",
        "area_name": "\u4e1c\u5b81\u53bf",
        "area_parent_id": "139",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1779",
        "area_name": "\u4e1c\u5b89\u533a",
        "area_parent_id": "139",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1780",
        "area_name": "\u5b81\u5b89\u5e02",
        "area_parent_id": "139",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1781",
        "area_name": "\u6797\u53e3\u53bf",
        "area_parent_id": "139",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1782",
        "area_name": "\u6d77\u6797\u5e02",
        "area_parent_id": "139",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1783",
        "area_name": "\u7231\u6c11\u533a",
        "area_parent_id": "139",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1784",
        "area_name": "\u7a46\u68f1\u5e02",
        "area_parent_id": "139",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1785",
        "area_name": "\u7ee5\u82ac\u6cb3\u5e02",
        "area_parent_id": "139",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1786",
        "area_name": "\u897f\u5b89\u533a",
        "area_parent_id": "139",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1787",
        "area_name": "\u9633\u660e\u533a",
        "area_parent_id": "139",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1788",
        "area_name": "\u4e94\u5927\u8fde\u6c60\u5e02",
        "area_parent_id": "140",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1789",
        "area_name": "\u5317\u5b89\u5e02",
        "area_parent_id": "140",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1790",
        "area_name": "\u5ae9\u6c5f\u53bf",
        "area_parent_id": "140",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1791",
        "area_name": "\u5b59\u5434\u53bf",
        "area_parent_id": "140",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1792",
        "area_name": "\u7231\u8f89\u533a",
        "area_parent_id": "140",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1793",
        "area_name": "\u8f66\u900a\u514b\u53bf",
        "area_parent_id": "140",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1794",
        "area_name": "\u900a\u514b\u53bf",
        "area_parent_id": "140",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1795",
        "area_name": "\u5170\u897f\u53bf",
        "area_parent_id": "141",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1796",
        "area_name": "\u5b89\u8fbe\u5e02",
        "area_parent_id": "141",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1797",
        "area_name": "\u5e86\u5b89\u53bf",
        "area_parent_id": "141",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1798",
        "area_name": "\u660e\u6c34\u53bf",
        "area_parent_id": "141",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1799",
        "area_name": "\u671b\u594e\u53bf",
        "area_parent_id": "141",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1800",
        "area_name": "\u6d77\u4f26\u5e02",
        "area_parent_id": "141",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1801",
        "area_name": "\u7ee5\u5316\u5e02\u5317\u6797\u533a",
        "area_parent_id": "141",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1802",
        "area_name": "\u7ee5\u68f1\u53bf",
        "area_parent_id": "141",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1803",
        "area_name": "\u8087\u4e1c\u5e02",
        "area_parent_id": "141",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1804",
        "area_name": "\u9752\u5188\u53bf",
        "area_parent_id": "141",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1805",
        "area_name": "\u547c\u739b\u53bf",
        "area_parent_id": "142",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1806",
        "area_name": "\u5854\u6cb3\u53bf",
        "area_parent_id": "142",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1807",
        "area_name": "\u5927\u5174\u5b89\u5cad\u5730\u533a\u52a0\u683c\u8fbe\u5947\u533a",
        "area_parent_id": "142",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1808",
        "area_name": "\u5927\u5174\u5b89\u5cad\u5730\u533a\u547c\u4e2d\u533a",
        "area_parent_id": "142",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1809",
        "area_name": "\u5927\u5174\u5b89\u5cad\u5730\u533a\u65b0\u6797\u533a",
        "area_parent_id": "142",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1810",
        "area_name": "\u5927\u5174\u5b89\u5cad\u5730\u533a\u677e\u5cad\u533a",
        "area_parent_id": "142",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "1811",
        "area_name": "\u6f20\u6cb3\u53bf",
        "area_parent_id": "142",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2027",
        "area_name": "\u4e0b\u5173\u533a",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2028",
        "area_name": "\u516d\u5408\u533a",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2029",
        "area_name": "\u5efa\u90ba\u533a",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2030",
        "area_name": "\u6816\u971e\u533a",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2031",
        "area_name": "\u6c5f\u5b81\u533a",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2032",
        "area_name": "\u6d66\u53e3\u533a",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2033",
        "area_name": "\u6ea7\u6c34\u53bf",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2034",
        "area_name": "\u7384\u6b66\u533a",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2035",
        "area_name": "\u767d\u4e0b\u533a",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2036",
        "area_name": "\u79e6\u6dee\u533a",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2037",
        "area_name": "\u96e8\u82b1\u53f0\u533a",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2038",
        "area_name": "\u9ad8\u6df3\u53bf",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2039",
        "area_name": "\u9f13\u697c\u533a",
        "area_parent_id": "162",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2040",
        "area_name": "\u5317\u5858\u533a",
        "area_parent_id": "163",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2041",
        "area_name": "\u5357\u957f\u533a",
        "area_parent_id": "163",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2042",
        "area_name": "\u5b9c\u5174\u5e02",
        "area_parent_id": "163",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2043",
        "area_name": "\u5d07\u5b89\u533a",
        "area_parent_id": "163",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2044",
        "area_name": "\u60e0\u5c71\u533a",
        "area_parent_id": "163",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2045",
        "area_name": "\u6c5f\u9634\u5e02",
        "area_parent_id": "163",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2046",
        "area_name": "\u6ee8\u6e56\u533a",
        "area_parent_id": "163",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2047",
        "area_name": "\u9521\u5c71\u533a",
        "area_parent_id": "163",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2048",
        "area_name": "\u4e30\u53bf",
        "area_parent_id": "164",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2049",
        "area_name": "\u4e5d\u91cc\u533a",
        "area_parent_id": "164",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2050",
        "area_name": "\u4e91\u9f99\u533a",
        "area_parent_id": "164",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2051",
        "area_name": "\u65b0\u6c82\u5e02",
        "area_parent_id": "164",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2052",
        "area_name": "\u6c9b\u53bf",
        "area_parent_id": "164",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2053",
        "area_name": "\u6cc9\u5c71\u533a",
        "area_parent_id": "164",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2054",
        "area_name": "\u7762\u5b81\u53bf",
        "area_parent_id": "164",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2055",
        "area_name": "\u8d3e\u6c6a\u533a",
        "area_parent_id": "164",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2056",
        "area_name": "\u90b3\u5dde\u5e02",
        "area_parent_id": "164",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2057",
        "area_name": "\u94dc\u5c71\u53bf",
        "area_parent_id": "164",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2058",
        "area_name": "\u9f13\u697c\u533a",
        "area_parent_id": "164",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2059",
        "area_name": "\u5929\u5b81\u533a",
        "area_parent_id": "165",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2060",
        "area_name": "\u621a\u5885\u5830\u533a",
        "area_parent_id": "165",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2061",
        "area_name": "\u65b0\u5317\u533a",
        "area_parent_id": "165",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2062",
        "area_name": "\u6b66\u8fdb\u533a",
        "area_parent_id": "165",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2063",
        "area_name": "\u6ea7\u9633\u5e02",
        "area_parent_id": "165",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2064",
        "area_name": "\u91d1\u575b\u5e02",
        "area_parent_id": "165",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2065",
        "area_name": "\u949f\u697c\u533a",
        "area_parent_id": "165",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2066",
        "area_name": "\u5434\u4e2d\u533a",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2067",
        "area_name": "\u5434\u6c5f\u5e02",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2068",
        "area_name": "\u592a\u4ed3\u5e02",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2069",
        "area_name": "\u5e38\u719f\u5e02",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2070",
        "area_name": "\u5e73\u6c5f\u533a",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2071",
        "area_name": "\u5f20\u5bb6\u6e2f\u5e02",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2072",
        "area_name": "\u6606\u5c71\u5e02",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2073",
        "area_name": "\u6ca7\u6d6a\u533a",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2074",
        "area_name": "\u76f8\u57ce\u533a",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2075",
        "area_name": "\u82cf\u5dde\u5de5\u4e1a\u56ed\u533a",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2076",
        "area_name": "\u864e\u4e18\u533a",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2077",
        "area_name": "\u91d1\u960a\u533a",
        "area_parent_id": "166",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2078",
        "area_name": "\u542f\u4e1c\u5e02",
        "area_parent_id": "167",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2079",
        "area_name": "\u5982\u4e1c\u53bf",
        "area_parent_id": "167",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2080",
        "area_name": "\u5982\u768b\u5e02",
        "area_parent_id": "167",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2081",
        "area_name": "\u5d07\u5ddd\u533a",
        "area_parent_id": "167",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2082",
        "area_name": "\u6d77\u5b89\u53bf",
        "area_parent_id": "167",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2083",
        "area_name": "\u6d77\u95e8\u5e02",
        "area_parent_id": "167",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2084",
        "area_name": "\u6e2f\u95f8\u533a",
        "area_parent_id": "167",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2085",
        "area_name": "\u901a\u5dde\u5e02",
        "area_parent_id": "167",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2086",
        "area_name": "\u4e1c\u6d77\u53bf",
        "area_parent_id": "168",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2087",
        "area_name": "\u65b0\u6d66\u533a",
        "area_parent_id": "168",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2088",
        "area_name": "\u6d77\u5dde\u533a",
        "area_parent_id": "168",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2089",
        "area_name": "\u704c\u4e91\u53bf",
        "area_parent_id": "168",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2090",
        "area_name": "\u704c\u5357\u53bf",
        "area_parent_id": "168",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2091",
        "area_name": "\u8d63\u6986\u53bf",
        "area_parent_id": "168",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2092",
        "area_name": "\u8fde\u4e91\u533a",
        "area_parent_id": "168",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2093",
        "area_name": "\u695a\u5dde\u533a",
        "area_parent_id": "169",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2094",
        "area_name": "\u6d2a\u6cfd\u53bf",
        "area_parent_id": "169",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2095",
        "area_name": "\u6d9f\u6c34\u53bf",
        "area_parent_id": "169",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2096",
        "area_name": "\u6dee\u9634\u533a",
        "area_parent_id": "169",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2097",
        "area_name": "\u6e05\u6cb3\u533a",
        "area_parent_id": "169",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2098",
        "area_name": "\u6e05\u6d66\u533a",
        "area_parent_id": "169",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2099",
        "area_name": "\u76f1\u7719\u53bf",
        "area_parent_id": "169",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2100",
        "area_name": "\u91d1\u6e56\u53bf",
        "area_parent_id": "169",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2101",
        "area_name": "\u4e1c\u53f0\u5e02",
        "area_parent_id": "170",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2102",
        "area_name": "\u4ead\u6e56\u533a",
        "area_parent_id": "170",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2103",
        "area_name": "\u54cd\u6c34\u53bf",
        "area_parent_id": "170",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2104",
        "area_name": "\u5927\u4e30\u5e02",
        "area_parent_id": "170",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2105",
        "area_name": "\u5c04\u9633\u53bf",
        "area_parent_id": "170",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2106",
        "area_name": "\u5efa\u6e56\u53bf",
        "area_parent_id": "170",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2107",
        "area_name": "\u6ee8\u6d77\u53bf",
        "area_parent_id": "170",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2108",
        "area_name": "\u76d0\u90fd\u533a",
        "area_parent_id": "170",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2109",
        "area_name": "\u961c\u5b81\u53bf",
        "area_parent_id": "170",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2110",
        "area_name": "\u4eea\u5f81\u5e02",
        "area_parent_id": "171",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2111",
        "area_name": "\u5b9d\u5e94\u53bf",
        "area_parent_id": "171",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2112",
        "area_name": "\u5e7f\u9675\u533a",
        "area_parent_id": "171",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2113",
        "area_name": "\u6c5f\u90fd\u5e02",
        "area_parent_id": "171",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2114",
        "area_name": "\u7ef4\u626c\u533a",
        "area_parent_id": "171",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2115",
        "area_name": "\u9097\u6c5f\u533a",
        "area_parent_id": "171",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2116",
        "area_name": "\u9ad8\u90ae\u5e02",
        "area_parent_id": "171",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2117",
        "area_name": "\u4e39\u5f92\u533a",
        "area_parent_id": "172",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2118",
        "area_name": "\u4e39\u9633\u5e02",
        "area_parent_id": "172",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2119",
        "area_name": "\u4eac\u53e3\u533a",
        "area_parent_id": "172",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2120",
        "area_name": "\u53e5\u5bb9\u5e02",
        "area_parent_id": "172",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2121",
        "area_name": "\u626c\u4e2d\u5e02",
        "area_parent_id": "172",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2122",
        "area_name": "\u6da6\u5dde\u533a",
        "area_parent_id": "172",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2123",
        "area_name": "\u5174\u5316\u5e02",
        "area_parent_id": "173",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2124",
        "area_name": "\u59dc\u5830\u5e02",
        "area_parent_id": "173",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2125",
        "area_name": "\u6cf0\u5174\u5e02",
        "area_parent_id": "173",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2126",
        "area_name": "\u6d77\u9675\u533a",
        "area_parent_id": "173",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2127",
        "area_name": "\u9756\u6c5f\u5e02",
        "area_parent_id": "173",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2128",
        "area_name": "\u9ad8\u6e2f\u533a",
        "area_parent_id": "173",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2129",
        "area_name": "\u5bbf\u57ce\u533a",
        "area_parent_id": "174",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2130",
        "area_name": "\u5bbf\u8c6b\u533a",
        "area_parent_id": "174",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2131",
        "area_name": "\u6cad\u9633\u53bf",
        "area_parent_id": "174",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2132",
        "area_name": "\u6cd7\u6d2a\u53bf",
        "area_parent_id": "174",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2133",
        "area_name": "\u6cd7\u9633\u53bf",
        "area_parent_id": "174",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2134",
        "area_name": "\u4e0a\u57ce\u533a",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2135",
        "area_name": "\u4e0b\u57ce\u533a",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2136",
        "area_name": "\u4e34\u5b89\u5e02",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2137",
        "area_name": "\u4f59\u676d\u533a",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2138",
        "area_name": "\u5bcc\u9633\u5e02",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2139",
        "area_name": "\u5efa\u5fb7\u5e02",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2140",
        "area_name": "\u62f1\u5885\u533a",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2141",
        "area_name": "\u6850\u5e90\u53bf",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2142",
        "area_name": "\u6c5f\u5e72\u533a",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2143",
        "area_name": "\u6df3\u5b89\u53bf",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2144",
        "area_name": "\u6ee8\u6c5f\u533a",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2145",
        "area_name": "\u8427\u5c71\u533a",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2146",
        "area_name": "\u897f\u6e56\u533a",
        "area_parent_id": "175",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2147",
        "area_name": "\u4f59\u59da\u5e02",
        "area_parent_id": "176",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2148",
        "area_name": "\u5317\u4ed1\u533a",
        "area_parent_id": "176",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2149",
        "area_name": "\u5949\u5316\u5e02",
        "area_parent_id": "176",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2150",
        "area_name": "\u5b81\u6d77\u53bf",
        "area_parent_id": "176",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2151",
        "area_name": "\u6148\u6eaa\u5e02",
        "area_parent_id": "176",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2152",
        "area_name": "\u6c5f\u4e1c\u533a",
        "area_parent_id": "176",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2153",
        "area_name": "\u6c5f\u5317\u533a",
        "area_parent_id": "176",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2154",
        "area_name": "\u6d77\u66d9\u533a",
        "area_parent_id": "176",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2155",
        "area_name": "\u8c61\u5c71\u53bf",
        "area_parent_id": "176",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2156",
        "area_name": "\u911e\u5dde\u533a",
        "area_parent_id": "176",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2157",
        "area_name": "\u9547\u6d77\u533a",
        "area_parent_id": "176",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2158",
        "area_name": "\u4e50\u6e05\u5e02",
        "area_parent_id": "177",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2159",
        "area_name": "\u5e73\u9633\u53bf",
        "area_parent_id": "177",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2160",
        "area_name": "\u6587\u6210\u53bf",
        "area_parent_id": "177",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2161",
        "area_name": "\u6c38\u5609\u53bf",
        "area_parent_id": "177",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2162",
        "area_name": "\u6cf0\u987a\u53bf",
        "area_parent_id": "177",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2163",
        "area_name": "\u6d1e\u5934\u53bf",
        "area_parent_id": "177",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2164",
        "area_name": "\u745e\u5b89\u5e02",
        "area_parent_id": "177",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2165",
        "area_name": "\u74ef\u6d77\u533a",
        "area_parent_id": "177",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2166",
        "area_name": "\u82cd\u5357\u53bf",
        "area_parent_id": "177",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2167",
        "area_name": "\u9e7f\u57ce\u533a",
        "area_parent_id": "177",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2168",
        "area_name": "\u9f99\u6e7e\u533a",
        "area_parent_id": "177",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2169",
        "area_name": "\u5357\u6e56\u533a",
        "area_parent_id": "178",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2170",
        "area_name": "\u5609\u5584\u53bf",
        "area_parent_id": "178",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2171",
        "area_name": "\u5e73\u6e56\u5e02",
        "area_parent_id": "178",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2172",
        "area_name": "\u6850\u4e61\u5e02",
        "area_parent_id": "178",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2173",
        "area_name": "\u6d77\u5b81\u5e02",
        "area_parent_id": "178",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2174",
        "area_name": "\u6d77\u76d0\u53bf",
        "area_parent_id": "178",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2175",
        "area_name": "\u79c0\u6d32\u533a",
        "area_parent_id": "178",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2176",
        "area_name": "\u5357\u6d54\u533a",
        "area_parent_id": "179",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2177",
        "area_name": "\u5434\u5174\u533a",
        "area_parent_id": "179",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2178",
        "area_name": "\u5b89\u5409\u53bf",
        "area_parent_id": "179",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2179",
        "area_name": "\u5fb7\u6e05\u53bf",
        "area_parent_id": "179",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2180",
        "area_name": "\u957f\u5174\u53bf",
        "area_parent_id": "179",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2181",
        "area_name": "\u4e0a\u865e\u5e02",
        "area_parent_id": "180",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2182",
        "area_name": "\u5d4a\u5dde\u5e02",
        "area_parent_id": "180",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2183",
        "area_name": "\u65b0\u660c\u53bf",
        "area_parent_id": "180",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2184",
        "area_name": "\u7ecd\u5174\u53bf",
        "area_parent_id": "180",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2185",
        "area_name": "\u8bf8\u66a8\u5e02",
        "area_parent_id": "180",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2186",
        "area_name": "\u8d8a\u57ce\u533a",
        "area_parent_id": "180",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2187",
        "area_name": "\u5b9a\u6d77\u533a",
        "area_parent_id": "181",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2188",
        "area_name": "\u5cb1\u5c71\u53bf",
        "area_parent_id": "181",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2189",
        "area_name": "\u5d4a\u6cd7\u53bf",
        "area_parent_id": "181",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2190",
        "area_name": "\u666e\u9640\u533a",
        "area_parent_id": "181",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2191",
        "area_name": "\u5e38\u5c71\u53bf",
        "area_parent_id": "182",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2192",
        "area_name": "\u5f00\u5316\u53bf",
        "area_parent_id": "182",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2193",
        "area_name": "\u67ef\u57ce\u533a",
        "area_parent_id": "182",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2194",
        "area_name": "\u6c5f\u5c71\u5e02",
        "area_parent_id": "182",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2195",
        "area_name": "\u8862\u6c5f\u533a",
        "area_parent_id": "182",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2196",
        "area_name": "\u9f99\u6e38\u53bf",
        "area_parent_id": "182",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2197",
        "area_name": "\u4e1c\u9633\u5e02",
        "area_parent_id": "183",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2198",
        "area_name": "\u4e49\u4e4c\u5e02",
        "area_parent_id": "183",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2199",
        "area_name": "\u5170\u6eaa\u5e02",
        "area_parent_id": "183",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2200",
        "area_name": "\u5a7a\u57ce\u533a",
        "area_parent_id": "183",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2201",
        "area_name": "\u6b66\u4e49\u53bf",
        "area_parent_id": "183",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2202",
        "area_name": "\u6c38\u5eb7\u5e02",
        "area_parent_id": "183",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2203",
        "area_name": "\u6d66\u6c5f\u53bf",
        "area_parent_id": "183",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2204",
        "area_name": "\u78d0\u5b89\u53bf",
        "area_parent_id": "183",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2205",
        "area_name": "\u91d1\u4e1c\u533a",
        "area_parent_id": "183",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2206",
        "area_name": "\u4e09\u95e8\u53bf",
        "area_parent_id": "184",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2207",
        "area_name": "\u4e34\u6d77\u5e02",
        "area_parent_id": "184",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2208",
        "area_name": "\u4ed9\u5c45\u53bf",
        "area_parent_id": "184",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2209",
        "area_name": "\u5929\u53f0\u53bf",
        "area_parent_id": "184",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2210",
        "area_name": "\u6912\u6c5f\u533a",
        "area_parent_id": "184",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2211",
        "area_name": "\u6e29\u5cad\u5e02",
        "area_parent_id": "184",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2212",
        "area_name": "\u7389\u73af\u53bf",
        "area_parent_id": "184",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2213",
        "area_name": "\u8def\u6865\u533a",
        "area_parent_id": "184",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2214",
        "area_name": "\u9ec4\u5ca9\u533a",
        "area_parent_id": "184",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2215",
        "area_name": "\u4e91\u548c\u53bf",
        "area_parent_id": "185",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2216",
        "area_name": "\u5e86\u5143\u53bf",
        "area_parent_id": "185",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2217",
        "area_name": "\u666f\u5b81\u7572\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "185",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2218",
        "area_name": "\u677e\u9633\u53bf",
        "area_parent_id": "185",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2219",
        "area_name": "\u7f19\u4e91\u53bf",
        "area_parent_id": "185",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2220",
        "area_name": "\u83b2\u90fd\u533a",
        "area_parent_id": "185",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2221",
        "area_name": "\u9042\u660c\u53bf",
        "area_parent_id": "185",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2222",
        "area_name": "\u9752\u7530\u53bf",
        "area_parent_id": "185",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2223",
        "area_name": "\u9f99\u6cc9\u5e02",
        "area_parent_id": "185",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2224",
        "area_name": "\u5305\u6cb3\u533a",
        "area_parent_id": "186",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2225",
        "area_name": "\u5e90\u9633\u533a",
        "area_parent_id": "186",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2226",
        "area_name": "\u7476\u6d77\u533a",
        "area_parent_id": "186",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2227",
        "area_name": "\u80a5\u4e1c\u53bf",
        "area_parent_id": "186",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2228",
        "area_name": "\u80a5\u897f\u53bf",
        "area_parent_id": "186",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2229",
        "area_name": "\u8700\u5c71\u533a",
        "area_parent_id": "186",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2230",
        "area_name": "\u957f\u4e30\u53bf",
        "area_parent_id": "186",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2231",
        "area_name": "\u4e09\u5c71\u533a",
        "area_parent_id": "187",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2232",
        "area_name": "\u5357\u9675\u53bf",
        "area_parent_id": "187",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2233",
        "area_name": "\u5f0b\u6c5f\u533a",
        "area_parent_id": "187",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2234",
        "area_name": "\u7e41\u660c\u53bf",
        "area_parent_id": "187",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2235",
        "area_name": "\u829c\u6e56\u53bf",
        "area_parent_id": "187",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2236",
        "area_name": "\u955c\u6e56\u533a",
        "area_parent_id": "187",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2237",
        "area_name": "\u9e20\u6c5f\u533a",
        "area_parent_id": "187",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2238",
        "area_name": "\u4e94\u6cb3\u53bf",
        "area_parent_id": "188",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2239",
        "area_name": "\u56fa\u9547\u53bf",
        "area_parent_id": "188",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2240",
        "area_name": "\u6000\u8fdc\u53bf",
        "area_parent_id": "188",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2241",
        "area_name": "\u6dee\u4e0a\u533a",
        "area_parent_id": "188",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2242",
        "area_name": "\u79b9\u4f1a\u533a",
        "area_parent_id": "188",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2243",
        "area_name": "\u868c\u5c71\u533a",
        "area_parent_id": "188",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2244",
        "area_name": "\u9f99\u5b50\u6e56\u533a",
        "area_parent_id": "188",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2245",
        "area_name": "\u516b\u516c\u5c71\u533a",
        "area_parent_id": "189",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2246",
        "area_name": "\u51e4\u53f0\u53bf",
        "area_parent_id": "189",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2247",
        "area_name": "\u5927\u901a\u533a",
        "area_parent_id": "189",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2248",
        "area_name": "\u6f58\u96c6\u533a",
        "area_parent_id": "189",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2249",
        "area_name": "\u7530\u5bb6\u5eb5\u533a",
        "area_parent_id": "189",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2250",
        "area_name": "\u8c22\u5bb6\u96c6\u533a",
        "area_parent_id": "189",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2251",
        "area_name": "\u5f53\u6d82\u53bf",
        "area_parent_id": "190",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2252",
        "area_name": "\u82b1\u5c71\u533a",
        "area_parent_id": "190",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2253",
        "area_name": "\u91d1\u5bb6\u5e84\u533a",
        "area_parent_id": "190",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2254",
        "area_name": "\u96e8\u5c71\u533a",
        "area_parent_id": "190",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2255",
        "area_name": "\u675c\u96c6\u533a",
        "area_parent_id": "191",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2256",
        "area_name": "\u6fc9\u6eaa\u53bf",
        "area_parent_id": "191",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2257",
        "area_name": "\u70c8\u5c71\u533a",
        "area_parent_id": "191",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2258",
        "area_name": "\u76f8\u5c71\u533a",
        "area_parent_id": "191",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2259",
        "area_name": "\u72ee\u5b50\u5c71\u533a",
        "area_parent_id": "192",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2260",
        "area_name": "\u90ca\u533a",
        "area_parent_id": "192",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2261",
        "area_name": "\u94dc\u5b98\u5c71\u533a",
        "area_parent_id": "192",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2262",
        "area_name": "\u94dc\u9675\u53bf",
        "area_parent_id": "192",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2263",
        "area_name": "\u5927\u89c2\u533a",
        "area_parent_id": "193",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2264",
        "area_name": "\u592a\u6e56\u53bf",
        "area_parent_id": "193",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2265",
        "area_name": "\u5b9c\u79c0\u533a",
        "area_parent_id": "193",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2266",
        "area_name": "\u5bbf\u677e\u53bf",
        "area_parent_id": "193",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2267",
        "area_name": "\u5cb3\u897f\u53bf",
        "area_parent_id": "193",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2268",
        "area_name": "\u6000\u5b81\u53bf",
        "area_parent_id": "193",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2269",
        "area_name": "\u671b\u6c5f\u53bf",
        "area_parent_id": "193",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2270",
        "area_name": "\u679e\u9633\u53bf",
        "area_parent_id": "193",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2271",
        "area_name": "\u6850\u57ce\u5e02",
        "area_parent_id": "193",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2272",
        "area_name": "\u6f5c\u5c71\u53bf",
        "area_parent_id": "193",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2273",
        "area_name": "\u8fce\u6c5f\u533a",
        "area_parent_id": "193",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2274",
        "area_name": "\u4f11\u5b81\u53bf",
        "area_parent_id": "194",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2275",
        "area_name": "\u5c6f\u6eaa\u533a",
        "area_parent_id": "194",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2276",
        "area_name": "\u5fbd\u5dde\u533a",
        "area_parent_id": "194",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2277",
        "area_name": "\u6b59\u53bf",
        "area_parent_id": "194",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2278",
        "area_name": "\u7941\u95e8\u53bf",
        "area_parent_id": "194",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2279",
        "area_name": "\u9ec4\u5c71\u533a",
        "area_parent_id": "194",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2280",
        "area_name": "\u9edf\u53bf",
        "area_parent_id": "194",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2281",
        "area_name": "\u5168\u6912\u53bf",
        "area_parent_id": "195",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2282",
        "area_name": "\u51e4\u9633\u53bf",
        "area_parent_id": "195",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2283",
        "area_name": "\u5357\u8c2f\u533a",
        "area_parent_id": "195",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2284",
        "area_name": "\u5929\u957f\u5e02",
        "area_parent_id": "195",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2285",
        "area_name": "\u5b9a\u8fdc\u53bf",
        "area_parent_id": "195",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2286",
        "area_name": "\u660e\u5149\u5e02",
        "area_parent_id": "195",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2287",
        "area_name": "\u6765\u5b89\u53bf",
        "area_parent_id": "195",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2288",
        "area_name": "\u7405\u73a1\u533a",
        "area_parent_id": "195",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2289",
        "area_name": "\u4e34\u6cc9\u53bf",
        "area_parent_id": "196",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2290",
        "area_name": "\u592a\u548c\u53bf",
        "area_parent_id": "196",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2291",
        "area_name": "\u754c\u9996\u5e02",
        "area_parent_id": "196",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2292",
        "area_name": "\u961c\u5357\u53bf",
        "area_parent_id": "196",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2293",
        "area_name": "\u988d\u4e1c\u533a",
        "area_parent_id": "196",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2294",
        "area_name": "\u988d\u5dde\u533a",
        "area_parent_id": "196",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2295",
        "area_name": "\u988d\u6cc9\u533a",
        "area_parent_id": "196",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2296",
        "area_name": "\u9896\u4e0a\u53bf",
        "area_parent_id": "196",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2297",
        "area_name": "\u57c7\u6865\u533a",
        "area_parent_id": "197",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2298",
        "area_name": "\u6cd7\u53bf\u8f96",
        "area_parent_id": "197",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2299",
        "area_name": "\u7075\u74a7\u53bf",
        "area_parent_id": "197",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2300",
        "area_name": "\u7800\u5c71\u53bf",
        "area_parent_id": "197",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2301",
        "area_name": "\u8427\u53bf",
        "area_parent_id": "197",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2302",
        "area_name": "\u542b\u5c71\u53bf",
        "area_parent_id": "198",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2303",
        "area_name": "\u548c\u53bf",
        "area_parent_id": "198",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2304",
        "area_name": "\u5c45\u5de2\u533a",
        "area_parent_id": "198",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2305",
        "area_name": "\u5e90\u6c5f\u53bf",
        "area_parent_id": "198",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2306",
        "area_name": "\u65e0\u4e3a\u53bf",
        "area_parent_id": "198",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2307",
        "area_name": "\u5bff\u53bf",
        "area_parent_id": "199",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2308",
        "area_name": "\u8212\u57ce\u53bf",
        "area_parent_id": "199",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2309",
        "area_name": "\u88d5\u5b89\u533a",
        "area_parent_id": "199",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2310",
        "area_name": "\u91d1\u5b89\u533a",
        "area_parent_id": "199",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2311",
        "area_name": "\u91d1\u5be8\u53bf",
        "area_parent_id": "199",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2312",
        "area_name": "\u970d\u5c71\u53bf",
        "area_parent_id": "199",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2313",
        "area_name": "\u970d\u90b1\u53bf",
        "area_parent_id": "199",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2314",
        "area_name": "\u5229\u8f9b\u53bf",
        "area_parent_id": "200",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2315",
        "area_name": "\u6da1\u9633\u53bf",
        "area_parent_id": "200",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2316",
        "area_name": "\u8499\u57ce\u53bf",
        "area_parent_id": "200",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2317",
        "area_name": "\u8c2f\u57ce\u533a",
        "area_parent_id": "200",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2318",
        "area_name": "\u4e1c\u81f3\u53bf",
        "area_parent_id": "201",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2319",
        "area_name": "\u77f3\u53f0\u53bf",
        "area_parent_id": "201",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2320",
        "area_name": "\u8d35\u6c60\u533a",
        "area_parent_id": "201",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2321",
        "area_name": "\u9752\u9633\u53bf",
        "area_parent_id": "201",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2322",
        "area_name": "\u5b81\u56fd\u5e02",
        "area_parent_id": "202",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2323",
        "area_name": "\u5ba3\u5dde\u533a",
        "area_parent_id": "202",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2324",
        "area_name": "\u5e7f\u5fb7\u53bf",
        "area_parent_id": "202",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2325",
        "area_name": "\u65cc\u5fb7\u53bf",
        "area_parent_id": "202",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2326",
        "area_name": "\u6cfe\u53bf",
        "area_parent_id": "202",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2327",
        "area_name": "\u7ee9\u6eaa\u53bf",
        "area_parent_id": "202",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2328",
        "area_name": "\u90ce\u6eaa\u53bf",
        "area_parent_id": "202",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2329",
        "area_name": "\u4ed3\u5c71\u533a",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2330",
        "area_name": "\u53f0\u6c5f\u533a",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2331",
        "area_name": "\u5e73\u6f6d\u53bf",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2332",
        "area_name": "\u664b\u5b89\u533a",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2333",
        "area_name": "\u6c38\u6cf0\u53bf",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2334",
        "area_name": "\u798f\u6e05\u5e02",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2335",
        "area_name": "\u7f57\u6e90\u53bf",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2336",
        "area_name": "\u8fde\u6c5f\u53bf",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2337",
        "area_name": "\u957f\u4e50\u5e02",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2338",
        "area_name": "\u95fd\u4faf\u53bf",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2339",
        "area_name": "\u95fd\u6e05\u53bf",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2340",
        "area_name": "\u9a6c\u5c3e\u533a",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2341",
        "area_name": "\u9f13\u697c\u533a",
        "area_parent_id": "203",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2342",
        "area_name": "\u540c\u5b89\u533a",
        "area_parent_id": "204",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2343",
        "area_name": "\u601d\u660e\u533a",
        "area_parent_id": "204",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2344",
        "area_name": "\u6d77\u6ca7\u533a",
        "area_parent_id": "204",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2345",
        "area_name": "\u6e56\u91cc\u533a",
        "area_parent_id": "204",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2346",
        "area_name": "\u7fd4\u5b89\u533a",
        "area_parent_id": "204",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2347",
        "area_name": "\u96c6\u7f8e\u533a",
        "area_parent_id": "204",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2348",
        "area_name": "\u4ed9\u6e38\u53bf",
        "area_parent_id": "205",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2349",
        "area_name": "\u57ce\u53a2\u533a",
        "area_parent_id": "205",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2350",
        "area_name": "\u6db5\u6c5f\u533a",
        "area_parent_id": "205",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2351",
        "area_name": "\u79c0\u5c7f\u533a",
        "area_parent_id": "205",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2352",
        "area_name": "\u8354\u57ce\u533a",
        "area_parent_id": "205",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2353",
        "area_name": "\u4e09\u5143\u533a",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2354",
        "area_name": "\u5927\u7530\u53bf",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2355",
        "area_name": "\u5b81\u5316\u53bf",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2356",
        "area_name": "\u5c06\u4e50\u53bf",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2357",
        "area_name": "\u5c24\u6eaa\u53bf",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2358",
        "area_name": "\u5efa\u5b81\u53bf",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2359",
        "area_name": "\u660e\u6eaa\u53bf",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2360",
        "area_name": "\u6885\u5217\u533a",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2361",
        "area_name": "\u6c38\u5b89\u5e02",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2362",
        "area_name": "\u6c99\u53bf",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2363",
        "area_name": "\u6cf0\u5b81\u53bf",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2364",
        "area_name": "\u6e05\u6d41\u53bf",
        "area_parent_id": "206",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2365",
        "area_name": "\u4e30\u6cfd\u533a",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2366",
        "area_name": "\u5357\u5b89\u5e02",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2367",
        "area_name": "\u5b89\u6eaa\u53bf",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2368",
        "area_name": "\u5fb7\u5316\u53bf",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2369",
        "area_name": "\u60e0\u5b89\u53bf",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2370",
        "area_name": "\u664b\u6c5f\u5e02",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2371",
        "area_name": "\u6c38\u6625\u53bf",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2372",
        "area_name": "\u6cc9\u6e2f\u533a",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2373",
        "area_name": "\u6d1b\u6c5f\u533a",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2374",
        "area_name": "\u77f3\u72ee\u5e02",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2375",
        "area_name": "\u91d1\u95e8\u53bf",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2376",
        "area_name": "\u9ca4\u57ce\u533a",
        "area_parent_id": "207",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2377",
        "area_name": "\u4e1c\u5c71\u53bf",
        "area_parent_id": "208",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2378",
        "area_name": "\u4e91\u9704\u53bf",
        "area_parent_id": "208",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2379",
        "area_name": "\u534e\u5b89\u53bf",
        "area_parent_id": "208",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2380",
        "area_name": "\u5357\u9756\u53bf",
        "area_parent_id": "208",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2381",
        "area_name": "\u5e73\u548c\u53bf",
        "area_parent_id": "208",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2382",
        "area_name": "\u6f33\u6d66\u53bf",
        "area_parent_id": "208",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2383",
        "area_name": "\u8297\u57ce\u533a",
        "area_parent_id": "208",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2384",
        "area_name": "\u8bcf\u5b89\u53bf",
        "area_parent_id": "208",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2385",
        "area_name": "\u957f\u6cf0\u53bf",
        "area_parent_id": "208",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2386",
        "area_name": "\u9f99\u6587\u533a",
        "area_parent_id": "208",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2387",
        "area_name": "\u9f99\u6d77\u5e02",
        "area_parent_id": "208",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2388",
        "area_name": "\u5149\u6cfd\u53bf",
        "area_parent_id": "209",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2389",
        "area_name": "\u5ef6\u5e73\u533a",
        "area_parent_id": "209",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2390",
        "area_name": "\u5efa\u74ef\u5e02",
        "area_parent_id": "209",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2391",
        "area_name": "\u5efa\u9633\u5e02",
        "area_parent_id": "209",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2392",
        "area_name": "\u653f\u548c\u53bf",
        "area_parent_id": "209",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2393",
        "area_name": "\u677e\u6eaa\u53bf",
        "area_parent_id": "209",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2394",
        "area_name": "\u6b66\u5937\u5c71\u5e02",
        "area_parent_id": "209",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2395",
        "area_name": "\u6d66\u57ce\u53bf",
        "area_parent_id": "209",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2396",
        "area_name": "\u90b5\u6b66\u5e02",
        "area_parent_id": "209",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2397",
        "area_name": "\u987a\u660c\u53bf",
        "area_parent_id": "209",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2398",
        "area_name": "\u4e0a\u676d\u53bf",
        "area_parent_id": "210",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2399",
        "area_name": "\u65b0\u7f57\u533a",
        "area_parent_id": "210",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2400",
        "area_name": "\u6b66\u5e73\u53bf",
        "area_parent_id": "210",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2401",
        "area_name": "\u6c38\u5b9a\u53bf",
        "area_parent_id": "210",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2402",
        "area_name": "\u6f33\u5e73\u5e02",
        "area_parent_id": "210",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2403",
        "area_name": "\u8fde\u57ce\u53bf",
        "area_parent_id": "210",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2404",
        "area_name": "\u957f\u6c40\u53bf",
        "area_parent_id": "210",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2405",
        "area_name": "\u53e4\u7530\u53bf",
        "area_parent_id": "211",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2406",
        "area_name": "\u5468\u5b81\u53bf",
        "area_parent_id": "211",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2407",
        "area_name": "\u5bff\u5b81\u53bf",
        "area_parent_id": "211",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2408",
        "area_name": "\u5c4f\u5357\u53bf",
        "area_parent_id": "211",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2409",
        "area_name": "\u67d8\u8363\u53bf",
        "area_parent_id": "211",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2410",
        "area_name": "\u798f\u5b89\u5e02",
        "area_parent_id": "211",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2411",
        "area_name": "\u798f\u9f0e\u5e02",
        "area_parent_id": "211",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2412",
        "area_name": "\u8549\u57ce\u533a",
        "area_parent_id": "211",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2413",
        "area_name": "\u971e\u6d66\u53bf",
        "area_parent_id": "211",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2414",
        "area_name": "\u4e1c\u6e56\u533a",
        "area_parent_id": "212",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2415",
        "area_name": "\u5357\u660c\u53bf",
        "area_parent_id": "212",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2416",
        "area_name": "\u5b89\u4e49\u53bf",
        "area_parent_id": "212",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2417",
        "area_name": "\u65b0\u5efa\u53bf",
        "area_parent_id": "212",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2418",
        "area_name": "\u6e7e\u91cc\u533a",
        "area_parent_id": "212",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2419",
        "area_name": "\u897f\u6e56\u533a",
        "area_parent_id": "212",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2420",
        "area_name": "\u8fdb\u8d24\u53bf",
        "area_parent_id": "212",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2421",
        "area_name": "\u9752\u4e91\u8c31\u533a",
        "area_parent_id": "212",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2422",
        "area_name": "\u9752\u5c71\u6e56\u533a",
        "area_parent_id": "212",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2423",
        "area_name": "\u4e50\u5e73\u5e02",
        "area_parent_id": "213",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2424",
        "area_name": "\u660c\u6c5f\u533a",
        "area_parent_id": "213",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2425",
        "area_name": "\u6d6e\u6881\u53bf",
        "area_parent_id": "213",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2426",
        "area_name": "\u73e0\u5c71\u533a",
        "area_parent_id": "213",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2427",
        "area_name": "\u4e0a\u6817\u53bf",
        "area_parent_id": "214",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2428",
        "area_name": "\u5b89\u6e90\u533a",
        "area_parent_id": "214",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2429",
        "area_name": "\u6e58\u4e1c\u533a",
        "area_parent_id": "214",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2430",
        "area_name": "\u82a6\u6eaa\u53bf",
        "area_parent_id": "214",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2431",
        "area_name": "\u83b2\u82b1\u53bf",
        "area_parent_id": "214",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2432",
        "area_name": "\u4e5d\u6c5f\u53bf",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2433",
        "area_name": "\u4fee\u6c34\u53bf",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2434",
        "area_name": "\u5e90\u5c71\u533a",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2435",
        "area_name": "\u5f6d\u6cfd\u53bf",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2436",
        "area_name": "\u5fb7\u5b89\u53bf",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2437",
        "area_name": "\u661f\u5b50\u53bf",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2438",
        "area_name": "\u6b66\u5b81\u53bf",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2439",
        "area_name": "\u6c38\u4fee\u53bf",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2440",
        "area_name": "\u6d54\u9633\u533a",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2441",
        "area_name": "\u6e56\u53e3\u53bf",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2442",
        "area_name": "\u745e\u660c\u5e02",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2443",
        "area_name": "\u90fd\u660c\u53bf",
        "area_parent_id": "215",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2444",
        "area_name": "\u5206\u5b9c\u53bf",
        "area_parent_id": "216",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2445",
        "area_name": "\u6e1d\u6c34\u533a",
        "area_parent_id": "216",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2446",
        "area_name": "\u4f59\u6c5f\u53bf",
        "area_parent_id": "217",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2447",
        "area_name": "\u6708\u6e56\u533a",
        "area_parent_id": "217",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2448",
        "area_name": "\u8d35\u6eaa\u5e02",
        "area_parent_id": "217",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2449",
        "area_name": "\u4e0a\u72b9\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2450",
        "area_name": "\u4e8e\u90fd\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2451",
        "area_name": "\u4f1a\u660c\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2452",
        "area_name": "\u4fe1\u4e30\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2453",
        "area_name": "\u5168\u5357\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2454",
        "area_name": "\u5174\u56fd\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2455",
        "area_name": "\u5357\u5eb7\u5e02",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2456",
        "area_name": "\u5927\u4f59\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2457",
        "area_name": "\u5b81\u90fd\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2458",
        "area_name": "\u5b89\u8fdc\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2459",
        "area_name": "\u5b9a\u5357\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2460",
        "area_name": "\u5bfb\u4e4c\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2461",
        "area_name": "\u5d07\u4e49\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2462",
        "area_name": "\u745e\u91d1\u5e02",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2463",
        "area_name": "\u77f3\u57ce\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2464",
        "area_name": "\u7ae0\u8d21\u533a",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2465",
        "area_name": "\u8d63\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2466",
        "area_name": "\u9f99\u5357\u53bf",
        "area_parent_id": "218",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2467",
        "area_name": "\u4e07\u5b89\u53bf",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2468",
        "area_name": "\u4e95\u5188\u5c71\u5e02",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2469",
        "area_name": "\u5409\u5b89\u53bf",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2470",
        "area_name": "\u5409\u5dde\u533a",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2471",
        "area_name": "\u5409\u6c34\u53bf",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2472",
        "area_name": "\u5b89\u798f\u53bf",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2473",
        "area_name": "\u5ce1\u6c5f\u53bf",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2474",
        "area_name": "\u65b0\u5e72\u53bf",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2475",
        "area_name": "\u6c38\u4e30\u53bf",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2476",
        "area_name": "\u6c38\u65b0\u53bf",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2477",
        "area_name": "\u6cf0\u548c\u53bf",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2478",
        "area_name": "\u9042\u5ddd\u53bf",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2479",
        "area_name": "\u9752\u539f\u533a",
        "area_parent_id": "219",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2480",
        "area_name": "\u4e07\u8f7d\u53bf",
        "area_parent_id": "220",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2481",
        "area_name": "\u4e0a\u9ad8\u53bf",
        "area_parent_id": "220",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2482",
        "area_name": "\u4e30\u57ce\u5e02",
        "area_parent_id": "220",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2483",
        "area_name": "\u5949\u65b0\u53bf",
        "area_parent_id": "220",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2484",
        "area_name": "\u5b9c\u4e30\u53bf",
        "area_parent_id": "220",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2485",
        "area_name": "\u6a1f\u6811\u5e02",
        "area_parent_id": "220",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2486",
        "area_name": "\u8881\u5dde\u533a",
        "area_parent_id": "220",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2487",
        "area_name": "\u94dc\u9f13\u53bf",
        "area_parent_id": "220",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2488",
        "area_name": "\u9756\u5b89\u53bf",
        "area_parent_id": "220",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2489",
        "area_name": "\u9ad8\u5b89\u5e02",
        "area_parent_id": "220",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2490",
        "area_name": "\u4e1c\u4e61\u53bf",
        "area_parent_id": "221",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2491",
        "area_name": "\u4e34\u5ddd\u533a",
        "area_parent_id": "221",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2492",
        "area_name": "\u4e50\u5b89\u53bf",
        "area_parent_id": "221",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2493",
        "area_name": "\u5357\u4e30\u53bf",
        "area_parent_id": "221",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2494",
        "area_name": "\u5357\u57ce\u53bf",
        "area_parent_id": "221",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2495",
        "area_name": "\u5b9c\u9ec4\u53bf",
        "area_parent_id": "221",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2496",
        "area_name": "\u5d07\u4ec1\u53bf",
        "area_parent_id": "221",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2497",
        "area_name": "\u5e7f\u660c\u53bf",
        "area_parent_id": "221",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2498",
        "area_name": "\u8d44\u6eaa\u53bf",
        "area_parent_id": "221",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2499",
        "area_name": "\u91d1\u6eaa\u53bf",
        "area_parent_id": "221",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2500",
        "area_name": "\u9ece\u5ddd\u53bf",
        "area_parent_id": "221",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2501",
        "area_name": "\u4e07\u5e74\u53bf",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2502",
        "area_name": "\u4e0a\u9976\u53bf",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2503",
        "area_name": "\u4f59\u5e72\u53bf",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2504",
        "area_name": "\u4fe1\u5dde\u533a",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2505",
        "area_name": "\u5a7a\u6e90\u53bf",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2506",
        "area_name": "\u5e7f\u4e30\u53bf",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2507",
        "area_name": "\u5f0b\u9633\u53bf",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2508",
        "area_name": "\u5fb7\u5174\u5e02",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2509",
        "area_name": "\u6a2a\u5cf0\u53bf",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2510",
        "area_name": "\u7389\u5c71\u53bf",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2511",
        "area_name": "\u9131\u9633\u53bf",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2512",
        "area_name": "\u94c5\u5c71\u53bf",
        "area_parent_id": "222",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2513",
        "area_name": "\u5386\u4e0b\u533a",
        "area_parent_id": "223",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2514",
        "area_name": "\u5386\u57ce\u533a",
        "area_parent_id": "223",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2515",
        "area_name": "\u5546\u6cb3\u53bf",
        "area_parent_id": "223",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2516",
        "area_name": "\u5929\u6865\u533a",
        "area_parent_id": "223",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2517",
        "area_name": "\u5e02\u4e2d\u533a",
        "area_parent_id": "223",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2518",
        "area_name": "\u5e73\u9634\u53bf",
        "area_parent_id": "223",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2519",
        "area_name": "\u69d0\u836b\u533a",
        "area_parent_id": "223",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2520",
        "area_name": "\u6d4e\u9633\u53bf",
        "area_parent_id": "223",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2521",
        "area_name": "\u7ae0\u4e18\u5e02",
        "area_parent_id": "223",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2522",
        "area_name": "\u957f\u6e05\u533a",
        "area_parent_id": "223",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2523",
        "area_name": "\u5373\u58a8\u5e02",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2524",
        "area_name": "\u56db\u65b9\u533a",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2525",
        "area_name": "\u57ce\u9633\u533a",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2526",
        "area_name": "\u5d02\u5c71\u533a",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2527",
        "area_name": "\u5e02\u5317\u533a",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2528",
        "area_name": "\u5e02\u5357\u533a",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2529",
        "area_name": "\u5e73\u5ea6\u5e02",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2530",
        "area_name": "\u674e\u6ca7\u533a",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2531",
        "area_name": "\u80f6\u5357\u5e02",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2532",
        "area_name": "\u80f6\u5dde\u5e02",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2533",
        "area_name": "\u83b1\u897f\u5e02",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2534",
        "area_name": "\u9ec4\u5c9b\u533a",
        "area_parent_id": "224",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2535",
        "area_name": "\u4e34\u6dc4\u533a",
        "area_parent_id": "225",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2536",
        "area_name": "\u535a\u5c71\u533a",
        "area_parent_id": "225",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2537",
        "area_name": "\u5468\u6751\u533a",
        "area_parent_id": "225",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2538",
        "area_name": "\u5f20\u5e97\u533a",
        "area_parent_id": "225",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2539",
        "area_name": "\u6853\u53f0\u53bf",
        "area_parent_id": "225",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2540",
        "area_name": "\u6c82\u6e90\u53bf",
        "area_parent_id": "225",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2541",
        "area_name": "\u6dc4\u5ddd\u533a",
        "area_parent_id": "225",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2542",
        "area_name": "\u9ad8\u9752\u53bf",
        "area_parent_id": "225",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2543",
        "area_name": "\u53f0\u513f\u5e84\u533a",
        "area_parent_id": "226",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2544",
        "area_name": "\u5c71\u4ead\u533a",
        "area_parent_id": "226",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2545",
        "area_name": "\u5cc4\u57ce\u533a",
        "area_parent_id": "226",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2546",
        "area_name": "\u5e02\u4e2d\u533a",
        "area_parent_id": "226",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2547",
        "area_name": "\u6ed5\u5dde\u5e02",
        "area_parent_id": "226",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2548",
        "area_name": "\u859b\u57ce\u533a",
        "area_parent_id": "226",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2549",
        "area_name": "\u4e1c\u8425\u533a",
        "area_parent_id": "227",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2550",
        "area_name": "\u5229\u6d25\u53bf",
        "area_parent_id": "227",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2551",
        "area_name": "\u57a6\u5229\u53bf",
        "area_parent_id": "227",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2552",
        "area_name": "\u5e7f\u9976\u53bf",
        "area_parent_id": "227",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2553",
        "area_name": "\u6cb3\u53e3\u533a",
        "area_parent_id": "227",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2554",
        "area_name": "\u62db\u8fdc\u5e02",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2555",
        "area_name": "\u6816\u971e\u5e02",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2556",
        "area_name": "\u6d77\u9633\u5e02",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2557",
        "area_name": "\u725f\u5e73\u533a",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2558",
        "area_name": "\u798f\u5c71\u533a",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2559",
        "area_name": "\u829d\u7f58\u533a",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2560",
        "area_name": "\u83b1\u5c71\u533a",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2561",
        "area_name": "\u83b1\u5dde\u5e02",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2562",
        "area_name": "\u83b1\u9633\u5e02",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2563",
        "area_name": "\u84ec\u83b1\u5e02",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2564",
        "area_name": "\u957f\u5c9b\u53bf",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2565",
        "area_name": "\u9f99\u53e3\u5e02",
        "area_parent_id": "228",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2566",
        "area_name": "\u4e34\u6710\u53bf",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2567",
        "area_name": "\u574a\u5b50\u533a",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2568",
        "area_name": "\u594e\u6587\u533a",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2569",
        "area_name": "\u5b89\u4e18\u5e02",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2570",
        "area_name": "\u5bd2\u4ead\u533a",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2571",
        "area_name": "\u5bff\u5149\u5e02",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2572",
        "area_name": "\u660c\u4e50\u53bf",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2573",
        "area_name": "\u660c\u9091\u5e02",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2574",
        "area_name": "\u6f4d\u57ce\u533a",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2575",
        "area_name": "\u8bf8\u57ce\u5e02",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2576",
        "area_name": "\u9752\u5dde\u5e02",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2577",
        "area_name": "\u9ad8\u5bc6\u5e02",
        "area_parent_id": "229",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2578",
        "area_name": "\u4efb\u57ce\u533a",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2579",
        "area_name": "\u5156\u5dde\u5e02",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2580",
        "area_name": "\u5609\u7965\u53bf",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2581",
        "area_name": "\u5e02\u4e2d\u533a",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2582",
        "area_name": "\u5fae\u5c71\u53bf",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2583",
        "area_name": "\u66f2\u961c\u5e02",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2584",
        "area_name": "\u6881\u5c71\u53bf",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2585",
        "area_name": "\u6c76\u4e0a\u53bf",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2586",
        "area_name": "\u6cd7\u6c34\u53bf",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2587",
        "area_name": "\u90b9\u57ce\u5e02",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2588",
        "area_name": "\u91d1\u4e61\u53bf",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2589",
        "area_name": "\u9c7c\u53f0\u53bf",
        "area_parent_id": "230",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2590",
        "area_name": "\u4e1c\u5e73\u53bf",
        "area_parent_id": "231",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2591",
        "area_name": "\u5b81\u9633\u53bf",
        "area_parent_id": "231",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2592",
        "area_name": "\u5cb1\u5cb3\u533a",
        "area_parent_id": "231",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2593",
        "area_name": "\u65b0\u6cf0\u5e02",
        "area_parent_id": "231",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2594",
        "area_name": "\u6cf0\u5c71\u533a",
        "area_parent_id": "231",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2595",
        "area_name": "\u80a5\u57ce\u5e02",
        "area_parent_id": "231",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2596",
        "area_name": "\u4e73\u5c71\u5e02",
        "area_parent_id": "232",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2597",
        "area_name": "\u6587\u767b\u5e02",
        "area_parent_id": "232",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2598",
        "area_name": "\u73af\u7fe0\u533a",
        "area_parent_id": "232",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2599",
        "area_name": "\u8363\u6210\u5e02",
        "area_parent_id": "232",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2600",
        "area_name": "\u4e1c\u6e2f\u533a",
        "area_parent_id": "233",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2601",
        "area_name": "\u4e94\u83b2\u53bf",
        "area_parent_id": "233",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2602",
        "area_name": "\u5c9a\u5c71\u533a",
        "area_parent_id": "233",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2603",
        "area_name": "\u8392\u53bf",
        "area_parent_id": "233",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2604",
        "area_name": "\u83b1\u57ce\u533a",
        "area_parent_id": "234",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2605",
        "area_name": "\u94a2\u57ce\u533a",
        "area_parent_id": "234",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2606",
        "area_name": "\u4e34\u6cad\u53bf",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2607",
        "area_name": "\u5170\u5c71\u533a",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2608",
        "area_name": "\u5e73\u9091\u53bf",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2609",
        "area_name": "\u6c82\u5357\u53bf",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2610",
        "area_name": "\u6c82\u6c34\u53bf",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2611",
        "area_name": "\u6cb3\u4e1c\u533a",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2612",
        "area_name": "\u7f57\u5e84\u533a",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2613",
        "area_name": "\u82cd\u5c71\u53bf",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2614",
        "area_name": "\u8392\u5357\u53bf",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2615",
        "area_name": "\u8499\u9634\u53bf",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2616",
        "area_name": "\u8d39\u53bf",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2617",
        "area_name": "\u90ef\u57ce\u53bf",
        "area_parent_id": "235",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2618",
        "area_name": "\u4e34\u9091\u53bf",
        "area_parent_id": "236",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2619",
        "area_name": "\u4e50\u9675\u5e02",
        "area_parent_id": "236",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2620",
        "area_name": "\u590f\u6d25\u53bf",
        "area_parent_id": "236",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2621",
        "area_name": "\u5b81\u6d25\u53bf",
        "area_parent_id": "236",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2622",
        "area_name": "\u5e73\u539f\u53bf",
        "area_parent_id": "236",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2623",
        "area_name": "\u5e86\u4e91\u53bf",
        "area_parent_id": "236",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2624",
        "area_name": "\u5fb7\u57ce\u533a",
        "area_parent_id": "236",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2625",
        "area_name": "\u6b66\u57ce\u53bf",
        "area_parent_id": "236",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2626",
        "area_name": "\u79b9\u57ce\u5e02",
        "area_parent_id": "236",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2627",
        "area_name": "\u9675\u53bf",
        "area_parent_id": "236",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2628",
        "area_name": "\u9f50\u6cb3\u53bf",
        "area_parent_id": "236",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2629",
        "area_name": "\u4e1c\u660c\u5e9c\u533a",
        "area_parent_id": "237",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2630",
        "area_name": "\u4e1c\u963f\u53bf",
        "area_parent_id": "237",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2631",
        "area_name": "\u4e34\u6e05\u5e02",
        "area_parent_id": "237",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2632",
        "area_name": "\u51a0\u53bf",
        "area_parent_id": "237",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2633",
        "area_name": "\u830c\u5e73\u53bf",
        "area_parent_id": "237",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2634",
        "area_name": "\u8398\u53bf",
        "area_parent_id": "237",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2635",
        "area_name": "\u9633\u8c37\u53bf",
        "area_parent_id": "237",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2636",
        "area_name": "\u9ad8\u5510\u53bf",
        "area_parent_id": "237",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2637",
        "area_name": "\u535a\u5174\u53bf",
        "area_parent_id": "238",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2638",
        "area_name": "\u60e0\u6c11\u53bf",
        "area_parent_id": "238",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2639",
        "area_name": "\u65e0\u68e3\u53bf",
        "area_parent_id": "238",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2640",
        "area_name": "\u6cbe\u5316\u53bf",
        "area_parent_id": "238",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2641",
        "area_name": "\u6ee8\u57ce\u533a",
        "area_parent_id": "238",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2642",
        "area_name": "\u90b9\u5e73\u53bf",
        "area_parent_id": "238",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2643",
        "area_name": "\u9633\u4fe1\u53bf",
        "area_parent_id": "238",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2644",
        "area_name": "\u4e1c\u660e\u53bf",
        "area_parent_id": "239",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2645",
        "area_name": "\u5355\u53bf",
        "area_parent_id": "239",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2646",
        "area_name": "\u5b9a\u9676\u53bf",
        "area_parent_id": "239",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2647",
        "area_name": "\u5de8\u91ce\u53bf",
        "area_parent_id": "239",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2648",
        "area_name": "\u6210\u6b66\u53bf",
        "area_parent_id": "239",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2649",
        "area_name": "\u66f9\u53bf",
        "area_parent_id": "239",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2650",
        "area_name": "\u7261\u4e39\u533a",
        "area_parent_id": "239",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2651",
        "area_name": "\u90d3\u57ce\u53bf",
        "area_parent_id": "239",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2652",
        "area_name": "\u9104\u57ce\u53bf",
        "area_parent_id": "239",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2653",
        "area_name": "\u4e0a\u8857\u533a",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2654",
        "area_name": "\u4e2d\u539f\u533a",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2655",
        "area_name": "\u4e2d\u725f\u53bf",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2656",
        "area_name": "\u4e8c\u4e03\u533a",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2657",
        "area_name": "\u5de9\u4e49\u5e02",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2658",
        "area_name": "\u60e0\u6d4e\u533a",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2659",
        "area_name": "\u65b0\u5bc6\u5e02",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2660",
        "area_name": "\u65b0\u90d1\u5e02",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2661",
        "area_name": "\u767b\u5c01\u5e02",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2662",
        "area_name": "\u7ba1\u57ce\u56de\u65cf\u533a",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2663",
        "area_name": "\u8365\u9633\u5e02",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2664",
        "area_name": "\u91d1\u6c34\u533a",
        "area_parent_id": "240",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2665",
        "area_name": "\u5170\u8003\u53bf",
        "area_parent_id": "241",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2666",
        "area_name": "\u5c09\u6c0f\u53bf",
        "area_parent_id": "241",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2667",
        "area_name": "\u5f00\u5c01\u53bf",
        "area_parent_id": "241",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2668",
        "area_name": "\u675e\u53bf",
        "area_parent_id": "241",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2669",
        "area_name": "\u79b9\u738b\u53f0\u533a",
        "area_parent_id": "241",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2670",
        "area_name": "\u901a\u8bb8\u53bf",
        "area_parent_id": "241",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2671",
        "area_name": "\u91d1\u660e\u533a",
        "area_parent_id": "241",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2672",
        "area_name": "\u987a\u6cb3\u56de\u65cf\u533a",
        "area_parent_id": "241",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2673",
        "area_name": "\u9f13\u697c\u533a",
        "area_parent_id": "241",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2674",
        "area_name": "\u9f99\u4ead\u533a",
        "area_parent_id": "241",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2675",
        "area_name": "\u4f0a\u5ddd\u53bf",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2676",
        "area_name": "\u5043\u5e08\u5e02",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2677",
        "area_name": "\u5409\u5229\u533a",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2678",
        "area_name": "\u5b5f\u6d25\u53bf",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2679",
        "area_name": "\u5b9c\u9633\u53bf",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2680",
        "area_name": "\u5d69\u53bf",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2681",
        "area_name": "\u65b0\u5b89\u53bf",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2682",
        "area_name": "\u683e\u5ddd\u53bf",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2683",
        "area_name": "\u6c5d\u9633\u53bf",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2684",
        "area_name": "\u6d1b\u5b81\u53bf",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2685",
        "area_name": "\u6d1b\u9f99\u533a",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2686",
        "area_name": "\u6da7\u897f\u533a",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2687",
        "area_name": "\u700d\u6cb3\u56de\u65cf\u533a",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2688",
        "area_name": "\u8001\u57ce\u533a",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2689",
        "area_name": "\u897f\u5de5\u533a",
        "area_parent_id": "242",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2690",
        "area_name": "\u536b\u4e1c\u533a",
        "area_parent_id": "243",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2691",
        "area_name": "\u53f6\u53bf",
        "area_parent_id": "243",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2692",
        "area_name": "\u5b9d\u4e30\u53bf",
        "area_parent_id": "243",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2693",
        "area_name": "\u65b0\u534e\u533a",
        "area_parent_id": "243",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2694",
        "area_name": "\u6c5d\u5dde\u5e02",
        "area_parent_id": "243",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2695",
        "area_name": "\u6e5b\u6cb3\u533a",
        "area_parent_id": "243",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2696",
        "area_name": "\u77f3\u9f99\u533a",
        "area_parent_id": "243",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2697",
        "area_name": "\u821e\u94a2\u5e02",
        "area_parent_id": "243",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2698",
        "area_name": "\u90cf\u53bf",
        "area_parent_id": "243",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2699",
        "area_name": "\u9c81\u5c71\u53bf",
        "area_parent_id": "243",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2700",
        "area_name": "\u5185\u9ec4\u53bf",
        "area_parent_id": "244",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2701",
        "area_name": "\u5317\u5173\u533a",
        "area_parent_id": "244",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2702",
        "area_name": "\u5b89\u9633\u53bf",
        "area_parent_id": "244",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2703",
        "area_name": "\u6587\u5cf0\u533a",
        "area_parent_id": "244",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2704",
        "area_name": "\u6797\u5dde\u5e02",
        "area_parent_id": "244",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2705",
        "area_name": "\u6bb7\u90fd\u533a",
        "area_parent_id": "244",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2706",
        "area_name": "\u6c64\u9634\u53bf",
        "area_parent_id": "244",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2707",
        "area_name": "\u6ed1\u53bf",
        "area_parent_id": "244",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2708",
        "area_name": "\u9f99\u5b89\u533a",
        "area_parent_id": "244",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2709",
        "area_name": "\u5c71\u57ce\u533a",
        "area_parent_id": "245",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2710",
        "area_name": "\u6d5a\u53bf",
        "area_parent_id": "245",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2711",
        "area_name": "\u6dc7\u53bf",
        "area_parent_id": "245",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2712",
        "area_name": "\u6dc7\u6ee8\u533a",
        "area_parent_id": "245",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2713",
        "area_name": "\u9e64\u5c71\u533a",
        "area_parent_id": "245",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2714",
        "area_name": "\u51e4\u6cc9\u533a",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2715",
        "area_name": "\u536b\u6ee8\u533a",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2716",
        "area_name": "\u536b\u8f89\u5e02",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2717",
        "area_name": "\u539f\u9633\u53bf",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2718",
        "area_name": "\u5c01\u4e18\u53bf",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2719",
        "area_name": "\u5ef6\u6d25\u53bf",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2720",
        "area_name": "\u65b0\u4e61\u53bf",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2721",
        "area_name": "\u7267\u91ce\u533a",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2722",
        "area_name": "\u7ea2\u65d7\u533a",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2723",
        "area_name": "\u83b7\u5609\u53bf",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2724",
        "area_name": "\u8f89\u53bf\u5e02",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2725",
        "area_name": "\u957f\u57a3\u53bf",
        "area_parent_id": "246",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2726",
        "area_name": "\u4e2d\u7ad9\u533a",
        "area_parent_id": "247",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2727",
        "area_name": "\u4fee\u6b66\u53bf",
        "area_parent_id": "247",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2728",
        "area_name": "\u535a\u7231\u53bf",
        "area_parent_id": "247",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2729",
        "area_name": "\u5b5f\u5dde\u5e02",
        "area_parent_id": "247",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2730",
        "area_name": "\u5c71\u9633\u533a",
        "area_parent_id": "247",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2731",
        "area_name": "\u6b66\u965f\u53bf",
        "area_parent_id": "247",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2732",
        "area_name": "\u6c81\u9633\u5e02",
        "area_parent_id": "247",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2733",
        "area_name": "\u6e29\u53bf",
        "area_parent_id": "247",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2734",
        "area_name": "\u89e3\u653e\u533a",
        "area_parent_id": "247",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2735",
        "area_name": "\u9a6c\u6751\u533a",
        "area_parent_id": "247",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2736",
        "area_name": "\u534e\u9f99\u533a",
        "area_parent_id": "248",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2737",
        "area_name": "\u5357\u4e50\u53bf",
        "area_parent_id": "248",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2738",
        "area_name": "\u53f0\u524d\u53bf",
        "area_parent_id": "248",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2739",
        "area_name": "\u6e05\u4e30\u53bf",
        "area_parent_id": "248",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2740",
        "area_name": "\u6fee\u9633\u53bf",
        "area_parent_id": "248",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2741",
        "area_name": "\u8303\u53bf",
        "area_parent_id": "248",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2742",
        "area_name": "\u79b9\u5dde\u5e02",
        "area_parent_id": "249",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2743",
        "area_name": "\u8944\u57ce\u53bf",
        "area_parent_id": "249",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2744",
        "area_name": "\u8bb8\u660c\u53bf",
        "area_parent_id": "249",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2745",
        "area_name": "\u9122\u9675\u53bf",
        "area_parent_id": "249",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2746",
        "area_name": "\u957f\u845b\u5e02",
        "area_parent_id": "249",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2747",
        "area_name": "\u9b4f\u90fd\u533a",
        "area_parent_id": "249",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2748",
        "area_name": "\u4e34\u988d\u53bf",
        "area_parent_id": "250",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2749",
        "area_name": "\u53ec\u9675\u533a",
        "area_parent_id": "250",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2750",
        "area_name": "\u6e90\u6c47\u533a",
        "area_parent_id": "250",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2751",
        "area_name": "\u821e\u9633\u53bf",
        "area_parent_id": "250",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2752",
        "area_name": "\u90fe\u57ce\u533a",
        "area_parent_id": "250",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2753",
        "area_name": "\u4e49\u9a6c\u5e02",
        "area_parent_id": "251",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2754",
        "area_name": "\u5362\u6c0f\u53bf",
        "area_parent_id": "251",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2755",
        "area_name": "\u6e11\u6c60\u53bf",
        "area_parent_id": "251",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2756",
        "area_name": "\u6e56\u6ee8\u533a",
        "area_parent_id": "251",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2757",
        "area_name": "\u7075\u5b9d\u5e02",
        "area_parent_id": "251",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2758",
        "area_name": "\u9655\u53bf",
        "area_parent_id": "251",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2759",
        "area_name": "\u5185\u4e61\u53bf",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2760",
        "area_name": "\u5357\u53ec\u53bf",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2761",
        "area_name": "\u5367\u9f99\u533a",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2762",
        "area_name": "\u5510\u6cb3\u53bf",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2763",
        "area_name": "\u5b9b\u57ce\u533a",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2764",
        "area_name": "\u65b0\u91ce\u53bf",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2765",
        "area_name": "\u65b9\u57ce\u53bf",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2766",
        "area_name": "\u6850\u67cf\u53bf",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2767",
        "area_name": "\u6dc5\u5ddd\u53bf",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2768",
        "area_name": "\u793e\u65d7\u53bf",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2769",
        "area_name": "\u897f\u5ce1\u53bf",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2770",
        "area_name": "\u9093\u5dde\u5e02",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2771",
        "area_name": "\u9547\u5e73\u53bf",
        "area_parent_id": "252",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2772",
        "area_name": "\u590f\u9091\u53bf",
        "area_parent_id": "253",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2773",
        "area_name": "\u5b81\u9675\u53bf",
        "area_parent_id": "253",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2774",
        "area_name": "\u67d8\u57ce\u53bf",
        "area_parent_id": "253",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2775",
        "area_name": "\u6c11\u6743\u53bf",
        "area_parent_id": "253",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2776",
        "area_name": "\u6c38\u57ce\u5e02",
        "area_parent_id": "253",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2777",
        "area_name": "\u7762\u53bf",
        "area_parent_id": "253",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2778",
        "area_name": "\u7762\u9633\u533a",
        "area_parent_id": "253",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2779",
        "area_name": "\u7cb1\u56ed\u533a",
        "area_parent_id": "253",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2780",
        "area_name": "\u865e\u57ce\u53bf",
        "area_parent_id": "253",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2781",
        "area_name": "\u5149\u5c71\u53bf",
        "area_parent_id": "254",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2782",
        "area_name": "\u5546\u57ce\u53bf",
        "area_parent_id": "254",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2783",
        "area_name": "\u56fa\u59cb\u53bf",
        "area_parent_id": "254",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2784",
        "area_name": "\u5e73\u6865\u533a",
        "area_parent_id": "254",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2785",
        "area_name": "\u606f\u53bf",
        "area_parent_id": "254",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2786",
        "area_name": "\u65b0\u53bf",
        "area_parent_id": "254",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2787",
        "area_name": "\u6d49\u6cb3\u533a",
        "area_parent_id": "254",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2788",
        "area_name": "\u6dee\u6ee8\u53bf",
        "area_parent_id": "254",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2789",
        "area_name": "\u6f62\u5ddd\u53bf",
        "area_parent_id": "254",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2790",
        "area_name": "\u7f57\u5c71\u53bf",
        "area_parent_id": "254",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2791",
        "area_name": "\u5546\u6c34\u53bf",
        "area_parent_id": "255",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2792",
        "area_name": "\u592a\u5eb7\u53bf",
        "area_parent_id": "255",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2793",
        "area_name": "\u5ddd\u6c47\u533a",
        "area_parent_id": "255",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2794",
        "area_name": "\u6276\u6c9f\u53bf",
        "area_parent_id": "255",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2795",
        "area_name": "\u6c88\u4e18\u53bf",
        "area_parent_id": "255",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2796",
        "area_name": "\u6dee\u9633\u53bf",
        "area_parent_id": "255",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2797",
        "area_name": "\u897f\u534e\u53bf",
        "area_parent_id": "255",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2798",
        "area_name": "\u90f8\u57ce\u53bf",
        "area_parent_id": "255",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2799",
        "area_name": "\u9879\u57ce\u5e02",
        "area_parent_id": "255",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2800",
        "area_name": "\u9e7f\u9091\u53bf",
        "area_parent_id": "255",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2801",
        "area_name": "\u4e0a\u8521\u53bf",
        "area_parent_id": "256",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2802",
        "area_name": "\u5e73\u8206\u53bf",
        "area_parent_id": "256",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2803",
        "area_name": "\u65b0\u8521\u53bf",
        "area_parent_id": "256",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2804",
        "area_name": "\u6b63\u9633\u53bf",
        "area_parent_id": "256",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2805",
        "area_name": "\u6c5d\u5357\u53bf",
        "area_parent_id": "256",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2806",
        "area_name": "\u6ccc\u9633\u53bf",
        "area_parent_id": "256",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2807",
        "area_name": "\u786e\u5c71\u53bf",
        "area_parent_id": "256",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2808",
        "area_name": "\u897f\u5e73\u53bf",
        "area_parent_id": "256",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2809",
        "area_name": "\u9042\u5e73\u53bf",
        "area_parent_id": "256",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2810",
        "area_name": "\u9a7f\u57ce\u533a",
        "area_parent_id": "256",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2811",
        "area_name": "\u6d4e\u6e90\u5e02",
        "area_parent_id": "257",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2812",
        "area_name": "\u4e1c\u897f\u6e56\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2813",
        "area_name": "\u65b0\u6d32\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2814",
        "area_name": "\u6b66\u660c\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2815",
        "area_name": "\u6c49\u5357\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2816",
        "area_name": "\u6c49\u9633\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2817",
        "area_name": "\u6c5f\u590f\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2818",
        "area_name": "\u6c5f\u5cb8\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2819",
        "area_name": "\u6c5f\u6c49\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2820",
        "area_name": "\u6d2a\u5c71\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2821",
        "area_name": "\u785a\u53e3\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2822",
        "area_name": "\u8521\u7538\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2823",
        "area_name": "\u9752\u5c71\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2824",
        "area_name": "\u9ec4\u9642\u533a",
        "area_parent_id": "258",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2825",
        "area_name": "\u4e0b\u9646\u533a",
        "area_parent_id": "259",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2826",
        "area_name": "\u5927\u51b6\u5e02",
        "area_parent_id": "259",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2827",
        "area_name": "\u897f\u585e\u5c71\u533a",
        "area_parent_id": "259",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2828",
        "area_name": "\u94c1\u5c71\u533a",
        "area_parent_id": "259",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2829",
        "area_name": "\u9633\u65b0\u53bf",
        "area_parent_id": "259",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2830",
        "area_name": "\u9ec4\u77f3\u6e2f\u533a",
        "area_parent_id": "259",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2831",
        "area_name": "\u4e39\u6c5f\u53e3\u5e02",
        "area_parent_id": "260",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2832",
        "area_name": "\u5f20\u6e7e\u533a",
        "area_parent_id": "260",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2833",
        "area_name": "\u623f\u53bf",
        "area_parent_id": "260",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2834",
        "area_name": "\u7af9\u5c71\u53bf",
        "area_parent_id": "260",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2835",
        "area_name": "\u7af9\u6eaa\u53bf",
        "area_parent_id": "260",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2836",
        "area_name": "\u8305\u7bad\u533a",
        "area_parent_id": "260",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2837",
        "area_name": "\u90e7\u53bf",
        "area_parent_id": "260",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2838",
        "area_name": "\u90e7\u897f\u53bf",
        "area_parent_id": "260",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2839",
        "area_name": "\u4e94\u5cf0\u571f\u5bb6\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2840",
        "area_name": "\u4f0d\u5bb6\u5c97\u533a",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2841",
        "area_name": "\u5174\u5c71\u53bf",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2842",
        "area_name": "\u5937\u9675\u533a",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2843",
        "area_name": "\u5b9c\u90fd\u5e02",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2844",
        "area_name": "\u5f53\u9633\u5e02",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2845",
        "area_name": "\u679d\u6c5f\u5e02",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2846",
        "area_name": "\u70b9\u519b\u533a",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2847",
        "area_name": "\u79ed\u5f52\u53bf",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2848",
        "area_name": "\u8662\u4ead\u533a",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2849",
        "area_name": "\u897f\u9675\u533a",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2850",
        "area_name": "\u8fdc\u5b89\u53bf",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2851",
        "area_name": "\u957f\u9633\u571f\u5bb6\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "261",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2852",
        "area_name": "\u4fdd\u5eb7\u53bf",
        "area_parent_id": "262",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2853",
        "area_name": "\u5357\u6f33\u53bf",
        "area_parent_id": "262",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2854",
        "area_name": "\u5b9c\u57ce\u5e02",
        "area_parent_id": "262",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2855",
        "area_name": "\u67a3\u9633\u5e02",
        "area_parent_id": "262",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2856",
        "area_name": "\u6a0a\u57ce\u533a",
        "area_parent_id": "262",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2857",
        "area_name": "\u8001\u6cb3\u53e3\u5e02",
        "area_parent_id": "262",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2858",
        "area_name": "\u8944\u57ce\u533a",
        "area_parent_id": "262",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2859",
        "area_name": "\u8944\u9633\u533a",
        "area_parent_id": "262",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2860",
        "area_name": "\u8c37\u57ce\u53bf",
        "area_parent_id": "262",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2861",
        "area_name": "\u534e\u5bb9\u533a",
        "area_parent_id": "263",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2862",
        "area_name": "\u7cb1\u5b50\u6e56",
        "area_parent_id": "263",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2863",
        "area_name": "\u9102\u57ce\u533a",
        "area_parent_id": "263",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2864",
        "area_name": "\u4e1c\u5b9d\u533a",
        "area_parent_id": "264",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2865",
        "area_name": "\u4eac\u5c71\u53bf",
        "area_parent_id": "264",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2866",
        "area_name": "\u6387\u5200\u533a",
        "area_parent_id": "264",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2867",
        "area_name": "\u6c99\u6d0b\u53bf",
        "area_parent_id": "264",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2868",
        "area_name": "\u949f\u7965\u5e02",
        "area_parent_id": "264",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2869",
        "area_name": "\u4e91\u68a6\u53bf",
        "area_parent_id": "265",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2870",
        "area_name": "\u5927\u609f\u53bf",
        "area_parent_id": "265",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2871",
        "area_name": "\u5b5d\u5357\u533a",
        "area_parent_id": "265",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2872",
        "area_name": "\u5b5d\u660c\u53bf",
        "area_parent_id": "265",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2873",
        "area_name": "\u5b89\u9646\u5e02",
        "area_parent_id": "265",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2874",
        "area_name": "\u5e94\u57ce\u5e02",
        "area_parent_id": "265",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2875",
        "area_name": "\u6c49\u5ddd\u5e02",
        "area_parent_id": "265",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2876",
        "area_name": "\u516c\u5b89\u53bf",
        "area_parent_id": "266",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2877",
        "area_name": "\u677e\u6ecb\u5e02",
        "area_parent_id": "266",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2878",
        "area_name": "\u6c5f\u9675\u53bf",
        "area_parent_id": "266",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2879",
        "area_name": "\u6c99\u5e02\u533a",
        "area_parent_id": "266",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2880",
        "area_name": "\u6d2a\u6e56\u5e02",
        "area_parent_id": "266",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2881",
        "area_name": "\u76d1\u5229\u53bf",
        "area_parent_id": "266",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2882",
        "area_name": "\u77f3\u9996\u5e02",
        "area_parent_id": "266",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2883",
        "area_name": "\u8346\u5dde\u533a",
        "area_parent_id": "266",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2884",
        "area_name": "\u56e2\u98ce\u53bf",
        "area_parent_id": "267",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2885",
        "area_name": "\u6b66\u7a74\u5e02",
        "area_parent_id": "267",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2886",
        "area_name": "\u6d60\u6c34\u53bf",
        "area_parent_id": "267",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2887",
        "area_name": "\u7ea2\u5b89\u53bf",
        "area_parent_id": "267",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2888",
        "area_name": "\u7f57\u7530\u53bf",
        "area_parent_id": "267",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2889",
        "area_name": "\u82f1\u5c71\u53bf",
        "area_parent_id": "267",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2890",
        "area_name": "\u8572\u6625\u53bf",
        "area_parent_id": "267",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2891",
        "area_name": "\u9ebb\u57ce\u5e02",
        "area_parent_id": "267",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2892",
        "area_name": "\u9ec4\u5dde\u533a",
        "area_parent_id": "267",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2893",
        "area_name": "\u9ec4\u6885\u53bf",
        "area_parent_id": "267",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2894",
        "area_name": "\u54b8\u5b89\u533a",
        "area_parent_id": "268",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2895",
        "area_name": "\u5609\u9c7c\u53bf",
        "area_parent_id": "268",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2896",
        "area_name": "\u5d07\u9633\u53bf",
        "area_parent_id": "268",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2897",
        "area_name": "\u8d64\u58c1\u5e02",
        "area_parent_id": "268",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2898",
        "area_name": "\u901a\u57ce\u53bf",
        "area_parent_id": "268",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2899",
        "area_name": "\u901a\u5c71\u53bf",
        "area_parent_id": "268",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2900",
        "area_name": "\u5e7f\u6c34\u5e02",
        "area_parent_id": "269",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2901",
        "area_name": "\u66fe\u90fd\u533a",
        "area_parent_id": "269",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2902",
        "area_name": "\u5229\u5ddd\u5e02",
        "area_parent_id": "270",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2903",
        "area_name": "\u54b8\u4e30\u53bf",
        "area_parent_id": "270",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2904",
        "area_name": "\u5ba3\u6069\u53bf",
        "area_parent_id": "270",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2905",
        "area_name": "\u5df4\u4e1c\u53bf",
        "area_parent_id": "270",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2906",
        "area_name": "\u5efa\u59cb\u53bf",
        "area_parent_id": "270",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2907",
        "area_name": "\u6069\u65bd\u5e02",
        "area_parent_id": "270",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2908",
        "area_name": "\u6765\u51e4\u53bf",
        "area_parent_id": "270",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2909",
        "area_name": "\u9e64\u5cf0\u53bf",
        "area_parent_id": "270",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2910",
        "area_name": "\u4ed9\u6843\u5e02",
        "area_parent_id": "271",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2911",
        "area_name": "\u6f5c\u6c5f\u5e02",
        "area_parent_id": "272",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2912",
        "area_name": "\u5929\u95e8\u5e02",
        "area_parent_id": "273",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2913",
        "area_name": "\u795e\u519c\u67b6\u6797\u533a",
        "area_parent_id": "274",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2914",
        "area_name": "\u5929\u5fc3\u533a",
        "area_parent_id": "275",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2915",
        "area_name": "\u5b81\u4e61\u53bf",
        "area_parent_id": "275",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2916",
        "area_name": "\u5cb3\u9e93\u533a",
        "area_parent_id": "275",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2917",
        "area_name": "\u5f00\u798f\u533a",
        "area_parent_id": "275",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2918",
        "area_name": "\u671b\u57ce\u53bf",
        "area_parent_id": "275",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2919",
        "area_name": "\u6d4f\u9633\u5e02",
        "area_parent_id": "275",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2920",
        "area_name": "\u8299\u84c9\u533a",
        "area_parent_id": "275",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2921",
        "area_name": "\u957f\u6c99\u53bf",
        "area_parent_id": "275",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2922",
        "area_name": "\u96e8\u82b1\u533a",
        "area_parent_id": "275",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2923",
        "area_name": "\u5929\u5143\u533a",
        "area_parent_id": "276",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2924",
        "area_name": "\u6538\u53bf",
        "area_parent_id": "276",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2925",
        "area_name": "\u682a\u6d32\u53bf",
        "area_parent_id": "276",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2926",
        "area_name": "\u708e\u9675\u53bf",
        "area_parent_id": "276",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2927",
        "area_name": "\u77f3\u5cf0\u533a",
        "area_parent_id": "276",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2928",
        "area_name": "\u82a6\u6dde\u533a",
        "area_parent_id": "276",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2929",
        "area_name": "\u8336\u9675\u53bf",
        "area_parent_id": "276",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2930",
        "area_name": "\u8377\u5858\u533a",
        "area_parent_id": "276",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2931",
        "area_name": "\u91b4\u9675\u5e02",
        "area_parent_id": "276",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2932",
        "area_name": "\u5cb3\u5858\u533a",
        "area_parent_id": "277",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2933",
        "area_name": "\u6e58\u4e61\u5e02",
        "area_parent_id": "277",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2934",
        "area_name": "\u6e58\u6f6d\u53bf",
        "area_parent_id": "277",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2935",
        "area_name": "\u96e8\u6e56\u533a",
        "area_parent_id": "277",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2936",
        "area_name": "\u97f6\u5c71\u5e02",
        "area_parent_id": "277",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2937",
        "area_name": "\u5357\u5cb3\u533a",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2938",
        "area_name": "\u5e38\u5b81\u5e02",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2939",
        "area_name": "\u73e0\u6656\u533a",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2940",
        "area_name": "\u77f3\u9f13\u533a",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2941",
        "area_name": "\u7941\u4e1c\u53bf",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2942",
        "area_name": "\u8012\u9633\u5e02",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2943",
        "area_name": "\u84b8\u6e58\u533a",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2944",
        "area_name": "\u8861\u4e1c\u53bf",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2945",
        "area_name": "\u8861\u5357\u53bf",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2946",
        "area_name": "\u8861\u5c71\u53bf",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2947",
        "area_name": "\u8861\u9633\u53bf",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2948",
        "area_name": "\u96c1\u5cf0\u533a",
        "area_parent_id": "278",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2949",
        "area_name": "\u5317\u5854\u533a",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2950",
        "area_name": "\u53cc\u6e05\u533a",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2951",
        "area_name": "\u57ce\u6b65\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2952",
        "area_name": "\u5927\u7965\u533a",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2953",
        "area_name": "\u65b0\u5b81\u53bf",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2954",
        "area_name": "\u65b0\u90b5\u53bf",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2955",
        "area_name": "\u6b66\u5188\u5e02",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2956",
        "area_name": "\u6d1e\u53e3\u53bf",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2957",
        "area_name": "\u7ee5\u5b81\u53bf",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2958",
        "area_name": "\u90b5\u4e1c\u53bf",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2959",
        "area_name": "\u90b5\u9633\u53bf",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2960",
        "area_name": "\u9686\u56de\u53bf",
        "area_parent_id": "279",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2961",
        "area_name": "\u4e34\u6e58\u5e02",
        "area_parent_id": "280",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2962",
        "area_name": "\u4e91\u6eaa\u533a",
        "area_parent_id": "280",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2963",
        "area_name": "\u534e\u5bb9\u53bf",
        "area_parent_id": "280",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2964",
        "area_name": "\u541b\u5c71\u533a",
        "area_parent_id": "280",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2965",
        "area_name": "\u5cb3\u9633\u53bf",
        "area_parent_id": "280",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2966",
        "area_name": "\u5cb3\u9633\u697c\u533a",
        "area_parent_id": "280",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2967",
        "area_name": "\u5e73\u6c5f\u53bf",
        "area_parent_id": "280",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2968",
        "area_name": "\u6c68\u7f57\u5e02",
        "area_parent_id": "280",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2969",
        "area_name": "\u6e58\u9634\u53bf",
        "area_parent_id": "280",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2970",
        "area_name": "\u4e34\u6fa7\u53bf",
        "area_parent_id": "281",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2971",
        "area_name": "\u5b89\u4e61\u53bf",
        "area_parent_id": "281",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2972",
        "area_name": "\u6843\u6e90\u53bf",
        "area_parent_id": "281",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2973",
        "area_name": "\u6b66\u9675\u533a",
        "area_parent_id": "281",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2974",
        "area_name": "\u6c49\u5bff\u53bf",
        "area_parent_id": "281",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2975",
        "area_name": "\u6d25\u5e02\u5e02",
        "area_parent_id": "281",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2976",
        "area_name": "\u6fa7\u53bf",
        "area_parent_id": "281",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2977",
        "area_name": "\u77f3\u95e8\u53bf",
        "area_parent_id": "281",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2978",
        "area_name": "\u9f0e\u57ce\u533a",
        "area_parent_id": "281",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2979",
        "area_name": "\u6148\u5229\u53bf",
        "area_parent_id": "282",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2980",
        "area_name": "\u6851\u690d\u53bf",
        "area_parent_id": "282",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2981",
        "area_name": "\u6b66\u9675\u6e90\u533a",
        "area_parent_id": "282",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2982",
        "area_name": "\u6c38\u5b9a\u533a",
        "area_parent_id": "282",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2983",
        "area_name": "\u5357\u53bf",
        "area_parent_id": "283",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2984",
        "area_name": "\u5b89\u5316\u53bf",
        "area_parent_id": "283",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2985",
        "area_name": "\u6843\u6c5f\u53bf",
        "area_parent_id": "283",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2986",
        "area_name": "\u6c85\u6c5f\u5e02",
        "area_parent_id": "283",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2987",
        "area_name": "\u8d44\u9633\u533a",
        "area_parent_id": "283",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2988",
        "area_name": "\u8d6b\u5c71\u533a",
        "area_parent_id": "283",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2989",
        "area_name": "\u4e34\u6b66\u53bf",
        "area_parent_id": "284",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2990",
        "area_name": "\u5317\u6e56\u533a",
        "area_parent_id": "284",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2991",
        "area_name": "\u5609\u79be\u53bf",
        "area_parent_id": "284",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2992",
        "area_name": "\u5b89\u4ec1\u53bf",
        "area_parent_id": "284",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2993",
        "area_name": "\u5b9c\u7ae0\u53bf",
        "area_parent_id": "284",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2994",
        "area_name": "\u6842\u4e1c\u53bf",
        "area_parent_id": "284",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2995",
        "area_name": "\u6842\u9633\u53bf",
        "area_parent_id": "284",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2996",
        "area_name": "\u6c38\u5174\u53bf",
        "area_parent_id": "284",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2997",
        "area_name": "\u6c5d\u57ce\u53bf",
        "area_parent_id": "284",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2998",
        "area_name": "\u82cf\u4ed9\u533a",
        "area_parent_id": "284",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "2999",
        "area_name": "\u8d44\u5174\u5e02",
        "area_parent_id": "284",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3000",
        "area_name": "\u4e1c\u5b89\u53bf",
        "area_parent_id": "285",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3001",
        "area_name": "\u51b7\u6c34\u6ee9\u533a",
        "area_parent_id": "285",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3002",
        "area_name": "\u53cc\u724c\u53bf",
        "area_parent_id": "285",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3003",
        "area_name": "\u5b81\u8fdc\u53bf",
        "area_parent_id": "285",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3004",
        "area_name": "\u65b0\u7530\u53bf",
        "area_parent_id": "285",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3005",
        "area_name": "\u6c5f\u534e\u7476\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "285",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3006",
        "area_name": "\u6c5f\u6c38\u53bf",
        "area_parent_id": "285",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3007",
        "area_name": "\u7941\u9633\u53bf",
        "area_parent_id": "285",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3008",
        "area_name": "\u84dd\u5c71\u53bf",
        "area_parent_id": "285",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3009",
        "area_name": "\u9053\u53bf",
        "area_parent_id": "285",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3010",
        "area_name": "\u96f6\u9675\u533a",
        "area_parent_id": "285",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3011",
        "area_name": "\u4e2d\u65b9\u53bf",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3012",
        "area_name": "\u4f1a\u540c\u53bf",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3013",
        "area_name": "\u65b0\u6643\u4f97\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3014",
        "area_name": "\u6c85\u9675\u53bf",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3015",
        "area_name": "\u6d2a\u6c5f\u5e02\/\u6d2a\u6c5f\u533a",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3016",
        "area_name": "\u6e86\u6d66\u53bf",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3017",
        "area_name": "\u82b7\u6c5f\u4f97\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3018",
        "area_name": "\u8fb0\u6eaa\u53bf",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3019",
        "area_name": "\u901a\u9053\u4f97\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3020",
        "area_name": "\u9756\u5dde\u82d7\u65cf\u4f97\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3021",
        "area_name": "\u9e64\u57ce\u533a",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3022",
        "area_name": "\u9ebb\u9633\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "286",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3023",
        "area_name": "\u51b7\u6c34\u6c5f\u5e02",
        "area_parent_id": "287",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3024",
        "area_name": "\u53cc\u5cf0\u53bf",
        "area_parent_id": "287",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3025",
        "area_name": "\u5a04\u661f\u533a",
        "area_parent_id": "287",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3026",
        "area_name": "\u65b0\u5316\u53bf",
        "area_parent_id": "287",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3027",
        "area_name": "\u6d9f\u6e90\u5e02",
        "area_parent_id": "287",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3028",
        "area_name": "\u4fdd\u9756\u53bf",
        "area_parent_id": "288",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3029",
        "area_name": "\u51e4\u51f0\u53bf",
        "area_parent_id": "288",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3030",
        "area_name": "\u53e4\u4e08\u53bf",
        "area_parent_id": "288",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3031",
        "area_name": "\u5409\u9996\u5e02",
        "area_parent_id": "288",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3032",
        "area_name": "\u6c38\u987a\u53bf",
        "area_parent_id": "288",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3033",
        "area_name": "\u6cf8\u6eaa\u53bf",
        "area_parent_id": "288",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3034",
        "area_name": "\u82b1\u57a3\u53bf",
        "area_parent_id": "288",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3035",
        "area_name": "\u9f99\u5c71\u53bf",
        "area_parent_id": "288",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3036",
        "area_name": "\u841d\u5c97\u533a",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3037",
        "area_name": "\u5357\u6c99\u533a",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3038",
        "area_name": "\u4ece\u5316\u5e02",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3039",
        "area_name": "\u589e\u57ce\u5e02",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3040",
        "area_name": "\u5929\u6cb3\u533a",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3041",
        "area_name": "\u6d77\u73e0\u533a",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3042",
        "area_name": "\u756a\u79ba\u533a",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3043",
        "area_name": "\u767d\u4e91\u533a",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3044",
        "area_name": "\u82b1\u90fd\u533a",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3045",
        "area_name": "\u8354\u6e7e\u533a",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3046",
        "area_name": "\u8d8a\u79c0\u533a",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3047",
        "area_name": "\u9ec4\u57d4\u533a",
        "area_parent_id": "289",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3048",
        "area_name": "\u4e50\u660c\u5e02",
        "area_parent_id": "290",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3049",
        "area_name": "\u4e73\u6e90\u7476\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "290",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3050",
        "area_name": "\u4ec1\u5316\u53bf",
        "area_parent_id": "290",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3051",
        "area_name": "\u5357\u96c4\u5e02",
        "area_parent_id": "290",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3052",
        "area_name": "\u59cb\u5174\u53bf",
        "area_parent_id": "290",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3053",
        "area_name": "\u65b0\u4e30\u53bf",
        "area_parent_id": "290",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3054",
        "area_name": "\u66f2\u6c5f\u533a",
        "area_parent_id": "290",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3055",
        "area_name": "\u6b66\u6c5f\u533a",
        "area_parent_id": "290",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3056",
        "area_name": "\u6d48\u6c5f\u533a",
        "area_parent_id": "290",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3057",
        "area_name": "\u7fc1\u6e90\u53bf",
        "area_parent_id": "290",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3058",
        "area_name": "\u5357\u5c71\u533a",
        "area_parent_id": "291",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3059",
        "area_name": "\u5b9d\u5b89\u533a",
        "area_parent_id": "291",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3060",
        "area_name": "\u76d0\u7530\u533a",
        "area_parent_id": "291",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3061",
        "area_name": "\u798f\u7530\u533a",
        "area_parent_id": "291",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3062",
        "area_name": "\u7f57\u6e56\u533a",
        "area_parent_id": "291",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3063",
        "area_name": "\u9f99\u5c97\u533a",
        "area_parent_id": "291",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3064",
        "area_name": "\u6597\u95e8\u533a",
        "area_parent_id": "292",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3065",
        "area_name": "\u91d1\u6e7e\u533a",
        "area_parent_id": "292",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3066",
        "area_name": "\u9999\u6d32\u533a",
        "area_parent_id": "292",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3067",
        "area_name": "\u5357\u6fb3\u53bf",
        "area_parent_id": "293",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3068",
        "area_name": "\u6f6e\u5357\u533a",
        "area_parent_id": "293",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3069",
        "area_name": "\u6f6e\u9633\u533a",
        "area_parent_id": "293",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3070",
        "area_name": "\u6f84\u6d77\u533a",
        "area_parent_id": "293",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3071",
        "area_name": "\u6fe0\u6c5f\u533a",
        "area_parent_id": "293",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3072",
        "area_name": "\u91d1\u5e73\u533a",
        "area_parent_id": "293",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3073",
        "area_name": "\u9f99\u6e56\u533a",
        "area_parent_id": "293",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3074",
        "area_name": "\u4e09\u6c34\u533a",
        "area_parent_id": "294",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3075",
        "area_name": "\u5357\u6d77\u533a",
        "area_parent_id": "294",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3076",
        "area_name": "\u7985\u57ce\u533a",
        "area_parent_id": "294",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3077",
        "area_name": "\u987a\u5fb7\u533a",
        "area_parent_id": "294",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3078",
        "area_name": "\u9ad8\u660e\u533a",
        "area_parent_id": "294",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3079",
        "area_name": "\u53f0\u5c71\u5e02",
        "area_parent_id": "295",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3080",
        "area_name": "\u5f00\u5e73\u5e02",
        "area_parent_id": "295",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3081",
        "area_name": "\u6069\u5e73\u5e02",
        "area_parent_id": "295",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3082",
        "area_name": "\u65b0\u4f1a\u533a",
        "area_parent_id": "295",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3083",
        "area_name": "\u6c5f\u6d77\u533a",
        "area_parent_id": "295",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3084",
        "area_name": "\u84ec\u6c5f\u533a",
        "area_parent_id": "295",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3085",
        "area_name": "\u9e64\u5c71\u5e02",
        "area_parent_id": "295",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3086",
        "area_name": "\u5434\u5ddd\u5e02",
        "area_parent_id": "296",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3087",
        "area_name": "\u5761\u5934\u533a",
        "area_parent_id": "296",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3088",
        "area_name": "\u5ec9\u6c5f\u5e02",
        "area_parent_id": "296",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3089",
        "area_name": "\u5f90\u95fb\u53bf",
        "area_parent_id": "296",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3090",
        "area_name": "\u8d64\u574e\u533a",
        "area_parent_id": "296",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3091",
        "area_name": "\u9042\u6eaa\u53bf",
        "area_parent_id": "296",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3092",
        "area_name": "\u96f7\u5dde\u5e02",
        "area_parent_id": "296",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3093",
        "area_name": "\u971e\u5c71\u533a",
        "area_parent_id": "296",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3094",
        "area_name": "\u9ebb\u7ae0\u533a",
        "area_parent_id": "296",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3095",
        "area_name": "\u4fe1\u5b9c\u5e02",
        "area_parent_id": "297",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3096",
        "area_name": "\u5316\u5dde\u5e02",
        "area_parent_id": "297",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3097",
        "area_name": "\u7535\u767d\u53bf",
        "area_parent_id": "297",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3098",
        "area_name": "\u8302\u5357\u533a",
        "area_parent_id": "297",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3099",
        "area_name": "\u8302\u6e2f\u533a",
        "area_parent_id": "297",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3100",
        "area_name": "\u9ad8\u5dde\u5e02",
        "area_parent_id": "297",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3101",
        "area_name": "\u56db\u4f1a\u5e02",
        "area_parent_id": "298",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3102",
        "area_name": "\u5c01\u5f00\u53bf",
        "area_parent_id": "298",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3103",
        "area_name": "\u5e7f\u5b81\u53bf",
        "area_parent_id": "298",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3104",
        "area_name": "\u5fb7\u5e86\u53bf",
        "area_parent_id": "298",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3105",
        "area_name": "\u6000\u96c6\u53bf",
        "area_parent_id": "298",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3106",
        "area_name": "\u7aef\u5dde\u533a",
        "area_parent_id": "298",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3107",
        "area_name": "\u9ad8\u8981\u5e02",
        "area_parent_id": "298",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3108",
        "area_name": "\u9f0e\u6e56\u533a",
        "area_parent_id": "298",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3109",
        "area_name": "\u535a\u7f57\u53bf",
        "area_parent_id": "299",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3110",
        "area_name": "\u60e0\u4e1c\u53bf",
        "area_parent_id": "299",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3111",
        "area_name": "\u60e0\u57ce\u533a",
        "area_parent_id": "299",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3112",
        "area_name": "\u60e0\u9633\u533a",
        "area_parent_id": "299",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3113",
        "area_name": "\u9f99\u95e8\u53bf",
        "area_parent_id": "299",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3114",
        "area_name": "\u4e30\u987a\u53bf",
        "area_parent_id": "300",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3115",
        "area_name": "\u4e94\u534e\u53bf",
        "area_parent_id": "300",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3116",
        "area_name": "\u5174\u5b81\u5e02",
        "area_parent_id": "300",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3117",
        "area_name": "\u5927\u57d4\u53bf",
        "area_parent_id": "300",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3118",
        "area_name": "\u5e73\u8fdc\u53bf",
        "area_parent_id": "300",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3119",
        "area_name": "\u6885\u53bf",
        "area_parent_id": "300",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3120",
        "area_name": "\u6885\u6c5f\u533a",
        "area_parent_id": "300",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3121",
        "area_name": "\u8549\u5cad\u53bf",
        "area_parent_id": "300",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3122",
        "area_name": "\u57ce\u533a",
        "area_parent_id": "301",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3123",
        "area_name": "\u6d77\u4e30\u53bf",
        "area_parent_id": "301",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3124",
        "area_name": "\u9646\u4e30\u5e02",
        "area_parent_id": "301",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3125",
        "area_name": "\u9646\u6cb3\u53bf",
        "area_parent_id": "301",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3126",
        "area_name": "\u4e1c\u6e90\u53bf",
        "area_parent_id": "302",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3127",
        "area_name": "\u548c\u5e73\u53bf",
        "area_parent_id": "302",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3128",
        "area_name": "\u6e90\u57ce\u533a",
        "area_parent_id": "302",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3129",
        "area_name": "\u7d2b\u91d1\u53bf",
        "area_parent_id": "302",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3130",
        "area_name": "\u8fde\u5e73\u53bf",
        "area_parent_id": "302",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3131",
        "area_name": "\u9f99\u5ddd\u53bf",
        "area_parent_id": "302",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3132",
        "area_name": "\u6c5f\u57ce\u533a",
        "area_parent_id": "303",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3133",
        "area_name": "\u9633\u4e1c\u53bf",
        "area_parent_id": "303",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3134",
        "area_name": "\u9633\u6625\u5e02",
        "area_parent_id": "303",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3135",
        "area_name": "\u9633\u897f\u53bf",
        "area_parent_id": "303",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3136",
        "area_name": "\u4f5b\u5188\u53bf",
        "area_parent_id": "304",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3137",
        "area_name": "\u6e05\u57ce\u533a",
        "area_parent_id": "304",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3138",
        "area_name": "\u6e05\u65b0\u53bf",
        "area_parent_id": "304",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3139",
        "area_name": "\u82f1\u5fb7\u5e02",
        "area_parent_id": "304",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3140",
        "area_name": "\u8fde\u5357\u7476\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "304",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3141",
        "area_name": "\u8fde\u5c71\u58ee\u65cf\u7476\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "304",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3142",
        "area_name": "\u8fde\u5dde\u5e02",
        "area_parent_id": "304",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3143",
        "area_name": "\u9633\u5c71\u53bf",
        "area_parent_id": "304",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3144",
        "area_name": "\u4e1c\u839e\u5e02",
        "area_parent_id": "305",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3145",
        "area_name": "\u4e2d\u5c71\u5e02",
        "area_parent_id": "306",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3146",
        "area_name": "\u6e58\u6865\u533a",
        "area_parent_id": "307",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3147",
        "area_name": "\u6f6e\u5b89\u53bf",
        "area_parent_id": "307",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3148",
        "area_name": "\u9976\u5e73\u53bf",
        "area_parent_id": "307",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3149",
        "area_name": "\u60e0\u6765\u53bf",
        "area_parent_id": "308",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3150",
        "area_name": "\u63ed\u4e1c\u53bf",
        "area_parent_id": "308",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3151",
        "area_name": "\u63ed\u897f\u53bf",
        "area_parent_id": "308",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3152",
        "area_name": "\u666e\u5b81\u5e02",
        "area_parent_id": "308",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3153",
        "area_name": "\u6995\u57ce\u533a",
        "area_parent_id": "308",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3154",
        "area_name": "\u4e91\u57ce\u533a",
        "area_parent_id": "309",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3155",
        "area_name": "\u4e91\u5b89\u53bf",
        "area_parent_id": "309",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3156",
        "area_name": "\u65b0\u5174\u53bf",
        "area_parent_id": "309",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3157",
        "area_name": "\u7f57\u5b9a\u5e02",
        "area_parent_id": "309",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3158",
        "area_name": "\u90c1\u5357\u53bf",
        "area_parent_id": "309",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3159",
        "area_name": "\u4e0a\u6797\u53bf",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3160",
        "area_name": "\u5174\u5b81\u533a",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3161",
        "area_name": "\u5bbe\u9633\u53bf",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3162",
        "area_name": "\u6a2a\u53bf",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3163",
        "area_name": "\u6b66\u9e23\u53bf",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3164",
        "area_name": "\u6c5f\u5357\u533a",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3165",
        "area_name": "\u826f\u5e86\u533a",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3166",
        "area_name": "\u897f\u4e61\u5858\u533a",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3167",
        "area_name": "\u9095\u5b81\u533a",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3168",
        "area_name": "\u9686\u5b89\u53bf",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3169",
        "area_name": "\u9752\u79c0\u533a",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3170",
        "area_name": "\u9a6c\u5c71\u53bf",
        "area_parent_id": "310",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3171",
        "area_name": "\u4e09\u6c5f\u4f97\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "311",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3172",
        "area_name": "\u57ce\u4e2d\u533a",
        "area_parent_id": "311",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3173",
        "area_name": "\u67f3\u5317\u533a",
        "area_parent_id": "311",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3174",
        "area_name": "\u67f3\u5357\u533a",
        "area_parent_id": "311",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3175",
        "area_name": "\u67f3\u57ce\u53bf",
        "area_parent_id": "311",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3176",
        "area_name": "\u67f3\u6c5f\u53bf",
        "area_parent_id": "311",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3177",
        "area_name": "\u878d\u5b89\u53bf",
        "area_parent_id": "311",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3178",
        "area_name": "\u878d\u6c34\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "311",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3179",
        "area_name": "\u9c7c\u5cf0\u533a",
        "area_parent_id": "311",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3180",
        "area_name": "\u9e7f\u5be8\u53bf",
        "area_parent_id": "311",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3181",
        "area_name": "\u4e03\u661f\u533a",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3182",
        "area_name": "\u4e34\u6842\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3183",
        "area_name": "\u5168\u5dde\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3184",
        "area_name": "\u5174\u5b89\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3185",
        "area_name": "\u53e0\u5f69\u533a",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3186",
        "area_name": "\u5e73\u4e50\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3187",
        "area_name": "\u606d\u57ce\u7476\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3188",
        "area_name": "\u6c38\u798f\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3189",
        "area_name": "\u704c\u9633\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3190",
        "area_name": "\u7075\u5ddd\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3191",
        "area_name": "\u79c0\u5cf0\u533a",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3192",
        "area_name": "\u8354\u6d66\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3193",
        "area_name": "\u8c61\u5c71\u533a",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3194",
        "area_name": "\u8d44\u6e90\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3195",
        "area_name": "\u9633\u6714\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3196",
        "area_name": "\u96c1\u5c71\u533a",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3197",
        "area_name": "\u9f99\u80dc\u5404\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "312",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3198",
        "area_name": "\u4e07\u79c0\u533a",
        "area_parent_id": "313",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3199",
        "area_name": "\u5c91\u6eaa\u5e02",
        "area_parent_id": "313",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3200",
        "area_name": "\u82cd\u68a7\u53bf",
        "area_parent_id": "313",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3201",
        "area_name": "\u8499\u5c71\u53bf",
        "area_parent_id": "313",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3202",
        "area_name": "\u85e4\u53bf",
        "area_parent_id": "313",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3203",
        "area_name": "\u8776\u5c71\u533a",
        "area_parent_id": "313",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3204",
        "area_name": "\u957f\u6d32\u533a",
        "area_parent_id": "313",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3205",
        "area_name": "\u5408\u6d66\u53bf",
        "area_parent_id": "314",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3206",
        "area_name": "\u6d77\u57ce\u533a",
        "area_parent_id": "314",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3207",
        "area_name": "\u94c1\u5c71\u6e2f\u533a",
        "area_parent_id": "314",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3208",
        "area_name": "\u94f6\u6d77\u533a",
        "area_parent_id": "314",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3209",
        "area_name": "\u4e0a\u601d\u53bf",
        "area_parent_id": "315",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3210",
        "area_name": "\u4e1c\u5174\u5e02",
        "area_parent_id": "315",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3211",
        "area_name": "\u6e2f\u53e3\u533a",
        "area_parent_id": "315",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3212",
        "area_name": "\u9632\u57ce\u533a",
        "area_parent_id": "315",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3213",
        "area_name": "\u6d66\u5317\u53bf",
        "area_parent_id": "316",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3214",
        "area_name": "\u7075\u5c71\u53bf",
        "area_parent_id": "316",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3215",
        "area_name": "\u94a6\u5317\u533a",
        "area_parent_id": "316",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3216",
        "area_name": "\u94a6\u5357\u533a",
        "area_parent_id": "316",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3217",
        "area_name": "\u5e73\u5357\u53bf",
        "area_parent_id": "317",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3218",
        "area_name": "\u6842\u5e73\u5e02",
        "area_parent_id": "317",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3219",
        "area_name": "\u6e2f\u5317\u533a",
        "area_parent_id": "317",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3220",
        "area_name": "\u6e2f\u5357\u533a",
        "area_parent_id": "317",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3221",
        "area_name": "\u8983\u5858\u533a",
        "area_parent_id": "317",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3222",
        "area_name": "\u5174\u4e1a\u53bf",
        "area_parent_id": "318",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3223",
        "area_name": "\u5317\u6d41\u5e02",
        "area_parent_id": "318",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3224",
        "area_name": "\u535a\u767d\u53bf",
        "area_parent_id": "318",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3225",
        "area_name": "\u5bb9\u53bf",
        "area_parent_id": "318",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3226",
        "area_name": "\u7389\u5dde\u533a",
        "area_parent_id": "318",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3227",
        "area_name": "\u9646\u5ddd\u53bf",
        "area_parent_id": "318",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3228",
        "area_name": "\u4e50\u4e1a\u53bf",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3229",
        "area_name": "\u51cc\u4e91\u53bf",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3230",
        "area_name": "\u53f3\u6c5f\u533a",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3231",
        "area_name": "\u5e73\u679c\u53bf",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3232",
        "area_name": "\u5fb7\u4fdd\u53bf",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3233",
        "area_name": "\u7530\u4e1c\u53bf",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3234",
        "area_name": "\u7530\u6797\u53bf",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3235",
        "area_name": "\u7530\u9633\u53bf",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3236",
        "area_name": "\u897f\u6797\u53bf",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3237",
        "area_name": "\u90a3\u5761\u53bf",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3238",
        "area_name": "\u9686\u6797\u5404\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3239",
        "area_name": "\u9756\u897f\u53bf",
        "area_parent_id": "319",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3240",
        "area_name": "\u516b\u6b65\u533a",
        "area_parent_id": "320",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3241",
        "area_name": "\u5bcc\u5ddd\u7476\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "320",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3242",
        "area_name": "\u662d\u5e73\u53bf",
        "area_parent_id": "320",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3243",
        "area_name": "\u949f\u5c71\u53bf",
        "area_parent_id": "320",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3244",
        "area_name": "\u4e1c\u5170\u53bf",
        "area_parent_id": "321",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3245",
        "area_name": "\u51e4\u5c71\u53bf",
        "area_parent_id": "321",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3246",
        "area_name": "\u5357\u4e39\u53bf",
        "area_parent_id": "321",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3247",
        "area_name": "\u5927\u5316\u7476\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "321",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3248",
        "area_name": "\u5929\u5ce8\u53bf",
        "area_parent_id": "321",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3249",
        "area_name": "\u5b9c\u5dde\u5e02",
        "area_parent_id": "321",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3250",
        "area_name": "\u5df4\u9a6c\u7476\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "321",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3251",
        "area_name": "\u73af\u6c5f\u6bdb\u5357\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "321",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3252",
        "area_name": "\u7f57\u57ce\u4eeb\u4f6c\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "321",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3253",
        "area_name": "\u90fd\u5b89\u7476\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "321",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3254",
        "area_name": "\u91d1\u57ce\u6c5f\u533a",
        "area_parent_id": "321",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3255",
        "area_name": "\u5174\u5bbe\u533a",
        "area_parent_id": "322",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3256",
        "area_name": "\u5408\u5c71\u5e02",
        "area_parent_id": "322",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3257",
        "area_name": "\u5ffb\u57ce\u53bf",
        "area_parent_id": "322",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3258",
        "area_name": "\u6b66\u5ba3\u53bf",
        "area_parent_id": "322",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3259",
        "area_name": "\u8c61\u5dde\u53bf",
        "area_parent_id": "322",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3260",
        "area_name": "\u91d1\u79c0\u7476\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "322",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3261",
        "area_name": "\u51ed\u7965\u5e02",
        "area_parent_id": "323",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3262",
        "area_name": "\u5927\u65b0\u53bf",
        "area_parent_id": "323",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3263",
        "area_name": "\u5929\u7b49\u53bf",
        "area_parent_id": "323",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3264",
        "area_name": "\u5b81\u660e\u53bf",
        "area_parent_id": "323",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3265",
        "area_name": "\u6276\u7ee5\u53bf",
        "area_parent_id": "323",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3266",
        "area_name": "\u6c5f\u5dde\u533a",
        "area_parent_id": "323",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3267",
        "area_name": "\u9f99\u5dde\u53bf",
        "area_parent_id": "323",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3268",
        "area_name": "\u743c\u5c71\u533a",
        "area_parent_id": "324",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3269",
        "area_name": "\u79c0\u82f1\u533a",
        "area_parent_id": "324",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3270",
        "area_name": "\u7f8e\u5170\u533a",
        "area_parent_id": "324",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3271",
        "area_name": "\u9f99\u534e\u533a",
        "area_parent_id": "324",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3272",
        "area_name": "\u4e09\u4e9a\u5e02",
        "area_parent_id": "325",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3273",
        "area_name": "\u4e94\u6307\u5c71\u5e02",
        "area_parent_id": "326",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3274",
        "area_name": "\u743c\u6d77\u5e02",
        "area_parent_id": "327",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3275",
        "area_name": "\u510b\u5dde\u5e02",
        "area_parent_id": "328",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3276",
        "area_name": "\u6587\u660c\u5e02",
        "area_parent_id": "329",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3277",
        "area_name": "\u4e07\u5b81\u5e02",
        "area_parent_id": "330",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3278",
        "area_name": "\u4e1c\u65b9\u5e02",
        "area_parent_id": "331",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3279",
        "area_name": "\u5b9a\u5b89\u53bf",
        "area_parent_id": "332",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3280",
        "area_name": "\u5c6f\u660c\u53bf",
        "area_parent_id": "333",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3281",
        "area_name": "\u6f84\u8fc8\u53bf",
        "area_parent_id": "334",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3282",
        "area_name": "\u4e34\u9ad8\u53bf",
        "area_parent_id": "335",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3283",
        "area_name": "\u767d\u6c99\u9ece\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "336",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3284",
        "area_name": "\u660c\u6c5f\u9ece\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "337",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3285",
        "area_name": "\u4e50\u4e1c\u9ece\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "338",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3286",
        "area_name": "\u9675\u6c34\u9ece\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "339",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3287",
        "area_name": "\u4fdd\u4ead\u9ece\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "340",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "3288",
        "area_name": "\u743c\u4e2d\u9ece\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "341",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4209",
        "area_name": "\u53cc\u6d41\u53bf",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4210",
        "area_name": "\u5927\u9091\u53bf",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4211",
        "area_name": "\u5d07\u5dde\u5e02",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4212",
        "area_name": "\u5f6d\u5dde\u5e02",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4213",
        "area_name": "\u6210\u534e\u533a",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4214",
        "area_name": "\u65b0\u6d25\u53bf",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4215",
        "area_name": "\u65b0\u90fd\u533a",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4216",
        "area_name": "\u6b66\u4faf\u533a",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4217",
        "area_name": "\u6e29\u6c5f\u533a",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4218",
        "area_name": "\u84b2\u6c5f\u53bf",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4219",
        "area_name": "\u909b\u5d03\u5e02",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4220",
        "area_name": "\u90eb\u53bf",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4221",
        "area_name": "\u90fd\u6c5f\u5830\u5e02",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4222",
        "area_name": "\u91d1\u5802\u53bf",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4223",
        "area_name": "\u91d1\u725b\u533a",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4224",
        "area_name": "\u9526\u6c5f\u533a",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4225",
        "area_name": "\u9752\u767d\u6c5f\u533a",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4226",
        "area_name": "\u9752\u7f8a\u533a",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4227",
        "area_name": "\u9f99\u6cc9\u9a7f\u533a",
        "area_parent_id": "385",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4228",
        "area_name": "\u5927\u5b89\u533a",
        "area_parent_id": "386",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4229",
        "area_name": "\u5bcc\u987a\u53bf",
        "area_parent_id": "386",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4230",
        "area_name": "\u6cbf\u6ee9\u533a",
        "area_parent_id": "386",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4231",
        "area_name": "\u81ea\u6d41\u4e95\u533a",
        "area_parent_id": "386",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4232",
        "area_name": "\u8363\u53bf",
        "area_parent_id": "386",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4233",
        "area_name": "\u8d21\u4e95\u533a",
        "area_parent_id": "386",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4234",
        "area_name": "\u4e1c\u533a",
        "area_parent_id": "387",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4235",
        "area_name": "\u4ec1\u548c\u533a",
        "area_parent_id": "387",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4236",
        "area_name": "\u76d0\u8fb9\u53bf",
        "area_parent_id": "387",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4237",
        "area_name": "\u7c73\u6613\u53bf",
        "area_parent_id": "387",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4238",
        "area_name": "\u897f\u533a",
        "area_parent_id": "387",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4239",
        "area_name": "\u53d9\u6c38\u53bf",
        "area_parent_id": "388",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4240",
        "area_name": "\u53e4\u853a\u53bf",
        "area_parent_id": "388",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4241",
        "area_name": "\u5408\u6c5f\u53bf",
        "area_parent_id": "388",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4242",
        "area_name": "\u6c5f\u9633\u533a",
        "area_parent_id": "388",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4243",
        "area_name": "\u6cf8\u53bf",
        "area_parent_id": "388",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4244",
        "area_name": "\u7eb3\u6eaa\u533a",
        "area_parent_id": "388",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4245",
        "area_name": "\u9f99\u9a6c\u6f6d\u533a",
        "area_parent_id": "388",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4246",
        "area_name": "\u4e2d\u6c5f\u53bf",
        "area_parent_id": "389",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4247",
        "area_name": "\u4ec0\u90a1\u5e02",
        "area_parent_id": "389",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4248",
        "area_name": "\u5e7f\u6c49\u5e02",
        "area_parent_id": "389",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4249",
        "area_name": "\u65cc\u9633\u533a",
        "area_parent_id": "389",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4250",
        "area_name": "\u7ef5\u7af9\u5e02",
        "area_parent_id": "389",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4251",
        "area_name": "\u7f57\u6c5f\u53bf",
        "area_parent_id": "389",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4252",
        "area_name": "\u4e09\u53f0\u53bf",
        "area_parent_id": "390",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4253",
        "area_name": "\u5317\u5ddd\u7f8c\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "390",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4254",
        "area_name": "\u5b89\u53bf",
        "area_parent_id": "390",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4255",
        "area_name": "\u5e73\u6b66\u53bf",
        "area_parent_id": "390",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4256",
        "area_name": "\u6893\u6f7c\u53bf",
        "area_parent_id": "390",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4257",
        "area_name": "\u6c5f\u6cb9\u5e02",
        "area_parent_id": "390",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4258",
        "area_name": "\u6daa\u57ce\u533a",
        "area_parent_id": "390",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4259",
        "area_name": "\u6e38\u4ed9\u533a",
        "area_parent_id": "390",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4260",
        "area_name": "\u76d0\u4ead\u53bf",
        "area_parent_id": "390",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4261",
        "area_name": "\u5143\u575d\u533a",
        "area_parent_id": "391",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4262",
        "area_name": "\u5229\u5dde\u533a",
        "area_parent_id": "391",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4263",
        "area_name": "\u5251\u9601\u53bf",
        "area_parent_id": "391",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4264",
        "area_name": "\u65fa\u82cd\u53bf",
        "area_parent_id": "391",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4265",
        "area_name": "\u671d\u5929\u533a",
        "area_parent_id": "391",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4266",
        "area_name": "\u82cd\u6eaa\u53bf",
        "area_parent_id": "391",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4267",
        "area_name": "\u9752\u5ddd\u53bf",
        "area_parent_id": "391",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4268",
        "area_name": "\u5927\u82f1\u53bf",
        "area_parent_id": "392",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4269",
        "area_name": "\u5b89\u5c45\u533a",
        "area_parent_id": "392",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4270",
        "area_name": "\u5c04\u6d2a\u53bf",
        "area_parent_id": "392",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4271",
        "area_name": "\u8239\u5c71\u533a",
        "area_parent_id": "392",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4272",
        "area_name": "\u84ec\u6eaa\u53bf",
        "area_parent_id": "392",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4273",
        "area_name": "\u4e1c\u5174\u533a",
        "area_parent_id": "393",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4274",
        "area_name": "\u5a01\u8fdc\u53bf",
        "area_parent_id": "393",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4275",
        "area_name": "\u5e02\u4e2d\u533a",
        "area_parent_id": "393",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4276",
        "area_name": "\u8d44\u4e2d\u53bf",
        "area_parent_id": "393",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4277",
        "area_name": "\u9686\u660c\u53bf",
        "area_parent_id": "393",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4278",
        "area_name": "\u4e94\u901a\u6865\u533a",
        "area_parent_id": "394",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4279",
        "area_name": "\u4e95\u7814\u53bf",
        "area_parent_id": "394",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4280",
        "area_name": "\u5939\u6c5f\u53bf",
        "area_parent_id": "394",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4281",
        "area_name": "\u5ce8\u7709\u5c71\u5e02",
        "area_parent_id": "394",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4282",
        "area_name": "\u5ce8\u8fb9\u5f5d\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "394",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4283",
        "area_name": "\u5e02\u4e2d\u533a",
        "area_parent_id": "394",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4284",
        "area_name": "\u6c90\u5ddd\u53bf",
        "area_parent_id": "394",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4285",
        "area_name": "\u6c99\u6e7e\u533a",
        "area_parent_id": "394",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4286",
        "area_name": "\u728d\u4e3a\u53bf",
        "area_parent_id": "394",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4287",
        "area_name": "\u91d1\u53e3\u6cb3\u533a",
        "area_parent_id": "394",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4288",
        "area_name": "\u9a6c\u8fb9\u5f5d\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "394",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4289",
        "area_name": "\u4eea\u9647\u53bf",
        "area_parent_id": "395",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4290",
        "area_name": "\u5357\u5145\u5e02\u5609\u9675\u533a",
        "area_parent_id": "395",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4291",
        "area_name": "\u5357\u90e8\u53bf",
        "area_parent_id": "395",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4292",
        "area_name": "\u5609\u9675\u533a",
        "area_parent_id": "395",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4293",
        "area_name": "\u8425\u5c71\u53bf",
        "area_parent_id": "395",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4294",
        "area_name": "\u84ec\u5b89\u53bf",
        "area_parent_id": "395",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4295",
        "area_name": "\u897f\u5145\u53bf",
        "area_parent_id": "395",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4296",
        "area_name": "\u9606\u4e2d\u5e02",
        "area_parent_id": "395",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4297",
        "area_name": "\u987a\u5e86\u533a",
        "area_parent_id": "395",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4298",
        "area_name": "\u9ad8\u576a\u533a",
        "area_parent_id": "395",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4299",
        "area_name": "\u4e1c\u5761\u533a",
        "area_parent_id": "396",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4300",
        "area_name": "\u4e39\u68f1\u53bf",
        "area_parent_id": "396",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4301",
        "area_name": "\u4ec1\u5bff\u53bf",
        "area_parent_id": "396",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4302",
        "area_name": "\u5f6d\u5c71\u53bf",
        "area_parent_id": "396",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4303",
        "area_name": "\u6d2a\u96c5\u53bf",
        "area_parent_id": "396",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4304",
        "area_name": "\u9752\u795e\u53bf",
        "area_parent_id": "396",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4305",
        "area_name": "\u5174\u6587\u53bf",
        "area_parent_id": "397",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4306",
        "area_name": "\u5357\u6eaa\u53bf",
        "area_parent_id": "397",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4307",
        "area_name": "\u5b9c\u5bbe\u53bf",
        "area_parent_id": "397",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4308",
        "area_name": "\u5c4f\u5c71\u53bf",
        "area_parent_id": "397",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4309",
        "area_name": "\u6c5f\u5b89\u53bf",
        "area_parent_id": "397",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4310",
        "area_name": "\u73d9\u53bf",
        "area_parent_id": "397",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4311",
        "area_name": "\u7b60\u8fde\u53bf",
        "area_parent_id": "397",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4312",
        "area_name": "\u7fe0\u5c4f\u533a",
        "area_parent_id": "397",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4313",
        "area_name": "\u957f\u5b81\u53bf",
        "area_parent_id": "397",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4314",
        "area_name": "\u9ad8\u53bf",
        "area_parent_id": "397",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4315",
        "area_name": "\u534e\u84e5\u5e02",
        "area_parent_id": "398",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4316",
        "area_name": "\u5cb3\u6c60\u53bf",
        "area_parent_id": "398",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4317",
        "area_name": "\u5e7f\u5b89\u533a",
        "area_parent_id": "398",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4318",
        "area_name": "\u6b66\u80dc\u53bf",
        "area_parent_id": "398",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4319",
        "area_name": "\u90bb\u6c34\u53bf",
        "area_parent_id": "398",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4320",
        "area_name": "\u4e07\u6e90\u5e02",
        "area_parent_id": "399",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4321",
        "area_name": "\u5927\u7af9\u53bf",
        "area_parent_id": "399",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4322",
        "area_name": "\u5ba3\u6c49\u53bf",
        "area_parent_id": "399",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4323",
        "area_name": "\u5f00\u6c5f\u53bf",
        "area_parent_id": "399",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4324",
        "area_name": "\u6e20\u53bf",
        "area_parent_id": "399",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4325",
        "area_name": "\u8fbe\u53bf",
        "area_parent_id": "399",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4326",
        "area_name": "\u901a\u5ddd\u533a",
        "area_parent_id": "399",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4327",
        "area_name": "\u540d\u5c71\u53bf",
        "area_parent_id": "400",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4328",
        "area_name": "\u5929\u5168\u53bf",
        "area_parent_id": "400",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4329",
        "area_name": "\u5b9d\u5174\u53bf",
        "area_parent_id": "400",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4330",
        "area_name": "\u6c49\u6e90\u53bf",
        "area_parent_id": "400",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4331",
        "area_name": "\u77f3\u68c9\u53bf",
        "area_parent_id": "400",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4332",
        "area_name": "\u82a6\u5c71\u53bf",
        "area_parent_id": "400",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4333",
        "area_name": "\u8365\u7ecf\u53bf",
        "area_parent_id": "400",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4334",
        "area_name": "\u96e8\u57ce\u533a",
        "area_parent_id": "400",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4335",
        "area_name": "\u5357\u6c5f\u53bf",
        "area_parent_id": "401",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4336",
        "area_name": "\u5df4\u5dde\u533a",
        "area_parent_id": "401",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4337",
        "area_name": "\u5e73\u660c\u53bf",
        "area_parent_id": "401",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4338",
        "area_name": "\u901a\u6c5f\u53bf",
        "area_parent_id": "401",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4339",
        "area_name": "\u4e50\u81f3\u53bf",
        "area_parent_id": "402",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4340",
        "area_name": "\u5b89\u5cb3\u53bf",
        "area_parent_id": "402",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4341",
        "area_name": "\u7b80\u9633\u5e02",
        "area_parent_id": "402",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4342",
        "area_name": "\u96c1\u6c5f\u533a",
        "area_parent_id": "402",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4343",
        "area_name": "\u4e5d\u5be8\u6c9f\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4344",
        "area_name": "\u58e4\u5858\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4345",
        "area_name": "\u5c0f\u91d1\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4346",
        "area_name": "\u677e\u6f58\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4347",
        "area_name": "\u6c76\u5ddd\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4348",
        "area_name": "\u7406\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4349",
        "area_name": "\u7ea2\u539f\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4350",
        "area_name": "\u82e5\u5c14\u76d6\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4351",
        "area_name": "\u8302\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4352",
        "area_name": "\u91d1\u5ddd\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4353",
        "area_name": "\u963f\u575d\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4354",
        "area_name": "\u9a6c\u5c14\u5eb7\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4355",
        "area_name": "\u9ed1\u6c34\u53bf",
        "area_parent_id": "403",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4356",
        "area_name": "\u4e39\u5df4\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4357",
        "area_name": "\u4e61\u57ce\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4358",
        "area_name": "\u5df4\u5858\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4359",
        "area_name": "\u5eb7\u5b9a\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4360",
        "area_name": "\u5f97\u8363\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4361",
        "area_name": "\u5fb7\u683c\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4362",
        "area_name": "\u65b0\u9f99\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4363",
        "area_name": "\u6cf8\u5b9a\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4364",
        "area_name": "\u7089\u970d\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4365",
        "area_name": "\u7406\u5858\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4366",
        "area_name": "\u7518\u5b5c\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4367",
        "area_name": "\u767d\u7389\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4368",
        "area_name": "\u77f3\u6e20\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4369",
        "area_name": "\u7a3b\u57ce\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4370",
        "area_name": "\u8272\u8fbe\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4371",
        "area_name": "\u9053\u5b5a\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4372",
        "area_name": "\u96c5\u6c5f\u53bf",
        "area_parent_id": "404",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4373",
        "area_name": "\u4f1a\u4e1c\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4374",
        "area_name": "\u4f1a\u7406\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4375",
        "area_name": "\u5195\u5b81\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4376",
        "area_name": "\u559c\u5fb7\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4377",
        "area_name": "\u5b81\u5357\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4378",
        "area_name": "\u5e03\u62d6\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4379",
        "area_name": "\u5fb7\u660c\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4380",
        "area_name": "\u662d\u89c9\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4381",
        "area_name": "\u666e\u683c\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4382",
        "area_name": "\u6728\u91cc\u85cf\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4383",
        "area_name": "\u7518\u6d1b\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4384",
        "area_name": "\u76d0\u6e90\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4385",
        "area_name": "\u7f8e\u59d1\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4386",
        "area_name": "\u897f\u660c",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4387",
        "area_name": "\u8d8a\u897f\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4388",
        "area_name": "\u91d1\u9633\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4389",
        "area_name": "\u96f7\u6ce2\u53bf",
        "area_parent_id": "405",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4390",
        "area_name": "\u4e4c\u5f53\u533a",
        "area_parent_id": "406",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4391",
        "area_name": "\u4e91\u5ca9\u533a",
        "area_parent_id": "406",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4392",
        "area_name": "\u4fee\u6587\u53bf",
        "area_parent_id": "406",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4393",
        "area_name": "\u5357\u660e\u533a",
        "area_parent_id": "406",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4394",
        "area_name": "\u5c0f\u6cb3\u533a",
        "area_parent_id": "406",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4395",
        "area_name": "\u5f00\u9633\u53bf",
        "area_parent_id": "406",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4396",
        "area_name": "\u606f\u70fd\u53bf",
        "area_parent_id": "406",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4397",
        "area_name": "\u6e05\u9547\u5e02",
        "area_parent_id": "406",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4398",
        "area_name": "\u767d\u4e91\u533a",
        "area_parent_id": "406",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4399",
        "area_name": "\u82b1\u6eaa\u533a",
        "area_parent_id": "406",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4400",
        "area_name": "\u516d\u679d\u7279\u533a",
        "area_parent_id": "407",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4401",
        "area_name": "\u6c34\u57ce\u53bf",
        "area_parent_id": "407",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4402",
        "area_name": "\u76d8\u53bf",
        "area_parent_id": "407",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4403",
        "area_name": "\u949f\u5c71\u533a",
        "area_parent_id": "407",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4404",
        "area_name": "\u4e60\u6c34\u53bf",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4405",
        "area_name": "\u4ec1\u6000\u5e02",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4406",
        "area_name": "\u4f59\u5e86\u53bf",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4407",
        "area_name": "\u51e4\u5188\u53bf",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4408",
        "area_name": "\u52a1\u5ddd\u4ee1\u4f6c\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4409",
        "area_name": "\u6850\u6893\u53bf",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4410",
        "area_name": "\u6b63\u5b89\u53bf",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4411",
        "area_name": "\u6c47\u5ddd\u533a",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4412",
        "area_name": "\u6e44\u6f6d\u53bf",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4413",
        "area_name": "\u7ea2\u82b1\u5c97\u533a",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4414",
        "area_name": "\u7ee5\u9633\u53bf",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4415",
        "area_name": "\u8d64\u6c34\u5e02",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4416",
        "area_name": "\u9053\u771f\u4ee1\u4f6c\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4417",
        "area_name": "\u9075\u4e49\u53bf",
        "area_parent_id": "408",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4418",
        "area_name": "\u5173\u5cad\u5e03\u4f9d\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "409",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4419",
        "area_name": "\u5e73\u575d\u53bf",
        "area_parent_id": "409",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4420",
        "area_name": "\u666e\u5b9a\u53bf",
        "area_parent_id": "409",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4421",
        "area_name": "\u7d2b\u4e91\u82d7\u65cf\u5e03\u4f9d\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "409",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4422",
        "area_name": "\u897f\u79c0\u533a",
        "area_parent_id": "409",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4423",
        "area_name": "\u9547\u5b81\u5e03\u4f9d\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "409",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4424",
        "area_name": "\u4e07\u5c71\u7279\u533a",
        "area_parent_id": "410",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4425",
        "area_name": "\u5370\u6c5f\u571f\u5bb6\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "410",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4426",
        "area_name": "\u5fb7\u6c5f\u53bf",
        "area_parent_id": "410",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4427",
        "area_name": "\u601d\u5357\u53bf",
        "area_parent_id": "410",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4428",
        "area_name": "\u677e\u6843\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "410",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4429",
        "area_name": "\u6c5f\u53e3\u53bf",
        "area_parent_id": "410",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4430",
        "area_name": "\u6cbf\u6cb3\u571f\u5bb6\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "410",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4431",
        "area_name": "\u7389\u5c4f\u4f97\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "410",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4432",
        "area_name": "\u77f3\u9621\u53bf",
        "area_parent_id": "410",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4433",
        "area_name": "\u94dc\u4ec1\u5e02",
        "area_parent_id": "410",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4434",
        "area_name": "\u5174\u4e49\u5e02",
        "area_parent_id": "411",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4435",
        "area_name": "\u5174\u4ec1\u53bf",
        "area_parent_id": "411",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4436",
        "area_name": "\u518c\u4ea8\u53bf",
        "area_parent_id": "411",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4437",
        "area_name": "\u5b89\u9f99\u53bf",
        "area_parent_id": "411",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4438",
        "area_name": "\u666e\u5b89\u53bf",
        "area_parent_id": "411",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4439",
        "area_name": "\u6674\u9686\u53bf",
        "area_parent_id": "411",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4440",
        "area_name": "\u671b\u8c1f\u53bf",
        "area_parent_id": "411",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4441",
        "area_name": "\u8d1e\u4e30\u53bf",
        "area_parent_id": "411",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4442",
        "area_name": "\u5927\u65b9\u53bf",
        "area_parent_id": "412",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4443",
        "area_name": "\u5a01\u5b81\u5f5d\u65cf\u56de\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "412",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4444",
        "area_name": "\u6bd5\u8282\u5e02",
        "area_parent_id": "412",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4445",
        "area_name": "\u7eb3\u96cd\u53bf",
        "area_parent_id": "412",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4446",
        "area_name": "\u7ec7\u91d1\u53bf",
        "area_parent_id": "412",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4447",
        "area_name": "\u8d6b\u7ae0\u53bf",
        "area_parent_id": "412",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4448",
        "area_name": "\u91d1\u6c99\u53bf",
        "area_parent_id": "412",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4449",
        "area_name": "\u9ed4\u897f\u53bf",
        "area_parent_id": "412",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4450",
        "area_name": "\u4e09\u7a57\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4451",
        "area_name": "\u4e39\u5be8\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4452",
        "area_name": "\u4ece\u6c5f\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4453",
        "area_name": "\u51ef\u91cc\u5e02",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4454",
        "area_name": "\u5251\u6cb3\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4455",
        "area_name": "\u53f0\u6c5f\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4456",
        "area_name": "\u5929\u67f1\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4457",
        "area_name": "\u5c91\u5de9\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4458",
        "area_name": "\u65bd\u79c9\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4459",
        "area_name": "\u6995\u6c5f\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4460",
        "area_name": "\u9526\u5c4f\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4461",
        "area_name": "\u9547\u8fdc\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4462",
        "area_name": "\u96f7\u5c71\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4463",
        "area_name": "\u9ebb\u6c5f\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4464",
        "area_name": "\u9ec4\u5e73\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4465",
        "area_name": "\u9ece\u5e73\u53bf",
        "area_parent_id": "413",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4466",
        "area_name": "\u4e09\u90fd\u6c34\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4467",
        "area_name": "\u5e73\u5858\u53bf",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4468",
        "area_name": "\u60e0\u6c34\u53bf",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4469",
        "area_name": "\u72ec\u5c71\u53bf",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4470",
        "area_name": "\u74ee\u5b89\u53bf",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4471",
        "area_name": "\u798f\u6cc9\u5e02",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4472",
        "area_name": "\u7f57\u7538\u53bf",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4473",
        "area_name": "\u8354\u6ce2\u53bf",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4474",
        "area_name": "\u8d35\u5b9a\u53bf",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4475",
        "area_name": "\u90fd\u5300\u5e02",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4476",
        "area_name": "\u957f\u987a\u53bf",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4477",
        "area_name": "\u9f99\u91cc\u53bf",
        "area_parent_id": "414",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4478",
        "area_name": "\u4e1c\u5ddd\u533a",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4479",
        "area_name": "\u4e94\u534e\u533a",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4480",
        "area_name": "\u5448\u8d21\u53bf",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4481",
        "area_name": "\u5b89\u5b81\u5e02",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4482",
        "area_name": "\u5b98\u6e21\u533a",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4483",
        "area_name": "\u5b9c\u826f\u53bf",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4484",
        "area_name": "\u5bcc\u6c11\u53bf",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4485",
        "area_name": "\u5bfb\u7538\u56de\u65cf\u5f5d\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4486",
        "area_name": "\u5d69\u660e\u53bf",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4487",
        "area_name": "\u664b\u5b81\u53bf",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4488",
        "area_name": "\u76d8\u9f99\u533a",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4489",
        "area_name": "\u77f3\u6797\u5f5d\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4490",
        "area_name": "\u7984\u529d\u5f5d\u65cf\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4491",
        "area_name": "\u897f\u5c71\u533a",
        "area_parent_id": "415",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4492",
        "area_name": "\u4f1a\u6cfd\u53bf",
        "area_parent_id": "416",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4493",
        "area_name": "\u5ba3\u5a01\u5e02",
        "area_parent_id": "416",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4494",
        "area_name": "\u5bcc\u6e90\u53bf",
        "area_parent_id": "416",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4495",
        "area_name": "\u5e08\u5b97\u53bf",
        "area_parent_id": "416",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4496",
        "area_name": "\u6cbe\u76ca\u53bf",
        "area_parent_id": "416",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4497",
        "area_name": "\u7f57\u5e73\u53bf",
        "area_parent_id": "416",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4498",
        "area_name": "\u9646\u826f\u53bf",
        "area_parent_id": "416",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4499",
        "area_name": "\u9a6c\u9f99\u53bf",
        "area_parent_id": "416",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4500",
        "area_name": "\u9e92\u9e9f\u533a",
        "area_parent_id": "416",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4501",
        "area_name": "\u5143\u6c5f\u54c8\u5c3c\u65cf\u5f5d\u65cf\u50a3\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "417",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4502",
        "area_name": "\u534e\u5b81\u53bf",
        "area_parent_id": "417",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4503",
        "area_name": "\u5ce8\u5c71\u5f5d\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "417",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4504",
        "area_name": "\u65b0\u5e73\u5f5d\u65cf\u50a3\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "417",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4505",
        "area_name": "\u6613\u95e8\u53bf",
        "area_parent_id": "417",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4506",
        "area_name": "\u6c5f\u5ddd\u53bf",
        "area_parent_id": "417",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4507",
        "area_name": "\u6f84\u6c5f\u53bf",
        "area_parent_id": "417",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4508",
        "area_name": "\u7ea2\u5854\u533a",
        "area_parent_id": "417",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4509",
        "area_name": "\u901a\u6d77\u53bf",
        "area_parent_id": "417",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4510",
        "area_name": "\u65bd\u7538\u53bf",
        "area_parent_id": "418",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4511",
        "area_name": "\u660c\u5b81\u53bf",
        "area_parent_id": "418",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4512",
        "area_name": "\u817e\u51b2\u53bf",
        "area_parent_id": "418",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4513",
        "area_name": "\u9686\u9633\u533a",
        "area_parent_id": "418",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4514",
        "area_name": "\u9f99\u9675\u53bf",
        "area_parent_id": "418",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4515",
        "area_name": "\u5927\u5173\u53bf",
        "area_parent_id": "419",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4516",
        "area_name": "\u5a01\u4fe1\u53bf",
        "area_parent_id": "419",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4517",
        "area_name": "\u5de7\u5bb6\u53bf",
        "area_parent_id": "419",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4518",
        "area_name": "\u5f5d\u826f\u53bf",
        "area_parent_id": "419",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4519",
        "area_name": "\u662d\u9633\u533a",
        "area_parent_id": "419",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4520",
        "area_name": "\u6c34\u5bcc\u53bf",
        "area_parent_id": "419",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4521",
        "area_name": "\u6c38\u5584\u53bf",
        "area_parent_id": "419",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4522",
        "area_name": "\u76d0\u6d25\u53bf",
        "area_parent_id": "419",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4523",
        "area_name": "\u7ee5\u6c5f\u53bf",
        "area_parent_id": "419",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4524",
        "area_name": "\u9547\u96c4\u53bf",
        "area_parent_id": "419",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4525",
        "area_name": "\u9c81\u7538\u53bf",
        "area_parent_id": "419",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4526",
        "area_name": "\u534e\u576a\u53bf",
        "area_parent_id": "420",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4527",
        "area_name": "\u53e4\u57ce\u533a",
        "area_parent_id": "420",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4528",
        "area_name": "\u5b81\u8497\u5f5d\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "420",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4529",
        "area_name": "\u6c38\u80dc\u53bf",
        "area_parent_id": "420",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4530",
        "area_name": "\u7389\u9f99\u7eb3\u897f\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "420",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4531",
        "area_name": "\u4e34\u7fd4\u533a",
        "area_parent_id": "422",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4532",
        "area_name": "\u4e91\u53bf",
        "area_parent_id": "422",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4533",
        "area_name": "\u51e4\u5e86\u53bf",
        "area_parent_id": "422",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4534",
        "area_name": "\u53cc\u6c5f\u62c9\u795c\u65cf\u4f64\u65cf\u5e03\u6717\u65cf\u50a3\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "422",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4535",
        "area_name": "\u6c38\u5fb7\u53bf",
        "area_parent_id": "422",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4536",
        "area_name": "\u6ca7\u6e90\u4f64\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "422",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4537",
        "area_name": "\u803f\u9a6c\u50a3\u65cf\u4f64\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "422",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4538",
        "area_name": "\u9547\u5eb7\u53bf",
        "area_parent_id": "422",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4539",
        "area_name": "\u5143\u8c0b\u53bf",
        "area_parent_id": "423",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4540",
        "area_name": "\u5357\u534e\u53bf",
        "area_parent_id": "423",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4541",
        "area_name": "\u53cc\u67cf\u53bf",
        "area_parent_id": "423",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4542",
        "area_name": "\u5927\u59da\u53bf",
        "area_parent_id": "423",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4543",
        "area_name": "\u59da\u5b89\u53bf",
        "area_parent_id": "423",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4544",
        "area_name": "\u695a\u96c4\u5e02",
        "area_parent_id": "423",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4545",
        "area_name": "\u6b66\u5b9a\u53bf",
        "area_parent_id": "423",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4546",
        "area_name": "\u6c38\u4ec1\u53bf",
        "area_parent_id": "423",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4547",
        "area_name": "\u725f\u5b9a\u53bf",
        "area_parent_id": "423",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4548",
        "area_name": "\u7984\u4e30\u53bf",
        "area_parent_id": "423",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4549",
        "area_name": "\u4e2a\u65e7\u5e02",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4550",
        "area_name": "\u5143\u9633\u53bf",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4551",
        "area_name": "\u5c4f\u8fb9\u82d7\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4552",
        "area_name": "\u5efa\u6c34\u53bf",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4553",
        "area_name": "\u5f00\u8fdc\u5e02",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4554",
        "area_name": "\u5f25\u52d2\u53bf",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4555",
        "area_name": "\u6cb3\u53e3\u7476\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4556",
        "area_name": "\u6cf8\u897f\u53bf",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4557",
        "area_name": "\u77f3\u5c4f\u53bf",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4558",
        "area_name": "\u7ea2\u6cb3\u53bf",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4559",
        "area_name": "\u7eff\u6625\u53bf",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4560",
        "area_name": "\u8499\u81ea\u53bf",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4561",
        "area_name": "\u91d1\u5e73\u82d7\u65cf\u7476\u65cf\u50a3\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "424",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4562",
        "area_name": "\u4e18\u5317\u53bf",
        "area_parent_id": "425",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4563",
        "area_name": "\u5bcc\u5b81\u53bf",
        "area_parent_id": "425",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4564",
        "area_name": "\u5e7f\u5357\u53bf",
        "area_parent_id": "425",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4565",
        "area_name": "\u6587\u5c71\u53bf",
        "area_parent_id": "425",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4566",
        "area_name": "\u781a\u5c71\u53bf",
        "area_parent_id": "425",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4567",
        "area_name": "\u897f\u7574\u53bf",
        "area_parent_id": "425",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4568",
        "area_name": "\u9a6c\u5173\u53bf",
        "area_parent_id": "425",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4569",
        "area_name": "\u9ebb\u6817\u5761\u53bf",
        "area_parent_id": "425",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4570",
        "area_name": "\u52d0\u6d77\u53bf",
        "area_parent_id": "426",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4571",
        "area_name": "\u52d0\u814a\u53bf",
        "area_parent_id": "426",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4572",
        "area_name": "\u666f\u6d2a\u5e02",
        "area_parent_id": "426",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4573",
        "area_name": "\u4e91\u9f99\u53bf",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4574",
        "area_name": "\u5251\u5ddd\u53bf",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4575",
        "area_name": "\u5357\u6da7\u5f5d\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4576",
        "area_name": "\u5927\u7406\u5e02",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4577",
        "area_name": "\u5bbe\u5ddd\u53bf",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4578",
        "area_name": "\u5dcd\u5c71\u5f5d\u65cf\u56de\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4579",
        "area_name": "\u5f25\u6e21\u53bf",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4580",
        "area_name": "\u6c38\u5e73\u53bf",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4581",
        "area_name": "\u6d31\u6e90\u53bf",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4582",
        "area_name": "\u6f3e\u6fde\u5f5d\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4583",
        "area_name": "\u7965\u4e91\u53bf",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4584",
        "area_name": "\u9e64\u5e86\u53bf",
        "area_parent_id": "427",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4585",
        "area_name": "\u6881\u6cb3\u53bf",
        "area_parent_id": "428",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4586",
        "area_name": "\u6f5e\u897f\u5e02",
        "area_parent_id": "428",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4587",
        "area_name": "\u745e\u4e3d\u5e02",
        "area_parent_id": "428",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4588",
        "area_name": "\u76c8\u6c5f\u53bf",
        "area_parent_id": "428",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4589",
        "area_name": "\u9647\u5ddd\u53bf",
        "area_parent_id": "428",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4590",
        "area_name": "\u5fb7\u94a6\u53bf",
        "area_parent_id": "430",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4591",
        "area_name": "\u7ef4\u897f\u5088\u50f3\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "430",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4592",
        "area_name": "\u9999\u683c\u91cc\u62c9\u53bf",
        "area_parent_id": "430",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4593",
        "area_name": "\u57ce\u5173\u533a",
        "area_parent_id": "431",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4594",
        "area_name": "\u5806\u9f99\u5fb7\u5e86\u53bf",
        "area_parent_id": "431",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4595",
        "area_name": "\u58a8\u7af9\u5de5\u5361\u53bf",
        "area_parent_id": "431",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4596",
        "area_name": "\u5c3c\u6728\u53bf",
        "area_parent_id": "431",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4597",
        "area_name": "\u5f53\u96c4\u53bf",
        "area_parent_id": "431",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4598",
        "area_name": "\u66f2\u6c34\u53bf",
        "area_parent_id": "431",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4599",
        "area_name": "\u6797\u5468\u53bf",
        "area_parent_id": "431",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4600",
        "area_name": "\u8fbe\u5b5c\u53bf",
        "area_parent_id": "431",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4601",
        "area_name": "\u4e01\u9752\u53bf",
        "area_parent_id": "432",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4602",
        "area_name": "\u516b\u5bbf\u53bf",
        "area_parent_id": "432",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4603",
        "area_name": "\u5bdf\u96c5\u53bf",
        "area_parent_id": "432",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4604",
        "area_name": "\u5de6\u8d21\u53bf",
        "area_parent_id": "432",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4605",
        "area_name": "\u660c\u90fd\u53bf",
        "area_parent_id": "432",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4606",
        "area_name": "\u6c5f\u8fbe\u53bf",
        "area_parent_id": "432",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4607",
        "area_name": "\u6d1b\u9686\u53bf",
        "area_parent_id": "432",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4608",
        "area_name": "\u7c7b\u4e4c\u9f50\u53bf",
        "area_parent_id": "432",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4609",
        "area_name": "\u8292\u5eb7\u53bf",
        "area_parent_id": "432",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4610",
        "area_name": "\u8d21\u89c9\u53bf",
        "area_parent_id": "432",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4611",
        "area_name": "\u8fb9\u575d\u53bf",
        "area_parent_id": "432",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4612",
        "area_name": "\u4e43\u4e1c\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4613",
        "area_name": "\u52a0\u67e5\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4614",
        "area_name": "\u624e\u56ca\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4615",
        "area_name": "\u63aa\u7f8e\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4616",
        "area_name": "\u66f2\u677e\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4617",
        "area_name": "\u6851\u65e5\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4618",
        "area_name": "\u6d1b\u624e\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4619",
        "area_name": "\u6d6a\u5361\u5b50\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4620",
        "area_name": "\u743c\u7ed3\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4621",
        "area_name": "\u8d21\u560e\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4622",
        "area_name": "\u9519\u90a3\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4623",
        "area_name": "\u9686\u5b50\u53bf",
        "area_parent_id": "433",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4624",
        "area_name": "\u4e9a\u4e1c\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4625",
        "area_name": "\u4ec1\u5e03\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4626",
        "area_name": "\u4ef2\u5df4\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4627",
        "area_name": "\u5357\u6728\u6797\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4628",
        "area_name": "\u5409\u9686\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4629",
        "area_name": "\u5b9a\u65e5\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4630",
        "area_name": "\u5b9a\u7ed3\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4631",
        "area_name": "\u5c97\u5df4\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4632",
        "area_name": "\u5eb7\u9a6c\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4633",
        "area_name": "\u62c9\u5b5c\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4634",
        "area_name": "\u65e5\u5580\u5219\u5e02",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4635",
        "area_name": "\u6602\u4ec1\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4636",
        "area_name": "\u6c5f\u5b5c\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4637",
        "area_name": "\u767d\u6717\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4638",
        "area_name": "\u8042\u62c9\u6728\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4639",
        "area_name": "\u8428\u560e\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4640",
        "area_name": "\u8428\u8fe6\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4641",
        "area_name": "\u8c22\u901a\u95e8\u53bf",
        "area_parent_id": "434",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4642",
        "area_name": "\u5609\u9ece\u53bf",
        "area_parent_id": "435",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4643",
        "area_name": "\u5b89\u591a\u53bf",
        "area_parent_id": "435",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4644",
        "area_name": "\u5c3c\u739b\u53bf",
        "area_parent_id": "435",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4645",
        "area_name": "\u5df4\u9752\u53bf",
        "area_parent_id": "435",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4646",
        "area_name": "\u6bd4\u5982\u53bf",
        "area_parent_id": "435",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4647",
        "area_name": "\u73ed\u6208\u53bf",
        "area_parent_id": "435",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4648",
        "area_name": "\u7533\u624e\u53bf",
        "area_parent_id": "435",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4649",
        "area_name": "\u7d22\u53bf",
        "area_parent_id": "435",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4650",
        "area_name": "\u8042\u8363\u53bf",
        "area_parent_id": "435",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4651",
        "area_name": "\u90a3\u66f2\u53bf",
        "area_parent_id": "435",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4652",
        "area_name": "\u5676\u5c14\u53bf",
        "area_parent_id": "436",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4653",
        "area_name": "\u63aa\u52e4\u53bf",
        "area_parent_id": "436",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4654",
        "area_name": "\u6539\u5219\u53bf",
        "area_parent_id": "436",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4655",
        "area_name": "\u65e5\u571f\u53bf",
        "area_parent_id": "436",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4656",
        "area_name": "\u666e\u5170\u53bf",
        "area_parent_id": "436",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4657",
        "area_name": "\u672d\u8fbe\u53bf",
        "area_parent_id": "436",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4658",
        "area_name": "\u9769\u5409\u53bf",
        "area_parent_id": "436",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4659",
        "area_name": "\u58a8\u8131\u53bf",
        "area_parent_id": "437",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4660",
        "area_name": "\u5bdf\u9685\u53bf",
        "area_parent_id": "437",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4661",
        "area_name": "\u5de5\u5e03\u6c5f\u8fbe\u53bf",
        "area_parent_id": "437",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4662",
        "area_name": "\u6717\u53bf",
        "area_parent_id": "437",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4663",
        "area_name": "\u6797\u829d\u53bf",
        "area_parent_id": "437",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4664",
        "area_name": "\u6ce2\u5bc6\u53bf",
        "area_parent_id": "437",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4665",
        "area_name": "\u7c73\u6797\u53bf",
        "area_parent_id": "437",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4666",
        "area_name": "\u4e34\u6f7c\u533a",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4667",
        "area_name": "\u5468\u81f3\u53bf",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4668",
        "area_name": "\u6237\u53bf",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4669",
        "area_name": "\u65b0\u57ce\u533a",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4670",
        "area_name": "\u672a\u592e\u533a",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4671",
        "area_name": "\u705e\u6865\u533a",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4672",
        "area_name": "\u7891\u6797\u533a",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4673",
        "area_name": "\u83b2\u6e56\u533a",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4674",
        "area_name": "\u84dd\u7530\u53bf",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4675",
        "area_name": "\u957f\u5b89\u533a",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4676",
        "area_name": "\u960e\u826f\u533a",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4677",
        "area_name": "\u96c1\u5854\u533a",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4678",
        "area_name": "\u9ad8\u9675\u53bf",
        "area_parent_id": "438",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4679",
        "area_name": "\u5370\u53f0\u533a",
        "area_parent_id": "439",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4680",
        "area_name": "\u5b9c\u541b\u53bf",
        "area_parent_id": "439",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4681",
        "area_name": "\u738b\u76ca\u533a",
        "area_parent_id": "439",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4682",
        "area_name": "\u8000\u5dde\u533a",
        "area_parent_id": "439",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4683",
        "area_name": "\u51e4\u53bf",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4684",
        "area_name": "\u51e4\u7fd4\u53bf",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4685",
        "area_name": "\u5343\u9633\u53bf",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4686",
        "area_name": "\u592a\u767d\u53bf",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4687",
        "area_name": "\u5c90\u5c71\u53bf",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4688",
        "area_name": "\u6276\u98ce\u53bf",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4689",
        "area_name": "\u6e2d\u6ee8\u533a",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4690",
        "area_name": "\u7709\u53bf",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4691",
        "area_name": "\u91d1\u53f0\u533a",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4692",
        "area_name": "\u9647\u53bf",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4693",
        "area_name": "\u9648\u4ed3\u533a",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4694",
        "area_name": "\u9e9f\u6e38\u53bf",
        "area_parent_id": "440",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4695",
        "area_name": "\u4e09\u539f\u53bf",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4696",
        "area_name": "\u5e72\u53bf",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4697",
        "area_name": "\u5174\u5e73\u5e02",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4698",
        "area_name": "\u5f6c\u53bf",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4699",
        "area_name": "\u65ec\u9091\u53bf",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4700",
        "area_name": "\u6768\u9675\u533a",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4701",
        "area_name": "\u6b66\u529f\u53bf",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4702",
        "area_name": "\u6c38\u5bff\u53bf",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4703",
        "area_name": "\u6cfe\u9633\u53bf",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4704",
        "area_name": "\u6df3\u5316\u53bf",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4705",
        "area_name": "\u6e2d\u57ce\u533a",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4706",
        "area_name": "\u793c\u6cc9\u53bf",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4707",
        "area_name": "\u79e6\u90fd\u533a",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4708",
        "area_name": "\u957f\u6b66\u53bf",
        "area_parent_id": "441",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4709",
        "area_name": "\u4e34\u6e2d\u533a",
        "area_parent_id": "442",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4710",
        "area_name": "\u534e\u53bf",
        "area_parent_id": "442",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4711",
        "area_name": "\u534e\u9634\u5e02",
        "area_parent_id": "442",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4712",
        "area_name": "\u5408\u9633\u53bf",
        "area_parent_id": "442",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4713",
        "area_name": "\u5927\u8354\u53bf",
        "area_parent_id": "442",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4714",
        "area_name": "\u5bcc\u5e73\u53bf",
        "area_parent_id": "442",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4715",
        "area_name": "\u6f7c\u5173\u53bf",
        "area_parent_id": "442",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4716",
        "area_name": "\u6f84\u57ce\u53bf",
        "area_parent_id": "442",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4717",
        "area_name": "\u767d\u6c34\u53bf",
        "area_parent_id": "442",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4718",
        "area_name": "\u84b2\u57ce\u53bf",
        "area_parent_id": "442",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4719",
        "area_name": "\u97e9\u57ce\u5e02",
        "area_parent_id": "442",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4720",
        "area_name": "\u5434\u8d77\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4721",
        "area_name": "\u5b50\u957f\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4722",
        "area_name": "\u5b89\u585e\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4723",
        "area_name": "\u5b9c\u5ddd\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4724",
        "area_name": "\u5b9d\u5854\u533a",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4725",
        "area_name": "\u5bcc\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4726",
        "area_name": "\u5ef6\u5ddd\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4727",
        "area_name": "\u5ef6\u957f\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4728",
        "area_name": "\u5fd7\u4e39\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4729",
        "area_name": "\u6d1b\u5ddd\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4730",
        "area_name": "\u7518\u6cc9\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4731",
        "area_name": "\u9ec4\u9675\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4732",
        "area_name": "\u9ec4\u9f99\u53bf",
        "area_parent_id": "443",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4733",
        "area_name": "\u4f5b\u576a\u53bf",
        "area_parent_id": "444",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4734",
        "area_name": "\u52c9\u53bf",
        "area_parent_id": "444",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4735",
        "area_name": "\u5357\u90d1\u53bf",
        "area_parent_id": "444",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4736",
        "area_name": "\u57ce\u56fa\u53bf",
        "area_parent_id": "444",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4737",
        "area_name": "\u5b81\u5f3a\u53bf",
        "area_parent_id": "444",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4738",
        "area_name": "\u6c49\u53f0\u533a",
        "area_parent_id": "444",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4739",
        "area_name": "\u6d0b\u53bf",
        "area_parent_id": "444",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4740",
        "area_name": "\u7559\u575d\u53bf",
        "area_parent_id": "444",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4741",
        "area_name": "\u7565\u9633\u53bf",
        "area_parent_id": "444",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4742",
        "area_name": "\u897f\u4e61\u53bf",
        "area_parent_id": "444",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4743",
        "area_name": "\u9547\u5df4\u53bf",
        "area_parent_id": "444",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4744",
        "area_name": "\u4f73\u53bf",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4745",
        "area_name": "\u5434\u5821\u53bf",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4746",
        "area_name": "\u5b50\u6d32\u53bf",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4747",
        "area_name": "\u5b9a\u8fb9\u53bf",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4748",
        "area_name": "\u5e9c\u8c37\u53bf",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4749",
        "area_name": "\u6986\u6797\u5e02\u6986\u9633\u533a",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4750",
        "area_name": "\u6a2a\u5c71\u53bf",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4751",
        "area_name": "\u6e05\u6da7\u53bf",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4752",
        "area_name": "\u795e\u6728\u53bf",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4753",
        "area_name": "\u7c73\u8102\u53bf",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4754",
        "area_name": "\u7ee5\u5fb7\u53bf",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4755",
        "area_name": "\u9756\u8fb9\u53bf",
        "area_parent_id": "445",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4756",
        "area_name": "\u5b81\u9655\u53bf",
        "area_parent_id": "446",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4757",
        "area_name": "\u5c9a\u768b\u53bf",
        "area_parent_id": "446",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4758",
        "area_name": "\u5e73\u5229\u53bf",
        "area_parent_id": "446",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4759",
        "area_name": "\u65ec\u9633\u53bf",
        "area_parent_id": "446",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4760",
        "area_name": "\u6c49\u6ee8\u533a",
        "area_parent_id": "446",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4761",
        "area_name": "\u6c49\u9634\u53bf",
        "area_parent_id": "446",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4762",
        "area_name": "\u767d\u6cb3\u53bf",
        "area_parent_id": "446",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4763",
        "area_name": "\u77f3\u6cc9\u53bf",
        "area_parent_id": "446",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4764",
        "area_name": "\u7d2b\u9633\u53bf",
        "area_parent_id": "446",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4765",
        "area_name": "\u9547\u576a\u53bf",
        "area_parent_id": "446",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4766",
        "area_name": "\u4e39\u51e4\u53bf",
        "area_parent_id": "447",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4767",
        "area_name": "\u5546\u5357\u53bf",
        "area_parent_id": "447",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4768",
        "area_name": "\u5546\u5dde\u533a",
        "area_parent_id": "447",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4769",
        "area_name": "\u5c71\u9633\u53bf",
        "area_parent_id": "447",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4770",
        "area_name": "\u67de\u6c34\u53bf",
        "area_parent_id": "447",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4771",
        "area_name": "\u6d1b\u5357\u53bf",
        "area_parent_id": "447",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4772",
        "area_name": "\u9547\u5b89\u53bf",
        "area_parent_id": "447",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4773",
        "area_name": "\u4e03\u91cc\u6cb3\u533a",
        "area_parent_id": "448",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4774",
        "area_name": "\u57ce\u5173\u533a",
        "area_parent_id": "448",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4775",
        "area_name": "\u5b89\u5b81\u533a",
        "area_parent_id": "448",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4776",
        "area_name": "\u6986\u4e2d\u53bf",
        "area_parent_id": "448",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4777",
        "area_name": "\u6c38\u767b\u53bf",
        "area_parent_id": "448",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4778",
        "area_name": "\u768b\u5170\u53bf",
        "area_parent_id": "448",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4779",
        "area_name": "\u7ea2\u53e4\u533a",
        "area_parent_id": "448",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4780",
        "area_name": "\u897f\u56fa\u533a",
        "area_parent_id": "448",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4781",
        "area_name": "\u5609\u5cea\u5173\u5e02",
        "area_parent_id": "449",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4782",
        "area_name": "\u6c38\u660c\u53bf",
        "area_parent_id": "450",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4783",
        "area_name": "\u91d1\u5ddd\u533a",
        "area_parent_id": "450",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4784",
        "area_name": "\u4f1a\u5b81\u53bf",
        "area_parent_id": "451",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4785",
        "area_name": "\u5e73\u5ddd\u533a",
        "area_parent_id": "451",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4786",
        "area_name": "\u666f\u6cf0\u53bf",
        "area_parent_id": "451",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4787",
        "area_name": "\u767d\u94f6\u533a",
        "area_parent_id": "451",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4788",
        "area_name": "\u9756\u8fdc\u53bf",
        "area_parent_id": "451",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4789",
        "area_name": "\u5f20\u5bb6\u5ddd\u56de\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "452",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4790",
        "area_name": "\u6b66\u5c71\u53bf",
        "area_parent_id": "452",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4791",
        "area_name": "\u6e05\u6c34\u53bf",
        "area_parent_id": "452",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4792",
        "area_name": "\u7518\u8c37\u53bf",
        "area_parent_id": "452",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4793",
        "area_name": "\u79e6\u5b89\u53bf",
        "area_parent_id": "452",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4794",
        "area_name": "\u79e6\u5dde\u533a",
        "area_parent_id": "452",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4795",
        "area_name": "\u9ea6\u79ef\u533a",
        "area_parent_id": "452",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4796",
        "area_name": "\u51c9\u5dde\u533a",
        "area_parent_id": "453",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4797",
        "area_name": "\u53e4\u6d6a\u53bf",
        "area_parent_id": "453",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4798",
        "area_name": "\u5929\u795d\u85cf\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "453",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4799",
        "area_name": "\u6c11\u52e4\u53bf",
        "area_parent_id": "453",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4800",
        "area_name": "\u4e34\u6cfd\u53bf",
        "area_parent_id": "454",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4801",
        "area_name": "\u5c71\u4e39\u53bf",
        "area_parent_id": "454",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4802",
        "area_name": "\u6c11\u4e50\u53bf",
        "area_parent_id": "454",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4803",
        "area_name": "\u7518\u5dde\u533a",
        "area_parent_id": "454",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4804",
        "area_name": "\u8083\u5357\u88d5\u56fa\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "454",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4805",
        "area_name": "\u9ad8\u53f0\u53bf",
        "area_parent_id": "454",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4806",
        "area_name": "\u534e\u4ead\u53bf",
        "area_parent_id": "455",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4807",
        "area_name": "\u5d06\u5cd2\u533a",
        "area_parent_id": "455",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4808",
        "area_name": "\u5d07\u4fe1\u53bf",
        "area_parent_id": "455",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4809",
        "area_name": "\u5e84\u6d6a\u53bf",
        "area_parent_id": "455",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4810",
        "area_name": "\u6cfe\u5ddd\u53bf",
        "area_parent_id": "455",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4811",
        "area_name": "\u7075\u53f0\u53bf",
        "area_parent_id": "455",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4812",
        "area_name": "\u9759\u5b81\u53bf",
        "area_parent_id": "455",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4813",
        "area_name": "\u6566\u714c\u5e02",
        "area_parent_id": "456",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4814",
        "area_name": "\u7389\u95e8\u5e02",
        "area_parent_id": "456",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4815",
        "area_name": "\u74dc\u5dde\u53bf\uff08\u539f\u5b89\u897f\u53bf\uff09",
        "area_parent_id": "456",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4816",
        "area_name": "\u8083\u5317\u8499\u53e4\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "456",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4817",
        "area_name": "\u8083\u5dde\u533a",
        "area_parent_id": "456",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4818",
        "area_name": "\u91d1\u5854\u53bf",
        "area_parent_id": "456",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4819",
        "area_name": "\u963f\u514b\u585e\u54c8\u8428\u514b\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "456",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4820",
        "area_name": "\u534e\u6c60\u53bf",
        "area_parent_id": "457",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4821",
        "area_name": "\u5408\u6c34\u53bf",
        "area_parent_id": "457",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4822",
        "area_name": "\u5b81\u53bf",
        "area_parent_id": "457",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4823",
        "area_name": "\u5e86\u57ce\u53bf",
        "area_parent_id": "457",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4824",
        "area_name": "\u6b63\u5b81\u53bf",
        "area_parent_id": "457",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4825",
        "area_name": "\u73af\u53bf",
        "area_parent_id": "457",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4826",
        "area_name": "\u897f\u5cf0\u533a",
        "area_parent_id": "457",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4827",
        "area_name": "\u9547\u539f\u53bf",
        "area_parent_id": "457",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4828",
        "area_name": "\u4e34\u6d2e\u53bf",
        "area_parent_id": "458",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4829",
        "area_name": "\u5b89\u5b9a\u533a",
        "area_parent_id": "458",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4830",
        "area_name": "\u5cb7\u53bf",
        "area_parent_id": "458",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4831",
        "area_name": "\u6e2d\u6e90\u53bf",
        "area_parent_id": "458",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4832",
        "area_name": "\u6f33\u53bf",
        "area_parent_id": "458",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4833",
        "area_name": "\u901a\u6e2d\u53bf",
        "area_parent_id": "458",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4834",
        "area_name": "\u9647\u897f\u53bf",
        "area_parent_id": "458",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4835",
        "area_name": "\u4e24\u5f53\u53bf",
        "area_parent_id": "459",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4836",
        "area_name": "\u5b95\u660c\u53bf",
        "area_parent_id": "459",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4837",
        "area_name": "\u5eb7\u53bf",
        "area_parent_id": "459",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4838",
        "area_name": "\u5fbd\u53bf",
        "area_parent_id": "459",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4839",
        "area_name": "\u6210\u53bf",
        "area_parent_id": "459",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4840",
        "area_name": "\u6587\u53bf",
        "area_parent_id": "459",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4841",
        "area_name": "\u6b66\u90fd\u533a",
        "area_parent_id": "459",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4842",
        "area_name": "\u793c\u53bf",
        "area_parent_id": "459",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4843",
        "area_name": "\u897f\u548c\u53bf",
        "area_parent_id": "459",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4844",
        "area_name": "\u4e1c\u4e61\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "460",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4845",
        "area_name": "\u4e34\u590f\u53bf",
        "area_parent_id": "460",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4846",
        "area_name": "\u4e34\u590f\u5e02",
        "area_parent_id": "460",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4847",
        "area_name": "\u548c\u653f\u53bf",
        "area_parent_id": "460",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4848",
        "area_name": "\u5e7f\u6cb3\u53bf",
        "area_parent_id": "460",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4849",
        "area_name": "\u5eb7\u4e50\u53bf",
        "area_parent_id": "460",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4850",
        "area_name": "\u6c38\u9756\u53bf",
        "area_parent_id": "460",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4851",
        "area_name": "\u79ef\u77f3\u5c71\u4fdd\u5b89\u65cf\u4e1c\u4e61\u65cf\u6492\u62c9\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "460",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4852",
        "area_name": "\u4e34\u6f6d\u53bf",
        "area_parent_id": "461",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4853",
        "area_name": "\u5353\u5c3c\u53bf",
        "area_parent_id": "461",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4854",
        "area_name": "\u5408\u4f5c\u5e02",
        "area_parent_id": "461",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4855",
        "area_name": "\u590f\u6cb3\u53bf",
        "area_parent_id": "461",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4856",
        "area_name": "\u739b\u66f2\u53bf",
        "area_parent_id": "461",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4857",
        "area_name": "\u788c\u66f2\u53bf",
        "area_parent_id": "461",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4858",
        "area_name": "\u821f\u66f2\u53bf",
        "area_parent_id": "461",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4859",
        "area_name": "\u8fed\u90e8\u53bf",
        "area_parent_id": "461",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4860",
        "area_name": "\u57ce\u4e1c\u533a",
        "area_parent_id": "462",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4861",
        "area_name": "\u57ce\u4e2d\u533a",
        "area_parent_id": "462",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4862",
        "area_name": "\u57ce\u5317\u533a",
        "area_parent_id": "462",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4863",
        "area_name": "\u57ce\u897f\u533a",
        "area_parent_id": "462",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4864",
        "area_name": "\u5927\u901a\u56de\u65cf\u571f\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "462",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4865",
        "area_name": "\u6e5f\u4e2d\u53bf",
        "area_parent_id": "462",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4866",
        "area_name": "\u6e5f\u6e90\u53bf",
        "area_parent_id": "462",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4867",
        "area_name": "\u4e50\u90fd\u53bf",
        "area_parent_id": "463",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4868",
        "area_name": "\u4e92\u52a9\u571f\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "463",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4869",
        "area_name": "\u5316\u9686\u56de\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "463",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4870",
        "area_name": "\u5e73\u5b89\u53bf",
        "area_parent_id": "463",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4871",
        "area_name": "\u5faa\u5316\u6492\u62c9\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "463",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4872",
        "area_name": "\u6c11\u548c\u56de\u65cf\u571f\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "463",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4873",
        "area_name": "\u521a\u5bdf\u53bf",
        "area_parent_id": "464",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4874",
        "area_name": "\u6d77\u664f\u53bf",
        "area_parent_id": "464",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4875",
        "area_name": "\u7941\u8fde\u53bf",
        "area_parent_id": "464",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4876",
        "area_name": "\u95e8\u6e90\u56de\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "464",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4877",
        "area_name": "\u540c\u4ec1\u53bf",
        "area_parent_id": "465",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4878",
        "area_name": "\u5c16\u624e\u53bf",
        "area_parent_id": "465",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4879",
        "area_name": "\u6cb3\u5357\u8499\u53e4\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "465",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4880",
        "area_name": "\u6cfd\u5e93\u53bf",
        "area_parent_id": "465",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4881",
        "area_name": "\u5171\u548c\u53bf",
        "area_parent_id": "466",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4882",
        "area_name": "\u5174\u6d77\u53bf",
        "area_parent_id": "466",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4883",
        "area_name": "\u540c\u5fb7\u53bf",
        "area_parent_id": "466",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4884",
        "area_name": "\u8d35\u5357\u53bf",
        "area_parent_id": "466",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4885",
        "area_name": "\u8d35\u5fb7\u53bf",
        "area_parent_id": "466",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4886",
        "area_name": "\u4e45\u6cbb\u53bf",
        "area_parent_id": "467",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4887",
        "area_name": "\u739b\u591a\u53bf",
        "area_parent_id": "467",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4888",
        "area_name": "\u739b\u6c81\u53bf",
        "area_parent_id": "467",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4889",
        "area_name": "\u73ed\u739b\u53bf",
        "area_parent_id": "467",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4890",
        "area_name": "\u7518\u5fb7\u53bf",
        "area_parent_id": "467",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4891",
        "area_name": "\u8fbe\u65e5\u53bf",
        "area_parent_id": "467",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4892",
        "area_name": "\u56ca\u8c26\u53bf",
        "area_parent_id": "468",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4893",
        "area_name": "\u66f2\u9ebb\u83b1\u53bf",
        "area_parent_id": "468",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4894",
        "area_name": "\u6742\u591a\u53bf",
        "area_parent_id": "468",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4895",
        "area_name": "\u6cbb\u591a\u53bf",
        "area_parent_id": "468",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4896",
        "area_name": "\u7389\u6811\u53bf",
        "area_parent_id": "468",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4897",
        "area_name": "\u79f0\u591a\u53bf",
        "area_parent_id": "468",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4898",
        "area_name": "\u4e4c\u5170\u53bf",
        "area_parent_id": "469",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4899",
        "area_name": "\u51b7\u6e56\u884c\u59d4",
        "area_parent_id": "469",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4900",
        "area_name": "\u5927\u67f4\u65e6\u884c\u59d4",
        "area_parent_id": "469",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4901",
        "area_name": "\u5929\u5cfb\u53bf",
        "area_parent_id": "469",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4902",
        "area_name": "\u5fb7\u4ee4\u54c8\u5e02",
        "area_parent_id": "469",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4903",
        "area_name": "\u683c\u5c14\u6728\u5e02",
        "area_parent_id": "469",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4904",
        "area_name": "\u832b\u5d16\u884c\u59d4",
        "area_parent_id": "469",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4905",
        "area_name": "\u90fd\u5170\u53bf",
        "area_parent_id": "469",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4906",
        "area_name": "\u5174\u5e86\u533a",
        "area_parent_id": "470",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4907",
        "area_name": "\u6c38\u5b81\u53bf",
        "area_parent_id": "470",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4908",
        "area_name": "\u7075\u6b66\u5e02",
        "area_parent_id": "470",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4909",
        "area_name": "\u897f\u590f\u533a",
        "area_parent_id": "470",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4910",
        "area_name": "\u8d3a\u5170\u53bf",
        "area_parent_id": "470",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4911",
        "area_name": "\u91d1\u51e4\u533a",
        "area_parent_id": "470",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4912",
        "area_name": "\u5927\u6b66\u53e3\u533a",
        "area_parent_id": "471",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4913",
        "area_name": "\u5e73\u7f57\u53bf",
        "area_parent_id": "471",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4914",
        "area_name": "\u60e0\u519c\u533a",
        "area_parent_id": "471",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4915",
        "area_name": "\u5229\u901a\u533a",
        "area_parent_id": "472",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4916",
        "area_name": "\u540c\u5fc3\u53bf",
        "area_parent_id": "472",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4917",
        "area_name": "\u76d0\u6c60\u53bf",
        "area_parent_id": "472",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4918",
        "area_name": "\u9752\u94dc\u5ce1\u5e02",
        "area_parent_id": "472",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4919",
        "area_name": "\u539f\u5dde\u533a",
        "area_parent_id": "473",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4920",
        "area_name": "\u5f6d\u9633\u53bf",
        "area_parent_id": "473",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4921",
        "area_name": "\u6cfe\u6e90\u53bf",
        "area_parent_id": "473",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4922",
        "area_name": "\u897f\u5409\u53bf",
        "area_parent_id": "473",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4923",
        "area_name": "\u9686\u5fb7\u53bf",
        "area_parent_id": "473",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4924",
        "area_name": "\u4e2d\u5b81\u53bf",
        "area_parent_id": "474",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4925",
        "area_name": "\u6c99\u5761\u5934\u533a",
        "area_parent_id": "474",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4926",
        "area_name": "\u6d77\u539f\u53bf",
        "area_parent_id": "474",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4927",
        "area_name": "\u4e1c\u5c71\u533a",
        "area_parent_id": "475",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4928",
        "area_name": "\u4e4c\u9c81\u6728\u9f50\u53bf",
        "area_parent_id": "475",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4929",
        "area_name": "\u5929\u5c71\u533a",
        "area_parent_id": "475",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4930",
        "area_name": "\u5934\u5c6f\u6cb3\u533a",
        "area_parent_id": "475",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4931",
        "area_name": "\u65b0\u5e02\u533a",
        "area_parent_id": "475",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4932",
        "area_name": "\u6c34\u78e8\u6c9f\u533a",
        "area_parent_id": "475",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4933",
        "area_name": "\u6c99\u4f9d\u5df4\u514b\u533a",
        "area_parent_id": "475",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4934",
        "area_name": "\u8fbe\u5742\u57ce\u533a",
        "area_parent_id": "475",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4935",
        "area_name": "\u4e4c\u5c14\u79be\u533a",
        "area_parent_id": "476",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4936",
        "area_name": "\u514b\u62c9\u739b\u4f9d\u533a",
        "area_parent_id": "476",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4937",
        "area_name": "\u72ec\u5c71\u5b50\u533a",
        "area_parent_id": "476",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4938",
        "area_name": "\u767d\u78b1\u6ee9\u533a",
        "area_parent_id": "476",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4939",
        "area_name": "\u5410\u9c81\u756a\u5e02",
        "area_parent_id": "477",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4940",
        "area_name": "\u6258\u514b\u900a\u53bf",
        "area_parent_id": "477",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4941",
        "area_name": "\u912f\u5584\u53bf",
        "area_parent_id": "477",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4942",
        "area_name": "\u4f0a\u543e\u53bf",
        "area_parent_id": "478",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4943",
        "area_name": "\u54c8\u5bc6\u5e02",
        "area_parent_id": "478",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4944",
        "area_name": "\u5df4\u91cc\u5764\u54c8\u8428\u514b\u81ea\u6cbb\u53bf",
        "area_parent_id": "478",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4945",
        "area_name": "\u5409\u6728\u8428\u5c14\u53bf",
        "area_parent_id": "479",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4946",
        "area_name": "\u547c\u56fe\u58c1\u53bf",
        "area_parent_id": "479",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4947",
        "area_name": "\u5947\u53f0\u53bf",
        "area_parent_id": "479",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4948",
        "area_name": "\u660c\u5409\u5e02",
        "area_parent_id": "479",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4949",
        "area_name": "\u6728\u5792\u54c8\u8428\u514b\u81ea\u6cbb\u53bf",
        "area_parent_id": "479",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4950",
        "area_name": "\u739b\u7eb3\u65af\u53bf",
        "area_parent_id": "479",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4951",
        "area_name": "\u7c73\u6cc9\u5e02",
        "area_parent_id": "479",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4952",
        "area_name": "\u961c\u5eb7\u5e02",
        "area_parent_id": "479",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4953",
        "area_name": "\u535a\u4e50\u5e02",
        "area_parent_id": "480",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4954",
        "area_name": "\u6e29\u6cc9\u53bf",
        "area_parent_id": "480",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4955",
        "area_name": "\u7cbe\u6cb3\u53bf",
        "area_parent_id": "480",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4956",
        "area_name": "\u535a\u6e56\u53bf",
        "area_parent_id": "481",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4957",
        "area_name": "\u548c\u7855\u53bf",
        "area_parent_id": "481",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4958",
        "area_name": "\u548c\u9759\u53bf",
        "area_parent_id": "481",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4959",
        "area_name": "\u5c09\u7281\u53bf",
        "area_parent_id": "481",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4960",
        "area_name": "\u5e93\u5c14\u52d2\u5e02",
        "area_parent_id": "481",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4961",
        "area_name": "\u7109\u8006\u56de\u65cf\u81ea\u6cbb\u53bf",
        "area_parent_id": "481",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4962",
        "area_name": "\u82e5\u7f8c\u53bf",
        "area_parent_id": "481",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4963",
        "area_name": "\u8f6e\u53f0\u53bf",
        "area_parent_id": "481",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4964",
        "area_name": "\u4e4c\u4ec0\u53bf",
        "area_parent_id": "482",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4965",
        "area_name": "\u5e93\u8f66\u53bf",
        "area_parent_id": "482",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4966",
        "area_name": "\u62dc\u57ce\u53bf",
        "area_parent_id": "482",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4967",
        "area_name": "\u65b0\u548c\u53bf",
        "area_parent_id": "482",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4968",
        "area_name": "\u67ef\u576a\u53bf",
        "area_parent_id": "482",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4969",
        "area_name": "\u6c99\u96c5\u53bf",
        "area_parent_id": "482",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4970",
        "area_name": "\u6e29\u5bbf\u53bf",
        "area_parent_id": "482",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4971",
        "area_name": "\u963f\u514b\u82cf\u5e02",
        "area_parent_id": "482",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4972",
        "area_name": "\u963f\u74e6\u63d0\u53bf",
        "area_parent_id": "482",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4973",
        "area_name": "\u4e4c\u6070\u53bf",
        "area_parent_id": "483",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4974",
        "area_name": "\u963f\u514b\u9676\u53bf",
        "area_parent_id": "483",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4975",
        "area_name": "\u963f\u5408\u5947\u53bf",
        "area_parent_id": "483",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4976",
        "area_name": "\u963f\u56fe\u4ec0\u5e02",
        "area_parent_id": "483",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4977",
        "area_name": "\u4f3d\u5e08\u53bf",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4978",
        "area_name": "\u53f6\u57ce\u53bf",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4979",
        "area_name": "\u5580\u4ec0\u5e02",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4980",
        "area_name": "\u5854\u4ec0\u5e93\u5c14\u5e72\u5854\u5409\u514b\u81ea\u6cbb\u53bf",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4981",
        "area_name": "\u5cb3\u666e\u6e56\u53bf",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4982",
        "area_name": "\u5df4\u695a\u53bf",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4983",
        "area_name": "\u6cfd\u666e\u53bf",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4984",
        "area_name": "\u758f\u52d2\u53bf",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4985",
        "area_name": "\u758f\u9644\u53bf",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4986",
        "area_name": "\u82f1\u5409\u6c99\u53bf",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4987",
        "area_name": "\u838e\u8f66\u53bf",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4988",
        "area_name": "\u9ea6\u76d6\u63d0\u53bf",
        "area_parent_id": "484",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4989",
        "area_name": "\u4e8e\u7530\u53bf",
        "area_parent_id": "485",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4990",
        "area_name": "\u548c\u7530\u53bf",
        "area_parent_id": "485",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4991",
        "area_name": "\u548c\u7530\u5e02",
        "area_parent_id": "485",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4992",
        "area_name": "\u58a8\u7389\u53bf",
        "area_parent_id": "485",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4993",
        "area_name": "\u6c11\u4e30\u53bf",
        "area_parent_id": "485",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4994",
        "area_name": "\u6d1b\u6d66\u53bf",
        "area_parent_id": "485",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4995",
        "area_name": "\u76ae\u5c71\u53bf",
        "area_parent_id": "485",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4996",
        "area_name": "\u7b56\u52d2\u53bf",
        "area_parent_id": "485",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4997",
        "area_name": "\u4f0a\u5b81\u53bf",
        "area_parent_id": "486",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4998",
        "area_name": "\u4f0a\u5b81\u5e02",
        "area_parent_id": "486",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "4999",
        "area_name": "\u594e\u5c6f\u5e02",
        "area_parent_id": "486",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5000",
        "area_name": "\u5bdf\u5e03\u67e5\u5c14\u9521\u4f2f\u81ea\u6cbb\u53bf",
        "area_parent_id": "486",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5001",
        "area_name": "\u5c3c\u52d2\u514b\u53bf",
        "area_parent_id": "486",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5002",
        "area_name": "\u5de9\u7559\u53bf",
        "area_parent_id": "486",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5003",
        "area_name": "\u65b0\u6e90\u53bf",
        "area_parent_id": "486",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5004",
        "area_name": "\u662d\u82cf\u53bf",
        "area_parent_id": "486",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5005",
        "area_name": "\u7279\u514b\u65af\u53bf",
        "area_parent_id": "486",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5006",
        "area_name": "\u970d\u57ce\u53bf",
        "area_parent_id": "486",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5007",
        "area_name": "\u4e4c\u82cf\u5e02",
        "area_parent_id": "487",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5008",
        "area_name": "\u548c\u5e03\u514b\u8d5b\u5c14\u8499\u53e4\u81ea\u6cbb\u53bf",
        "area_parent_id": "487",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5009",
        "area_name": "\u5854\u57ce\u5e02",
        "area_parent_id": "487",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5010",
        "area_name": "\u6258\u91cc\u53bf",
        "area_parent_id": "487",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5011",
        "area_name": "\u6c99\u6e7e\u53bf",
        "area_parent_id": "487",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5012",
        "area_name": "\u88d5\u6c11\u53bf",
        "area_parent_id": "487",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5013",
        "area_name": "\u989d\u654f\u53bf",
        "area_parent_id": "487",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5014",
        "area_name": "\u5409\u6728\u4e43\u53bf",
        "area_parent_id": "488",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5015",
        "area_name": "\u54c8\u5df4\u6cb3\u53bf",
        "area_parent_id": "488",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5016",
        "area_name": "\u5bcc\u8574\u53bf",
        "area_parent_id": "488",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5017",
        "area_name": "\u5e03\u5c14\u6d25\u53bf",
        "area_parent_id": "488",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5018",
        "area_name": "\u798f\u6d77\u53bf",
        "area_parent_id": "488",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5019",
        "area_name": "\u963f\u52d2\u6cf0\u5e02",
        "area_parent_id": "488",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5020",
        "area_name": "\u9752\u6cb3\u53bf",
        "area_parent_id": "488",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5021",
        "area_name": "\u77f3\u6cb3\u5b50\u5e02",
        "area_parent_id": "489",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5022",
        "area_name": "\u963f\u62c9\u5c14\u5e02",
        "area_parent_id": "490",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5023",
        "area_name": "\u56fe\u6728\u8212\u514b\u5e02",
        "area_parent_id": "491",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "5024",
        "area_name": "\u4e94\u5bb6\u6e20\u5e02",
        "area_parent_id": "492",
        "area_deep": "3",
        "area_region": null
    }, {
        "area_id": "45055",
        "area_name": "\u6d77\u5916",
        "area_parent_id": "35",
        "area_deep": "2",
        "area_region": null
    }];

    this.getAreaList = function(pId){
        var area = [];

        for(var i = 0 ; i < areaList.length ;i++){
            if(areaList[i].area_parent_id == pId){
                area.push(areaList[i]);
            }
        }

        return area;
    };

    this.getCityByProvince = function(pName){
        var list = getAreaList('0');
        var city = [];
        for(var i = 0 ; i < list.length ; i++){
            var p = list[i];
            var name = unescape(p.area_name.replace(/\\u/g, "%u"));
            if(pName.indexOf(name) != -1){
                var pId = p.area_id;
                city = getAreaList(pId);
                break;
            }
        }

        return city;
    };

    this.getArea = function(id){
        var area = {};

        for(var i = 0 ; i < areaList.length ;i++){
            if(areaList[i].area_id == id){
                area = areaList[i];
                break;
            }
        }

        return area;
    };

    return this;
});
